...
const adminAccessCurrentUsernameEl = document.getElementById("admin-access-current-username");
...
const adminAccessConfirmPasswordEl = document.getElementById("admin-access-confirm-password");
...
const metricPendingLandlordAccessEl = document.getElementById("metric-pending-landlord-access");
...
const accountPasswordRecoveryBodyEl = document.getElementById("account-password-recovery-body");
const refreshAccountPasswordRecoveryBtn = document.getElementById("refresh-account-password-recovery");
...
const adminBillingBuildingSelectEl = document.getElementById("admin-billing-building-select");
const refreshAdminBillingBtnEl = document.getElementById("refresh-admin-billing");
const deleteAdminBillingBuildingBtnEl = document.getElementById("delete-admin-billing-building");
const adminBillingSummaryEl = document.getElementById("admin-billing-summary");
const adminMonthlyCombinedChargeFormEl = document.getElementById("admin-monthly-combined-charge-form");
const adminMonthlyCombinedChargeMonthEl = document.getElementById("admin-monthly-combined-charge-month");
const adminMonthlyCombinedChargeAmountEl = document.getElementById("admin-monthly-combined-charge-amount");
...
const adminUtilityBillFixedChargeEl = document.getElementById("admin-utility-bill-fixed-charge");
const adminUtilityBillPreviousReadingEl = document.getElementById("admin-utility-bill-previous-reading");
const adminUtilityBillCurrentReadingEl = document.getElementById("admin-utility-bill-current-reading");
...
function getBuildingDisplayNameById(buildingId, fallback = "Selected building") {
    return getBuildingDisplayName(state.buildings.find((item) => item.id === buildingId), fallback);
}
...
function renderAdminAccess() {
    ...
    const source = state.adminAccess?.source === "app_state"
...
function renderOverview(overview) {
    ...
    if (metricPendingLandlordAccessEl instanceof HTMLElement) {
        metricPendingLandlordAccessEl.textContent = String(overview?.pendingLandlordAccess ?? 0);
    }
    if (metricResidentRecoveryEl instanceof HTMLElement) {
        metricResidentRecoveryEl.textContent = String(overview?.residentPasswordRecoveryPending ?? 0);
    }
    if (metricAccountRecoveryEl instanceof HTMLElement) {
        metricAccountRecoveryEl.textContent = String(overview?.accountPasswordRecoveryPending ?? 0);
    }
    if (platformSummaryEl instanceof HTMLElement) {
        ...
        const pendingRecoveries = Number(overview?.residentPasswordRecoveryPending ?? 0) +
...
function renderLandlordAccessRequests(requests) {
    ...
    visibleRequests.forEach((request) => {
        const row = document.createElement("tr");
        const reviewedBy = request.reviewedBy?.fullName ?? request.reviewedBy?.email ?? "legacy admin";
      ...
      `;
            ...
            const handleDecision = (action) => {
                ...
                void (async () => {
                    try {
                        await submitLandlordAccessDecision(request.id, action, noteInput instanceof HTMLInputElement ? noteInput.value.trim() : "");
            ...
            const canRevokeLandlord = request.status === "approved" && request.user?.role === "landlord";
        ...
        `;
                const revokeButton = row.querySelector('button[data-action="revoke-landlord"]');
                if (revokeButton instanceof HTMLButtonElement) {
                    revokeButton.addEventListener("click", () => {
                        const shouldProceed = window.confirm(`Remove landlord role for ${request.user.fullName} (${request.user.phone})?\nThis revokes active sessions and clears building ownership links.`);
                        if (!shouldProceed) {
                            return;
                        }
                        const noteRaw = window.prompt("Optional admin note for this landlord removal. Leave blank to skip.");
                        ...
                        void (async () => {
                            try {
                                ...
                                setStatus(`Removed landlord role from ${request.user.fullName}. Cleared ${Number(revoked.clearedBuildingsCount ?? 0)} building owner link(s).`);
                                await Promise.all([
                                    loadOverview(),
...
async function submitPasswordRecoveryDecision(requestId, action, temporaryPassword, note) {
    await requestJson(`/api/admin/auth/resident/password-recovery-requests/${encodeURIComponent(requestId)}`, {
        method: "PATCH",
        headers: {
            "content-type": "application/json"
        },
...
async function submitAccountPasswordRecoveryDecision(requestId, action, temporaryPassword, note) {
    await requestJson(`/api/admin/auth/account/password-recovery-requests/${encodeURIComponent(requestId)}`, {
        method: "PATCH",
        headers: {
            "content-type": "application/json"
        },
...
function renderPasswordRecoveryRequests(requests) {
    ...
    requests.forEach((request) => {
      ...
      `;
            const tempPasswordInput = row.querySelector('input[data-action="temporary-password"]');
            const noteInput = row.querySelector('input[data-action="note"]');
            const approveButton = row.querySelector('button[data-action="approve"]');
            const rejectButton = row.querySelector('button[data-action="reject"]');
            const handleDecision = (action) => {
                ...
                void (async () => {
                    try {
                        const temporaryPassword = tempPasswordInput instanceof HTMLInputElement
                            ? tempPasswordInput.value.trim()
                            : "";
                        if (action === "approve" && temporaryPassword.length < 8) {
                            throw new Error("Temporary password must be at least 8 characters.");
                        }
                        await submitPasswordRecoveryDecision(request.id, action, temporaryPassword, noteInput instanceof HTMLInputElement ? noteInput.value.trim() : "");
                        setStatus(`Resident recovery ${request.id.slice(0, 8)} ${action === "approve" ? "approved" : "rejected"}.`);
...
function renderAccountPasswordRecoveryRequests(requests) {
    ...
    if (!Array.isArray(requests) || requests.length === 0) {
        appendEmptyRow(accountPasswordRecoveryBodyEl, 7, "No landlord account recovery requests found.");
        return;
    }
    requests.forEach((request) => {
        const row = document.createElement("tr");
        ...
        const reviewedBy = request.reviewedByRole ?? "-";
        const reviewedAt = request.reviewedAt ? formatDateTime(request.reviewedAt) : "-";
        const identifier = request.identifierType === "phone"
            ...
            const handleDecision = (action) => {
                ...
                void (async () => {
                    try {
                        ...
                        await submitAccountPasswordRecoveryDecision(request.id, action, temporaryPassword, noteInput instanceof HTMLInputElement ? noteInput.value.trim() : "");
                        setStatus(`Account recovery ${request.id.slice(0, 8)} ${action === "approve" ? "approved" : "rejected"}.`);
...
function renderLandlordPortfolios(buildings) {
    ...
    if (portfolios.length === 0) {
        appendEmptyRow(landlordPortfoliosBodyEl, 6, "No landlord portfolios yet. Assign buildings to landlord accounts first.");
        return;
    }
...
function renderOwnershipGaps(buildings) {
    ...
    if (gaps.length === 0) {
        appendEmptyRow(ownershipGapBodyEl, 6, "Every registered building currently has an assigned landlord owner.");
        return;
    }
...
function renderRegistrySummary() {
    ...
    const trackedUnits = buildings.reduce((sum, item) => sum + (typeof item.units === "number" && Number.isFinite(item.units) ? item.units : 0), 0);
...
async function deleteBuildingFromAdmin(buildingId, buildingName) {
    const shouldProceed = window.confirm(`Delete ${buildingName}? This permanently removes the building and linked unit records.`);
    if (!shouldProceed) {
        return false;
    }
...
function renderAdminUtilityPayments(rows) {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
        appendEmptyRow(adminUtilityPaymentsBodyEl, 9, "No utility payments found for the selected building.");
        return;
    }
    rows.forEach((item) => {
        const row = document.createElement("tr");
...
function renderAdminUtilityRegistry(rows) {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
        appendEmptyRow(adminUtilityRegistryBodyEl, 6, "No rooms found for the selected building.");
        return;
    }
    rows
        .slice()
...
function syncAdminBillingBuildingOptions() {
    ...
    const sortedBuildings = [...state.buildings].sort((left, right) => `${left.name}:${left.id}`.localeCompare(`${right.name}:${right.id}`));
...
async function loadAdminUtilityRegistry() {
    ...
    const payload = await requestJson(`/api/admin/buildings/${encodeURIComponent(buildingId)}/utility-registry`);
...
async function loadAdminUtilityPayments() {
    ...
    const payload = await requestJson(`/api/admin/utilities/payments?buildingId=${encodeURIComponent(buildingId)}&limit=120`);
...
async function loadAdminMonthlyCombinedCharge() {
    ...
    const payload = await requestJson(`/api/admin/buildings/${encodeURIComponent(buildingId)}/monthly-combined-utility-charge?billingMonth=${encodeURIComponent(billingMonth)}`);
    if (adminMonthlyCombinedChargeAmountEl instanceof HTMLInputElement) {
        adminMonthlyCombinedChargeAmountEl.value =
...
function renderBuildings(rows) {
    ...
    rows
        ...
        const ownerLabel = building.landlordOwnerName
            ? `${escapeHtml(building.landlordOwnerName)}<br /><small>${escapeHtml(building.landlordOwnerPhone ?? "-")}</small>`
...
function createBuildingPayload() {
    const unitsRaw = buildingUnitsEl instanceof HTMLInputElement
    ...
    return {
        name: buildingNameEl instanceof HTMLInputElement
            ? String(buildingNameEl.value ?? "").trim()
            : "",
        county: buildingCountyEl instanceof HTMLInputElement
            ? String(buildingCountyEl.value ?? "").trim()
            : "",
        address: buildingAddressEl instanceof HTMLInputElement
            ? String(buildingAddressEl.value ?? "").trim()
            : "",
        units,
        cctvStatus: buildingCctvStatusEl instanceof HTMLSelectElement
...
async function loadPasswordRecoveryRequests() {
    const payload = await requestJson("/api/admin/auth/resident/password-recovery-requests?limit=500");
    renderPasswordRecoveryRequests(payload.data ?? []);
}
async function loadAccountPasswordRecoveryRequests() {
    const payload = await requestJson("/api/admin/auth/account/password-recovery-requests?limit=500");
...
if (buildingsBodyEl instanceof HTMLElement) {
    buildingsBodyEl.addEventListener("click", (event) => {
        ...
        if (action === "assign-building-landlord") {
            ...
            const identifierRaw = window.prompt(`Assign landlord for ${buildingName}.\nEnter landlord phone or email:`, currentOwnerPhone);
            ...
            void (async () => {
                try {
                    const payload = await requestJson(`/api/admin/buildings/${encodeURIComponent(buildingId)}/landlord`, {
                        method: "PATCH",
                        headers: {
                            "content-type": "application/json"
                        },
                    ...
                    setStatus(`Assigned ${landlord?.fullName ?? "landlord"} to ${buildingName}.`);
                    await Promise.all([
                        loadOverview(),
                        loadBuildings(),
                        loadLandlordAccessRequests()
...
if (adminUtilityRegistryBodyEl instanceof HTMLElement) {
    adminUtilityRegistryBodyEl.addEventListener("click", (event) => {
        ...
        const electricityInput = row.querySelector('input[data-field="electricityFixedChargeKsh"]');
        const combinedInput = row.querySelector('input[data-field="combinedUtilityChargeKsh"]');
        clearError();
        target.disabled = true;
        void (async () => {
            try {
                await requestJson(`/api/admin/buildings/${encodeURIComponent(buildingId)}/utility-registry`, {
                    method: "PUT",
                    headers: {
                        "content-type": "application/json"
                    },
                    body: JSON.stringify({
                        rows: [
                            {
                                houseNumber,
                                waterFixedChargeKsh: waterInput instanceof HTMLInputElement
                                    ? toOptionalNumber(waterInput.value)
                                    : undefined,
                                electricityFixedChargeKsh: electricityInput instanceof HTMLInputElement
                                    ? toOptionalNumber(electricityInput.value)
                                    : undefined,
                                combinedUtilityChargeKsh: combinedInput instanceof HTMLInputElement
...
if (adminMonthlyCombinedChargeFormEl instanceof HTMLFormElement) {
    adminMonthlyCombinedChargeFormEl.addEventListener("submit", (event) => {
        ...
        const submitButton = adminMonthlyCombinedChargeFormEl.querySelector('button[type="submit"]');
        if (submitButton instanceof HTMLButtonElement) {
            submitButton.disabled = true;
        }
        void (async () => {
            try {
                await requestJson(`/api/admin/buildings/${encodeURIComponent(buildingId)}/monthly-combined-utility-charge`, {
                    method: "PUT",
                    headers: {
                        "content-type": "application/json"
                    },
                ...
                setStatus(`Monthly combined utility charge saved for ${buildingId} (${formatBillingMonth(billingMonth)}).`);
...
if (adminUtilityBillFormEl instanceof HTMLFormElement) {
    adminUtilityBillFormEl.addEventListener("submit", (event) => {
        ...
        void (async () => {
            try {
                await requestJson(`/api/admin/utilities/${encodeURIComponent(utilityType)}/${encodeURIComponent(houseNumber)}/bills`, {
                    method: "POST",
                    headers: {
                        "content-type": "application/json"
                    },
                ...
                setStatus(`${utilityType} bill posted for house ${houseNumber} (${formatBillingMonth(billingMonth)}) in ${buildingId}.`);
...
if (adminUtilityPaymentFormEl instanceof HTMLFormElement) {
    adminUtilityPaymentFormEl.addEventListener("submit", (event) => {
        ...
        const payload = {
            ...
            providerReference: String(adminUtilityPaymentReferenceEl?.value ?? "").trim() || undefined,
        ...
        const submitButton = adminUtilityPaymentFormEl.querySelector('button[type="submit"]');
        if (submitButton instanceof HTMLButtonElement) {
            submitButton.disabled = true;
        }
        void (async () => {
            try {
                const response = await requestJson(`/api/admin/utilities/${encodeURIComponent(utilityType)}/${encodeURIComponent(houseNumber)}/payments`, {
                    method: "POST",
                    headers: {
                        "content-type": "application/json"
                    },
                ...
                setStatus(`${utilityType} payment recorded for house ${houseNumber}${coverage ? ` covering ${coverage}` : ""}.`);
...
adminUtilityPaymentsBodyEl?.addEventListener("click", (event) => {
    ...
    const confirmed = window.confirm(`Delete ${provider || "manual"} ${utilityType} payment ${reference || paymentId} for house ${houseNumber}? M-PESA records stay locked.`);
    if (!confirmed) {
        return;
    }
    ...
    void (async () => {
        try {
            await requestJson(`/api/admin/utilities/${encodeURIComponent(utilityType)}/${encodeURIComponent(houseNumber)}/payments/${encodeURIComponent(paymentId)}?buildingId=${encodeURIComponent(buildingId)}`, { method: "DELETE" });
...
deleteAdminBillingBuildingBtnEl?.addEventListener("click", () => {
    ...
    const buildingName = state.buildings.find((item) => item.id === buildingId)?.name ?? buildingId;
