...
function renderBuildings(buildings) {
    ...
    registryCountEl.textContent = `${buildings.length} building${buildings.length === 1 ? "" : "s"}`;
    buildings.forEach((building, index) => {
        ...
        fragment.querySelector(".incidents").textContent = String(building.openIncidents ?? 0);
        fragment.querySelector(".updated-at").textContent = formatUpdatedAt(building.updatedAt);
...
function renderCctv(buildings) {
    ...
    cctvCountEl.textContent = `${buildings.length} building${buildings.length === 1 ? "" : "s"}`;
    buildings.forEach((building, index) => {
...
function renderBuildingOptions(buildings) {
    ...
    const selectedBuildingId = state.selectedBuildingId && buildings.some((item) => item.id === state.selectedBuildingId)
...
function renderWifiPackages(packages) {
    ...
    packages.forEach((pkg) => {
        ...
        fragment.querySelector(".wifi-package-meta").textContent = `${pkg.hours}h • KSh ${pkg.priceKsh}`;
...
async function pollPaymentStatus(checkoutReference, attempt = 0) {
    try {
        const response = await fetch(`/api/wifi/payments/${encodeURIComponent(checkoutReference)}`);
        if (!response.ok) {
        ...
        if (payment.status === "active") {
            stopPaymentPolling();
            wifiStatusEl.textContent = `Connected. Voucher ${payment.voucher.username} / ${payment.voucher.password} (expires ${new Date(payment.voucher.expiresAt).toLocaleString()}).`;
            return;
        }
...
async function loadWifiPackagesForBuilding(buildingId) {
    ...
    const response = await fetch(`/api/wifi/packages?buildingId=${encodeURIComponent(buildingId)}`);
    if (!response.ok) {
...
wifiBuildingEl.addEventListener("change", () => {
    ...
    void loadWifiPackagesForBuilding(state.selectedBuildingId).catch((error) => {
        const message = error instanceof Error ? error.message : "Unable to load Wi-Fi packages.";
        showWifiError(message);
    });
});
