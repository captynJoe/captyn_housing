...
const landlordPasswordChangeFormEl = document.getElementById("landlord-password-change-form");
const landlordNewPasswordEl = document.getElementById("landlord-new-password");
const landlordConfirmPasswordEl = document.getElementById("landlord-confirm-password");
const landlordPasswordChangeBtnEl = document.getElementById("landlord-password-change-btn");
const landlordPasswordChangeErrorEl = document.getElementById("landlord-password-change-error");
const landlordForgotFormEl = document.getElementById("landlord-forgot-form");
const landlordForgotIdentifierEl = document.getElementById("landlord-forgot-identifier");
...
function setManagerMode(nextMode) {
    ...
    const statusMessages = {
        landlord: "Landlord sign-in accepts email, phone, or the recovery username for legacy manager access.",
        staff: "Staff sign-in uses the email or phone number issued by the landlord.",
        caretaker: "House manager sign-in uses phone number and the approved verification house."
    };
...
function identifierLoginPayload(identifier, password) {
    ...
    const looksLikePhone = /^(\+254|254|0)\d{9}$/.test(normalized.replace(/[\s-]/g, ""));
...
async function handleSignedInRole(role, identity = {}) {
    ...
    if (role === "tenant") {
        ...
        const phoneMask = typeof identity.phoneMask === "string" ? identity.phoneMask : "";
        if (email || phoneMask) {
            setStatus(`Signed in as resident (${email || "no email"}${phoneMask ? ` • ${phoneMask}` : ""}). Use the resident portal for this account.`);
        }
        else {
...
function showPermanentPasswordForm(role, identity = {}) {
    ...
    setStatus(`Temporary password accepted for ${label}. Set a permanent password to continue.`);
...
async function signInStaff() {
    ...
    if (!looksLikeEmail(identifier) && !looksLikeKenyaPhone(identifier)) {
        showPanelError(loginErrorEl, "Staff sign-in uses email or phone. Choose Landlord if you need recovery username access.", { reveal: true });
        return;
    }
    if (!password) {
...
async function signInCaretaker() {
    ...
    try {
        ...
        if (!password) {
            ...
            showPanelError(loginErrorEl, setupRequired
                ? "First-time setup required. Enter and confirm a new password below."
                : "Enter your house manager password.", { reveal: true });
        ...
        const message = error instanceof Error ? error.message : "Unable to sign in house manager.";
        showPanelError(loginErrorEl, message, { reveal: true });
        setStatus("Check the message above and try again.");
    }
    finally {
...
async function requestPasswordReset(event) {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : "Unable to submit recovery request.";
...
async function submitPermanentPasswordChange(event) {
    ...
    if (newPassword.length < 8) {
        showPanelError(landlordPasswordChangeErrorEl, "New password must be at least 8 characters.", { reveal: true });
        return;
    }
    if (newPassword !== confirmPassword) {
        showPanelError(landlordPasswordChangeErrorEl, "Confirmation password must match the new password.", { reveal: true });
        return;
    }
