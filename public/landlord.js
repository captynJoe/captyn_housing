...
const LANDLORD_SW_URL = "/resident-sw.js?v=20260717d";
...
const landlordNotificationsBadgeEl = document.getElementById("landlord-notifications-badge");
const landlordPushAlertsBtnEl = document.getElementById("landlord-push-alerts-btn");
const landlordNotificationsPanelEl = document.getElementById("landlord-notifications-panel");
const landlordNotificationsSummaryEl = document.getElementById("landlord-notifications-summary");
const landlordNotificationsListEl = document.getElementById("landlord-notifications-list");
const landlordNotificationsReadBtnEl = document.getElementById("landlord-notifications-read-btn");
const landlordNotificationsRefreshBtnEl = document.getElementById("landlord-notifications-refresh-btn");
...
const messageRulePaymentReceiptsEl = document.getElementById("message-rule-payment-receipts");
const messageRuleRentRemindersEl = document.getElementById("message-rule-rent-reminders");
const messageRuleUtilityRemindersEl = document.getElementById("message-rule-utility-reminders");
const messageRuleOverdueNoticesEl = document.getElementById("message-rule-overdue-notices");
...
const landlordGlobalSearchBuildingEl = document.getElementById("landlord-global-search-building");
const roomsWorkspacePathSegments = window.location.pathname.split("/").filter(Boolean);
const roomsWorkspaceBuildingId = roomsWorkspacePathSegments.length === 3 &&
    roomsWorkspacePathSegments[0] === "landlord" &&
...
const landlordFocusBuildingSelectEl = document.getElementById("landlord-focus-building-select");
const settingsBuildingSelectEl = document.getElementById("settings-building-select");
...
const closeCreateBuildingDrawerBtnEl = document.getElementById("close-create-building-drawer-btn");
...
const openDirectTenantDrawerBtnEl = document.getElementById("open-direct-tenant-drawer-btn");
const directTenantDrawerEl = document.getElementById("direct-tenant-drawer");
const directTenantDrawerBackdropEl = document.getElementById("direct-tenant-drawer-backdrop");
const closeDirectTenantDrawerBtnEl = document.getElementById("close-direct-tenant-drawer-btn");
...
const createBuildingHouseNumbersEl = document.getElementById("create-building-house-numbers");
...
const generateHouseNumbersBtnEl = document.getElementById("generate-house-numbers-btn");
...
const buildingCandidateRoomsCountEl = document.getElementById("building-candidate-rooms-count");
const buildingExistingRoomsEl = document.getElementById("building-existing-rooms");
const buildingExistingRoomsSummaryEl = document.getElementById("building-existing-rooms-summary");
...
const buildingManagementSummaryEl = document.getElementById("building-management-summary");
const buildingPhotoFormEl = document.getElementById("building-photo-form");
const buildingPhotoBuildingSelectEl = document.getElementById("building-photo-building-select");
...
const ownerStaffManagementPanelEl = document.getElementById("owner-staff-management-panel");
...
const caretakerManagementPanelEl = document.getElementById("caretaker-management-panel");
const caretakerFormEl = document.getElementById("caretaker-form");
const caretakerBuildingSelectEl = document.getElementById("caretaker-building-select");
...
const residentsOpenRentSheetBtnEl = document.getElementById("residents-open-rent-sheet-btn");
...
const landlordTicketFilterStatusEl = document.getElementById("landlord-ticket-filter-status");
const landlordTicketFilterQueueEl = document.getElementById("landlord-ticket-filter-queue");
const landlordTicketBuildingSelectEl = document.getElementById("landlord-ticket-building-select");
const landlordTicketsBodyEl = document.getElementById("landlord-tickets-body");
const refreshLandlordTicketsBtnEl = document.getElementById("refresh-landlord-tickets");
...
const paymentCorrectionBuildingSelectEl = document.getElementById("payment-correction-building-select");
const paymentCorrectionHouseEl = document.getElementById("payment-correction-house");
const paymentCorrectionUtilityTypeEl = document.getElementById("payment-correction-utility-type");
const refreshPaymentCorrectionsBtnEl = document.getElementById("refresh-payment-corrections");
const correctionRentPaymentsBodyEl = document.getElementById("correction-rent-payments-body");
const correctionUtilityPaymentsBodyEl = document.getElementById("correction-utility-payments-body");
const keyedPricesBodyEl = document.getElementById("keyed-prices-body");
const billingHoldCorrectionFormEl = document.getElementById("billing-hold-correction-form");
const billingHoldBuildingSelectEl = document.getElementById("billing-hold-building-select");
const billingHoldHouseEl = document.getElementById("billing-hold-house");
const billingHoldScopeEl = document.getElementById("billing-hold-scope");
const billingHoldUtilityTypeEl = document.getElementById("billing-hold-utility-type");
const billingHoldStartMonthEl = document.getElementById("billing-hold-start-month");
const billingHoldEndMonthEl = document.getElementById("billing-hold-end-month");
const billingHoldReasonEl = document.getElementById("billing-hold-reason");
const billingHoldHelpEl = document.getElementById("billing-hold-help");
...
const rentSheetDefaultMonthlyRentEl = document.getElementById("rent-sheet-default-monthly-rent");
const rentSheetDefaultDepositEl = document.getElementById("rent-sheet-default-deposit");
const rentSheetDefaultDueDayEl = document.getElementById("rent-sheet-default-due-day");
const rentSheetDefaultGraceDaysEl = document.getElementById("rent-sheet-default-grace-days");
const rentSheetChargeStartDateEl = document.getElementById("rent-sheet-charge-start-date");
...
const rentSheetStatusEl = document.getElementById("rent-sheet-status");
...
const overviewDailyIssuesEl = document.getElementById("overview-daily-issues");
...
const utilitySheetBuildingSelectEl = document.getElementById("utility-sheet-building-select");
const utilitySheetBillingMonthEl = document.getElementById("utility-sheet-billing-month");
...
const utilitySheetWaterFixedDefaultEl = document.getElementById("utility-sheet-water-fixed-default");
const utilitySheetElectricFixedDefaultEl = document.getElementById("utility-sheet-electric-fixed-default");
const utilitySheetBuildingCombinedChargeEl = document.getElementById("utility-sheet-building-combined-charge");
const utilitySheetCombinedChargeEl = document.getElementById("utility-sheet-combined-charge");
...
const overviewUtilityPaymentBackdropEl = document.getElementById("overview-utility-payment-backdrop");
const overviewUtilityPaymentModalEl = document.getElementById("overview-utility-payment-modal");
const closeOverviewUtilityPaymentBtnEl = document.getElementById("close-overview-utility-payment-btn");
const overviewUtilityPaymentFormEl = document.getElementById("overview-utility-payment-form");
const overviewUtilityPaymentSummaryEl = document.getElementById("overview-utility-payment-summary");
const overviewUtilityPaymentBuildingEl = document.getElementById("overview-utility-payment-building");
const overviewUtilityPaymentHouseEl = document.getElementById("overview-utility-payment-house");
const overviewUtilityPaymentTypeLabelEl = document.getElementById("overview-utility-payment-type-label");
const overviewUtilityPaymentMonthEl = document.getElementById("overview-utility-payment-month");
const overviewUtilityPaymentAmountEl = document.getElementById("overview-utility-payment-amount");
const overviewUtilityPaymentPaidAtEl = document.getElementById("overview-utility-payment-paid-at");
const overviewUtilityPaymentReferenceEl = document.getElementById("overview-utility-payment-reference");
const overviewUtilityPaymentHelpEl = document.getElementById("overview-utility-payment-help");
const overviewUtilityPaymentSubmitBtnEl = document.getElementById("overview-utility-payment-submit-btn");
const moveOutSettlementBackdropEl = document.getElementById("move-out-settlement-backdrop");
const moveOutSettlementModalEl = document.getElementById("move-out-settlement-modal");
const closeMoveOutSettlementBtnEl = document.getElementById("close-move-out-settlement-btn");
const moveOutSettlementFormEl = document.getElementById("move-out-settlement-form");
const moveOutSettlementSummaryEl = document.getElementById("move-out-settlement-summary");
const moveOutSettlementTotalsEl = document.getElementById("move-out-settlement-totals");
...
const moveOutSettlementSubmitBtnEl = document.getElementById("move-out-settlement-submit-btn");
...
const utilityBillPreviousReadingEl = document.getElementById("utility-bill-previous-reading");
const utilityBillCurrentReadingEl = document.getElementById("utility-bill-current-reading");
...
const utilityBillInputGuidanceEl = document.getElementById("utility-bill-input-guidance");
...
const moveOutSettlementReportSummaryEl = document.getElementById("move-out-settlement-report-summary");
const moveOutSettlementsBodyEl = document.getElementById("move-out-settlements-body");
const refreshMoveOutSettlementsBtnEl = document.getElementById("refresh-move-out-settlements");
...
const state = {
    ...
    payments: [],
    rentPayments: [],
    selectedPaymentCorrectionBuildingId: "all",
    expenditures: [],
    moveOutSettlements: [],
...
function utilityBuildingHouseLookupKey(utilityType, buildingId, houseNumber) {
    ...
    return `${String(utilityType ?? "").trim().toLowerCase()}::${normalizeLookupBuildingId(buildingId)}::${normalizedHouse}`;
}
function utilityBuildingHouseMonthLookupKey(utilityType, buildingId, houseNumber, billingMonth) {
...
function buildMeterIndex(rows) {
    const index = new Map();
    (Array.isArray(rows) ? rows : []).forEach((item) => {
        const key = utilityBuildingHouseLookupKey(item.utilityType, item.buildingId, item.houseNumber);
        if (key) {
            index.set(key, item);
        }
    });
...
function buildUtilityBillMonthIndex(rows) {
    const index = new Map();
    (Array.isArray(rows) ? rows : []).forEach((item) => {
        const key = utilityBuildingHouseMonthLookupKey(item.utilityType, item.buildingId, item.houseNumber, item.billingMonth);
        if (!key) {
            return;
        }
        const current = index.get(key);
...
function setBuildings(rows) {
    ...
    state.buildingById = new Map(state.buildings
...
function setPaymentAccess(rows) {
    ...
    state.paymentAccessByBuildingId = new Map(state.paymentAccess
...
function setPaymentProfiles(payload) {
    ...
    state.buildingPaymentProfileByBuildingId = new Map(assignments
        .map((item) => [normalizeLookupBuildingId(item.buildingId), item])
        .filter(([key]) => Boolean(key)));
}
function setPaymentInstructions(rows) {
    ...
    state.buildingPaymentInstructionByBuildingId = new Map(instructions
        .map((item) => [normalizeLookupBuildingId(item.buildingId), item])
        .filter(([key]) => Boolean(key)));
}
...
function setRegistryReadingBills(rows) {
    ...
    state.registryReadingBillByKey = buildUtilityBillMonthIndex(state.registryReadingBills);
}
...
function getLatestUtilityBill(utilityType, buildingId, houseNumber) {
    ...
    return (state.latestUtilityBillByKey.get(exactKey) ??
        state.latestUtilityBillByKey.get(utilityBuildingHouseLookupKey(utilityType, "", houseNumber)) ??
        null);
}
function getUtilityBillForMonth(utilityType, buildingId, houseNumber, billingMonth) {
    const exactKey = utilityBuildingHouseMonthLookupKey(utilityType, buildingId, houseNumber, billingMonth);
    if (!exactKey) {
        return null;
    }
    const legacyKey = utilityBuildingHouseMonthLookupKey(utilityType, "", houseNumber, billingMonth);
    return (state.registryReadingBillByKey.get(exactKey) ??
...
function setStatus(message) {
    const formatted = formatHouseManagementText(message);
...
function formatHouseManagementText(message) {
    return String(message ?? "")
        .replace(/\bcaretakers\b/gi, (match) => match[0] === "C" ? "House managers" : "house managers")
        .replace(/\bcaretaker\b/gi, (match) => match[0] === "C" ? "House manager" : "house manager");
}
...
function syncRoleScopedNavigation() {
    [...landlordNavButtons, ...landlordFocusTargetButtons].forEach((button) => {
        if (!(button instanceof HTMLButtonElement)) {
            return;
        }
        const view = button.dataset.landlordView || button.dataset.landlordFocusTargetView || "";
        button.classList.toggle("hidden", !isLandlordViewAvailableForRole(view));
    });
}
const applicationsNavButtonEl = landlordNavButtons.find((button) => button instanceof HTMLButtonElement && button.dataset.landlordView === "applications");
function updateApplicationsIndicator() {
    ...
    const summary = hasPending
        ? `${pendingCount} pending tenant application${pendingCount === 1 ? "" : "s"}. New resident access requests refresh automatically every 30 seconds.`
...
function updateOwnerNotificationControls() {
    ...
    if (landlordNotificationsBtnEl instanceof HTMLButtonElement) {
        ...
        landlordNotificationsBtnEl.setAttribute("aria-expanded", state.ownerNotificationsOpen ? "true" : "false");
    ...
    if (landlordNotificationsPanelEl instanceof HTMLElement) {
        landlordNotificationsPanelEl.classList.toggle("hidden", !owner || !state.ownerNotificationsOpen);
    }
...
function renderOwnerNotifications() {
    ...
    if (landlordNotificationsSummaryEl instanceof HTMLElement) {
        landlordNotificationsSummaryEl.textContent =
            notifications.length === 0
                ? "No owner alerts yet."
                : `${unreadCount} unread of ${notifications.length} recent alert${notifications.length === 1 ? "" : "s"}.`;
    }
    ...
    notifications.slice(0, 30).forEach((notification) => {
        ...
        item.className = `landlord-notification-item ${notification.read ? "is-read" : "is-unread"}`;
        if (notification.url) {
...
function setAutomaticMessageRules(rules = {}) {
    state.automaticMessageRules = {
        ...
        paymentReceiptsEnabled: typeof rules.paymentReceiptsEnabled === "boolean"
            ? rules.paymentReceiptsEnabled
            : true,
        rentRemindersEnabled: typeof rules.rentRemindersEnabled === "boolean"
            ? rules.rentRemindersEnabled
            : true,
        utilityRemindersEnabled: typeof rules.utilityRemindersEnabled === "boolean"
            ? rules.utilityRemindersEnabled
            : true,
        overdueNoticesEnabled: typeof rules.overdueNoticesEnabled === "boolean"
...
function renderMessageCenterBuildingOptions() {
    ...
    const current = state.selectedMessageBuildingId ||
...
function renderAutomaticMessageRules() {
    ...
    const buildingName = rules.buildingName ||
...
function updateMessageCenterRecipientFields() {
    ...
    if (messageRecipientScopeEl instanceof HTMLSelectElement) {
        const phoneOption = [...messageRecipientScopeEl.options].find((option) => option.value === "phone");
        if (phoneOption) {
...
function renderMessageCenter() {
    ...
    const selectedBuildingId = String(state.selectedMessageBuildingId || getFocusedBuildingId() || "").trim();
    const buildingLabel = selectedBuildingId
        ? getBuildingDisplayNameById(selectedBuildingId, selectedBuildingId)
        : "All buildings";
    const openIssues = Array.isArray(state.tickets)
        ? state.tickets.filter((ticket) => String(ticket?.status || "") !== "resolved").length
        : 0;
    if (messageCenterSummaryEl instanceof HTMLElement) {
        ...
        const issueText = `${openIssues} open resident issue${openIssues === 1 ? "" : "s"}`;
        const messageText = `${messages.length} recent SMS message${messages.length === 1 ? "" : "s"}`;
        messageCenterSummaryEl.textContent = sms.enabled
            ? `${buildingLabel} has ${issueText} and ${messageText}. ${provider} is ready${sender}.`
            : `${buildingLabel} has ${issueText}. SMS is not configured on this server.`;
    }
    ...
    if (messages.length === 0) {
        messagesBodyEl.innerHTML = '<tr><td colspan="7">No SMS activity yet.</td></tr>';
        return;
    }
    messagesBodyEl.innerHTML = messages
        ...
        const building = message.buildingName ||
...
function applyRoleCapabilities() {
    ...
    document.body.classList.toggle("landlord-staff-role", staff);
    ...
    openCreateBuildingDrawerButtons.forEach((button) => {
        ...
        button.classList.toggle("hidden", !ownerAccess);
    });
    if (openRentSheetBtnEl instanceof HTMLButtonElement) {
        openRentSheetBtnEl.classList.toggle("hidden", caretaker);
    }
    if (openDirectTenantDrawerBtnEl instanceof HTMLButtonElement) {
        openDirectTenantDrawerBtnEl.classList.toggle("hidden", caretaker);
    }
    updateOwnerNotificationControls();
    renderMessageCenter();
    renderSettingsPanel();
...
function compareBuildingRecords(a, b) {
    const nameComparison = buildingLabelCollator.compare(String(a?.name ?? ""), String(b?.name ?? ""));
    if (nameComparison !== 0) {
...
function setActiveLandlordView(nextView) {
    ...
    const targetView = isLandlordViewAvailableForRole(requestedTargetView)
        ? requestedTargetView
        : getDefaultLandlordViewForRole();
    state.activeLandlordView = targetView;
    landlordNavButtons.forEach((button) => {
        if (!(button instanceof HTMLButtonElement)) {
            return;
        }
        button.classList.toggle("active", (button.dataset.landlordView || button.dataset.landlordFocusTargetView) === targetView);
    });
...
function setDirectTenantStatus(message) {
    if (directTenantStatusEl instanceof HTMLElement) {
        directTenantStatusEl.textContent = message;
    }
}
function getDefaultDirectTenantBuildingId(preferredBuildingId = "") {
    const preferred = String(preferredBuildingId || "").trim();
    if (preferred && state.buildings.some((item) => item.id === preferred)) {
        return preferred;
    }
    const residentsSelection = state.selectedResidentsBuildingId && state.selectedResidentsBuildingId !== "all"
        ? state.selectedResidentsBuildingId
        : "";
    if (residentsSelection &&
        state.buildings.some((item) => item.id === residentsSelection)) {
        return residentsSelection;
    }
    if (state.selectedRegistryBuildingId &&
        state.buildings.some((item) => item.id === state.selectedRegistryBuildingId)) {
        return state.selectedRegistryBuildingId;
    }
    return state.buildings[0]?.id ?? "";
}
function syncDirectTenantBuildingOptions(preferredBuildingId = "") {
    if (!(directTenantBuildingEl instanceof HTMLSelectElement)) {
        return;
    }
    directTenantBuildingEl.replaceChildren();
    if (!Array.isArray(state.buildings) || state.buildings.length === 0) {
        const option = document.createElement("option");
        option.value = "";
        option.textContent = "No buildings";
        directTenantBuildingEl.append(option);
        directTenantBuildingEl.disabled = true;
        return;
    }
    const selected = getDefaultDirectTenantBuildingId(preferredBuildingId);
    directTenantBuildingEl.disabled = false;
    state.buildings.forEach((building) => {
        const option = document.createElement("option");
        option.value = building.id;
...
function openDirectTenantDrawer(prefill = {}) {
    if (isCaretakerRole()) {
        showError("House manager accounts cannot add tenants directly.");
        return;
    }
    if (!(directTenantDrawerEl instanceof HTMLElement)) {
        return;
    }
    syncDirectTenantBuildingOptions(prefill.buildingId);
    if (directTenantBuildingEl instanceof HTMLSelectElement) {
    ...
    setDirectTenantStatus("Temporary password: ID number. Password change required at first sign-in.");
    clearError();
    directTenantDrawerEl.classList.remove("hidden");
    if (directTenantDrawerBackdropEl instanceof HTMLElement) {
        directTenantDrawerBackdropEl.classList.remove("hidden");
    }
    if (directTenantHouseEl instanceof HTMLInputElement && !directTenantHouseEl.value) {
        directTenantHouseEl.focus();
    }
    else if (directTenantNameEl instanceof HTMLInputElement) {
        directTenantNameEl.focus();
    }
}
function closeDirectTenantDrawer() {
    if (!(directTenantDrawerEl instanceof HTMLElement)) {
        return;
    }
    directTenantDrawerEl.classList.add("hidden");
    if (directTenantDrawerBackdropEl instanceof HTMLElement) {
        directTenantDrawerBackdropEl.classList.add("hidden");
    }
}
...
function formatUtilityPaymentCoverage(data, fallbackBillingMonth) {
    const coveredMonths = Array.isArray(data?.allocations)
        ? [...new Set(data.allocations
                .map((item) => String(item?.bill?.billingMonth ?? item?.event?.billingMonth ?? "").trim())
                .filter(Boolean))]
        : [];
...
function isUtilityPlaceholderBill(item) {
    return (utilityAmount(item?.amountKsh) <= 0 &&
        utilityAmount(item?.balanceKsh) <= 0 &&
        !hasUtilityPayments(item));
}
function getUtilityPaidAmount(item) {
    if (hasUtilityPayments(item)) {
        return item.payments.reduce((sum, payment) => sum + utilityAmount(payment?.amountKsh), 0);
    }
    return Math.max(0, utilityAmount(item?.amountKsh) - utilityAmount(item?.balanceKsh));
}
function findRegistryRoom(buildingId, houseNumber) {
    return (getIndexedRoom(state.registryRoomByKey, buildingId, houseNumber) ??
...
function getRoomMeterProfile(buildingId, houseNumber) {
    ...
    const configuredElectricity = findConfiguredMeter("electricity", buildingId, houseNumber)?.meterNumber;
    const waterMeterNumber = normalizeUtilityMeterNumber(registryRoom?.waterMeterNumber ?? configuredWater ?? "");
    const electricityMeterNumber = normalizeUtilityMeterNumber(registryRoom?.electricityMeterNumber ?? configuredElectricity ?? "");
    return {
        waterMeterNumber,
...
function shouldAwaitMeterReadings(item) {
    ...
    return (getRoomMeterProfile(item.buildingId, item.houseNumber).hasBothMeters &&
...
function isUtilityBillBalanceVisible(item) {
    ...
    const visibleAt = subtractUtcDays(dueDate, UTILITY_BALANCE_VISIBILITY_WINDOW_DAYS);
    if (!visibleAt) {
        return true;
    }
...
function getVisibleUtilityBills(rows) {
    return (Array.isArray(rows) ? rows : []).filter((item) => !isUtilityPlaceholderBill(item) && isUtilityBillBalanceVisible(item));
}
...
function utilityStatusToneClass(status) {
    const normalized = String(status ?? "").trim().toLowerCase();
    if (normalized === "overdue" || normalized === "overdue_payable") {
        return "is-danger";
    }
    if (normalized === "payable" || normalized === "due_soon") {
        return "is-warning";
    }
    if (normalized === "clear") {
        return "is-success";
...
function tableRowToneClass(toneClass) {
    switch (String(toneClass ?? "").trim()) {
        case "is-danger":
            return "table-row-tone-danger";
        case "is-warning":
            return "table-row-tone-warning";
        case "is-success":
            return "table-row-tone-success";
        default:
            return "";
    }
}
function renderTableStatusPill(label, toneClass) {
    const normalizedLabel = String(label ?? "").trim() || "-";
    const normalizedTone = String(toneClass ?? "is-muted").trim() || "is-muted";
    return `<span class="table-status-pill ${normalizedTone}">${escapeHtml(normalizedLabel)}</span>`;
}
...
function compareStableRoomOrder(a, b) {
    const buildingDelta = compareBuildingRecords({
        id: String(a?.buildingId ?? ""),
        name: String(a?.buildingName ?? getBuildingNameById(a?.buildingId, "") ?? "")
    }, {
        id: String(b?.buildingId ?? ""),
        name: String(b?.buildingName ?? getBuildingNameById(b?.buildingId, "") ?? "")
    });
    if (buildingDelta !== 0) {
...
function updateResidentsSearchSummary(totalRows, visibleRows) {
    ...
    if (!query) {
        residentsSearchSummaryEl.textContent = `Showing ${visibleRows} room${visibleRows === 1 ? "" : "s"} for the selected building filter.`;
        return;
    }
    residentsSearchSummaryEl.textContent = `Found ${visibleRows} of ${totalRows} room${totalRows === 1 ? "" : "s"} for "${query}".`;
}
function getResidentOccupancyLabel(resident) {
    const hasResident = resident?.hasActiveResident || resident?.residentUserId || resident?.residentName;
    if (!hasResident) {
        return "vacant";
    }
...
function getUtilityRoomSummaryIndex() {
    if (!(state.utilityRoomSummaryByKey instanceof Map) ||
...
function getUtilitySummaryResident(item) {
    ...
    return (findResidentDirectoryEntry(item.buildingId, item.houseNumber) ??
...
function matchesUtilitySummaryTenantFilters(item) {
    ...
    if (query) {
        const fallbackSearch = [
            ...
            .join(" ")
            .toLowerCase();
        if (!(resident && matchesResidentSearch(resident, query)) &&
...
function getResidentOperationalCurrentDueKsh(resident, utilitySummary = null) {
    const utilityCurrentDueKsh = Math.max(getResidentCurrentUtilityDueKsh(resident), utilityAmount(utilitySummary?.payableBalanceKsh));
...
function getResidentOperationalArrearsKsh(resident, utilitySummary = null) {
    const utilityArrearsKsh = Math.max(getResidentUtilityArrearsKsh(resident), utilityAmount(utilitySummary?.overdueBalanceKsh));
    return (getResidentRentArrearsKsh(resident) +
        utilityArrearsKsh +
        getResidentExpenseBalanceKsh(resident));
}
function getResidentOperationalOutstandingKsh(resident, utilitySummary = null) {
    const calculatedBalanceKsh = getResidentOperationalCurrentDueKsh(resident, utilitySummary) +
...
function getResidentUtilityBillingStatusLabel(resident, utilitySummary = null) {
    const utilityArrearsKsh = Math.max(getResidentUtilityArrearsKsh(resident), utilityAmount(utilitySummary?.overdueBalanceKsh));
    const currentUtilityDueKsh = Math.max(getResidentCurrentUtilityDueKsh(resident), utilityAmount(utilitySummary?.payableBalanceKsh));
...
function getResidentExpenseBalanceKsh(resident) {
    ...
    const explicitExpense = Number(resident?.expenseBalanceKsh ?? resident?.expenseArrearsKsh ?? 0);
...
function getResidentUtilityBills(resident) {
    return (Array.isArray(state.bills) ? state.bills : [])
        ...
        .sort((left, right) => compareIsoDateDesc(left?.dueDate || left?.updatedAt || left?.createdAt, right?.dueDate || right?.updatedAt || right?.createdAt) || String(right?.billingMonth || "").localeCompare(String(left?.billingMonth || "")));
}
function getResidentUtilityPayments(resident) {
    return (Array.isArray(state.payments) ? state.payments : [])
        .filter((item) => matchesResidentRoomScope(item, resident))
        .sort((left, right) => compareIsoDateDesc(left?.paidAt || left?.createdAt, right?.paidAt || right?.createdAt));
}
...
function getResidentCurrentChargeDueKsh(resident, agreement) {
    return (getResidentCurrentRentDueKsh(resident, agreement) +
        getResidentCurrentUtilityDueKsh(resident));
}
function getResidentArrearsBalanceKsh(resident, agreement) {
    return (getResidentRentArrearsKsh(resident, agreement) +
...
function getResidentOverdueStartDate(resident) {
    ...
    return (String(resident.rentOverdueStartsAt ?? "").trim() ||
...
function getResidentBillingStatusLabel(resident) {
    ...
    if (rentStatus === "overdue") {
        ...
        const hasRentProfile = Boolean(resident.rentPaymentStatus ||
            resident.rentStatus ||
...
function getResidentBillingToneClass(resident, utilitySummary = null) {
    const hasResident = resident?.hasActiveResident || resident?.residentUserId || resident?.residentName;
    if (!hasResident) {
        return "is-muted";
    }
    ...
    const resolvedUtilitySummary = utilitySummary ?? getResidentUtilityRoomSummary(resident);
    const arrearsKsh = getResidentOperationalArrearsKsh(resident, resolvedUtilitySummary);
    const currentDueKsh = getResidentOperationalCurrentDueKsh(resident, resolvedUtilitySummary);
    if (arrearsKsh > 0) {
        return "is-danger";
    }
    if (currentDueKsh > 0) {
...
function getRentCollectionVisualState(item) {
    const arrearsKsh = Math.max(0, Number(item?.arrearsKsh ?? 0));
    const currentOutstandingKsh = Math.max(0, Number(item?.currentMonthOutstandingKsh ?? item?.balanceKsh ?? 0));
    const currentPaidKsh = Math.max(0, Number(item?.currentMonthPaidKsh ?? item?.paidAmountKsh ?? 0));
    if (arrearsKsh > 0) {
        return {
            label: "Overdue",
            toneClass: "is-danger"
    ...
    if (currentOutstandingKsh > 0) {
        return {
            label: currentPaidKsh > 0 ? "Partial" : "Unpaid",
            toneClass: "is-warning"
        };
    }
    const rawLabel = String(item?.paymentStatus ?? "").trim().toUpperCase();
    return {
        label: rawLabel === "PAID" || !rawLabel ? "Paid" : rawLabel.replaceAll("_", " "),
...
function getResidentTotalRentPaidKsh(resident) {
    ...
    const totalPaid = Number(resident?.totalRentPaidKsh ?? resident?.paidAmountKsh ?? resident?.rentPaidKsh);
...
function getVisibleResidentDirectoryRows(rows) {
    ...
    return sortResidentsForDirectory(dedupeResidentDirectoryRows(allRows).filter((resident) => matchesResidentStatusFilter(resident, state.residentStatusFilter) &&
...
function residentDirectoryPreference(resident) {
    ...
    return {
        ...
        occupancyPriority: resident?.hasActiveResident || resident?.residentUserId || resident?.residentName ? 0 : 1,
...
function dedupeResidentDirectoryRows(rows) {
    ...
    (Array.isArray(rows) ? rows : []).forEach((resident) => {
        const key = `${String(resident?.buildingId ?? "").trim()}::${normalizeHouse(resident?.houseNumber)}`;
        if (!key.endsWith("::")) {
...
function getResidentLookupExactMatches(rows, query) {
    ...
    return rows.filter((resident) => {
        ...
        return (houseNumber === normalizedQuery ||
...
function getRoomsDeepLinkBuildingId() {
    return roomsWorkspaceBuildingId;
}
function isRoomsWorkspaceRoute() {
    return Boolean(roomsWorkspaceBuildingId);
}
function applyRoomsWorkspaceLayout() {
    document.body.classList.toggle("landlord-rooms-route", isRoomsWorkspaceRoute());
...
function buildRoomAccountPath(buildingId, houseNumber) {
    return `/landlord/rooms/${encodeURIComponent(String(buildingId ?? "").trim())}/${encodeURIComponent(normalizeHouse(houseNumber))}`;
}
...
function openResidentSearchMatch() {
    ...
    if (exactMatches.length === 1) {
        openRoomAccountPage(exactMatches[0].buildingId, exactMatches[0].houseNumber);
        return;
    }
    ...
    showError(`Found ${visibleRows.length} rooms for "${query}". Refine the search or choose a building first.`);
}
...
function sortResidentsForDirectory(rows) {
    return [...rows].sort(compareStableRoomOrder);
}
...
function renderResidentsOverview(rows) {
    ...
    const vacantCount = items.filter((resident) => getResidentOccupancyLabel(resident) === "vacant").length;
    const pendingCount = items.filter((resident) => getResidentOccupancyLabel(resident) === "pending_review").length;
    const withCurrentDue = items.filter((resident) => matchesResidentStatusFilter(resident, "current_due"));
    const withArrears = items.filter((resident) => matchesResidentStatusFilter(resident, "overdue"));
    const awaitingReadings = items.filter((resident) => matchesResidentStatusFilter(resident, "awaiting_readings"));
    const clearRooms = items.filter((resident) => matchesResidentStatusFilter(resident, "clear"));
    const totalCurrentDue = withCurrentDue.reduce((sum, resident) => sum +
        getResidentOperationalCurrentDueKsh(resident, getResidentUtilityRoomSummary(resident)), 0);
    const totalArrears = withArrears.reduce((sum, resident) => sum +
        getResidentOperationalArrearsKsh(resident, getResidentUtilityRoomSummary(resident)), 0);
...
function summarizeUtilityRooms(rows) {
    ...
    return [...grouped.values()]
        ...
        .sort((a, b) => {
        const buildingDelta = String(a.buildingId ?? "").localeCompare(String(b.buildingId ?? ""));
        if (buildingDelta !== 0) {
            return buildingDelta;
        }
        return compareHouseNumber(a.houseNumber, b.houseNumber);
...
function getResidentDisplayName(resident, fallback = "Details pending") {
    ...
    const displayName = rawName && !isRoomDerivedResidentName(rawName, resident?.houseNumber)
        ? rawName
        : fallback;
...
function getResidentOutstandingBalanceKsh(resident) {
    ...
    if (Number.isFinite(rentBalance) ||
        Number.isFinite(utilityBalance) ||
        Number.isFinite(expenseBalance)) {
        return Math.max(0, (Number.isFinite(rentBalance) ? rentBalance : 0) +
            (Number.isFinite(utilityBalance) ? utilityBalance : 0) +
            (Number.isFinite(expenseBalance) ? expenseBalance : 0));
    }
    const legacyBalance = Number(resident?.balanceKsh ?? resident?.outstandingBalanceKsh ?? 0);
...
function buildResidentAgreementUrl(resident) {
    return `/api/landlord/buildings/${encodeURIComponent(resident.buildingId)}/houses/${encodeURIComponent(resident.houseNumber)}/agreement`;
}
...
function summarizeResidentOccupation(resident) {
    ...
    const organizationSummary = resident?.organizationName?.trim()
        ? `${resident.organizationName.trim()}${resident?.organizationLocation?.trim() ? ` • ${resident.organizationLocation.trim()}` : ""}`
        : "";
...
function summarizeEmergencyContact(resident) {
    ...
    return `${resident.emergencyContactName.trim()}${resident?.emergencyContactPhone?.trim() ? ` • ${resident.emergencyContactPhone.trim()}` : ""}`;
}
...
function syncSelectedResidentAfterRefresh(buildingId, houseNumber) {
    ...
    if (String(state.selectedResident.buildingId ?? "") !== String(buildingId ?? "") ||
...
function expandHouseRangeExpression(value) {
    ...
    const prefix = start.prefix && end.prefix
    ...
    return expandRoomRange(prefix, start.number, end.number, Math.max(start.width, end.width));
}
...
function buildHouseNumberBlocks({ format, prefixRaw, separator, start, count, step }) {
    ...
    return blocks.flatMap((entry) => {
        ...
        if (blockMatch) {
            ...
            const width = getRangePadWidth(blockMatch[2], blockMatch[3]);
            return expandRoomRange(`${prefix}${separator}`, blockStart, blockEnd, width);
        }
        return Array.from({ length: count }, (_item, index) => {
...
function renderGeneratedHousePreview(houses) {
    ...
    const existingSet = new Set(getSelectedBuildingHouseNumbers().map((item) => normalizeHouse(item)));
...
function getSelectedBuildingHouseNumbers() {
    ...
    const fromDirectory = state.residentDirectory
        .filter((item) => normalizeLookupBuildingId(item.buildingId) ===
...
function renderRoomChipList(container, rooms, options = {}) {
    ...
    const existingSet = new Set((options.existingRooms ?? []).map((item) => normalizeHouse(item)));
    const removableBuildingId = String(options.removableBuildingId ?? "").trim();
    const canRemoveRooms = Boolean(options.removable && removableBuildingId && !isCaretakerRole());
    visibleRooms.forEach((room) => {
        ...
        if (canRemoveRooms) {
            ...
            button.type = "button";
            button.className = "building-room-remove-btn";
            button.dataset.action = "remove-room";
            button.dataset.buildingId = removableBuildingId;
            button.dataset.houseNumber = normalized;
            button.setAttribute("aria-label", `Remove room ${normalized}`);
...
function renderBuildingRoomDrawerState() {
    ...
    const newCandidateRooms = candidateRooms.filter((item) => !existingSet.has(normalizeHouse(item)));
    ...
    if (buildingExistingRoomsSummaryEl instanceof HTMLElement) {
        buildingExistingRoomsSummaryEl.textContent = `${existingRooms.length} room${existingRooms.length === 1 ? "" : "s"}`;
    }
    renderRoomChipList(buildingExistingRoomsEl, existingRooms, {
        emptyText: "No rooms in this building yet.",
        removable: true,
        removableBuildingId: state.selectedRoomBuildingId
    });
}
...
function normalizeUtilityRateDefaults(rateDefaults, fallbackBuildingId = "") {
    ...
    return {
        buildingId,
        waterRatePerUnitKsh: rateDefaults?.waterRatePerUnitKsh == null
            ? undefined
            : Number(rateDefaults.waterRatePerUnitKsh),
        electricityRatePerUnitKsh: rateDefaults?.electricityRatePerUnitKsh == null
            ? undefined
...
function setUtilityPricingState(buildingConfiguration, rateDefaults, fallbackBuildingId = "") {
    const normalizedBuildingId = String(buildingConfiguration?.buildingId || fallbackBuildingId || "").trim();
...
function getUtilityRateDefault(utilityType, buildingId) {
    ...
    const candidate = utilityType === "water"
...
function syncUtilitySheetRateDefaults() {
    ...
    const defaults = state.utilityRateDefaults;
    const waterValue = numberToInputString(defaults?.waterRatePerUnitKsh ?? DEFAULT_WATER_RATE_PER_UNIT_KSH);
...
function syncUtilitySheetBuildingFixedDefaults() {
    ...
    utilitySheetWaterFixedDefaultEl.value = numberToInputString(state.utilitySheetBuildingConfiguration?.defaultWaterFixedChargeKsh);
    utilitySheetElectricFixedDefaultEl.value = numberToInputString(state.utilitySheetBuildingConfiguration?.defaultElectricityFixedChargeKsh);
}
function syncUtilitySheetCombinedCharge() {
    if (!(utilitySheetCombinedChargeEl instanceof HTMLInputElement)) {
        return;
    }
    utilitySheetCombinedChargeEl.value = numberToInputString(state.utilitySheetMonthlyCombinedCharge?.amountKsh);
}
function syncUtilitySheetBuildingCombinedCharge() {
    if (!(utilitySheetBuildingCombinedChargeEl instanceof HTMLInputElement)) {
        return;
    }
    utilitySheetBuildingCombinedChargeEl.value = numberToInputString(state.utilitySheetBuildingConfiguration?.defaultCombinedUtilityChargeKsh);
}
async function loadUtilitySheetBuildingConfiguration() {
    const buildingId = String(utilitySheetBuildingSelectEl?.value || state.selectedRegistryBuildingId || "").trim();
...
function getRoomUtilityFixedChargeDefault(utilityType, buildingId, houseNumber) {
    ...
    const roomValue = utilityType === "water"
...
function getSelectedRegistryReadingMonth() {
    ...
    return (getLatestAvailableUtilityBillingMonth(getSelectedUtilityBuildingId()) ||
...
function describeRegistryChargeSetup(item, buildingId, billingMonth) {
    ...
    const buildingDefaultCombinedKsh = Math.max(0, Number(configuration?.defaultCombinedUtilityChargeKsh ?? 0));
    const resolvedWaterFixedKsh = getRoomUtilityFixedChargeDefault("water", buildingId, item?.houseNumber);
    const resolvedElectricityFixedKsh = getRoomUtilityFixedChargeDefault("electricity", buildingId, item?.houseNumber);
    const fixedParts = [];
    ...
    if (Number.isFinite(Number(resolvedElectricityFixedKsh)) &&
    ...
    if (billingMode === "combined_charge") {
        ...
        if (monthlyOverrideKsh > 0) {
            return {
                ...
                detail: `Applied for ${formatBillingMonth(billingMonth)} when the room has no room default charge.`
            };
        }
...
function formatRegistryChargeSetupMarkup(item, buildingId, billingMonth) {
    ...
    <div class="charge-setup-copy">
      <span class="charge-mode-badge is-${escapeHtml(setup.tone)}">${escapeHtml(setup.label)}</span>
...
function renderRegistryChargeSummary(rows) {
    ...
    const setupSummary = setupRows.map((item) => describeRegistryChargeSetup(item, buildingId, billingMonth));
    ...
    if (buildingMode === "combined_charge") {
        summaryLines.push(`Charge order: room default -> ${formatBillingMonth(billingMonth)} monthly adjustment -> building default.`);
        summaryLines.push(monthlyCombinedCharge && Number(monthlyCombinedCharge.amountKsh) > 0
            ? `${formatBillingMonth(billingMonth)} monthly adjustment is ${formatCurrency(Number(monthlyCombinedCharge.amountKsh))}.`
            : `${formatBillingMonth(billingMonth)} monthly adjustment is not set.`);
        summaryLines.push(buildingDefaultCombinedKsh > 0
            ? `Building default combined charge is ${formatCurrency(buildingDefaultCombinedKsh)}.`
            : "Building default combined charge is not set.");
    }
    else if (buildingMode === "fixed_charge") {
        summaryLines.push("This building posts fixed-charge utility bills unless a room is fully metered.");
        summaryLines.push(`Building defaults: water ${buildingWaterFixedKsh > 0 ? formatCurrency(buildingWaterFixedKsh) : "not set"} • electricity ${buildingElectricFixedKsh > 0 ? formatCurrency(buildingElectricFixedKsh) : "not set"}.`);
    }
    else if (buildingMode === "disabled") {
        summaryLines.push("Utility billing is disabled for this building.");
    }
    else {
        summaryLines.push("This building relies on meter readings by default, with fixed-charge fallback where needed.");
        summaryLines.push(`Building defaults: water ${buildingWaterFixedKsh > 0 ? formatCurrency(buildingWaterFixedKsh) : "not set"} • electricity ${buildingElectricFixedKsh > 0 ? formatCurrency(buildingElectricFixedKsh) : "not set"}.`);
    }
...
function formatRegistryReadingMarkup(item, billingMonth) {
    ...
    if (!hasPreviousReading && !hasCurrentReading) {
      ...
      <div class="registry-reading-cell is-empty">
        <strong>-</strong>
        <small>${escapeHtml(utilityAmount(item.amountKsh) > 0 ? "Combined charge" : "No reading")}</small>
      </div>
    `;
    }
    ...
    if (hasPreviousReading &&
        ...
        detail = `${numberToInputString(previousReading)} -> ${numberToInputString(currentReading)}`;
    }
    ...
    return `
    <div class="registry-reading-cell${isRestoredBaseline ? " is-restored" : ""}" title="${escapeHtml(note || `${billingMonth || "Selected month"} reading`)}">
...
async function loadUtilitySheetMonthlyCombinedCharge() {
    ...
    const payload = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/monthly-combined-utility-charge?billingMonth=${encodeURIComponent(billingMonth)}`);
...
function renderUtilitySheetRows(rows) {
    ...
    const buildingId = getSelectedUtilityBuildingId();
    const isCombinedChargeBuilding = String(state.utilitySheetBuildingConfiguration?.buildingId ?? "").trim() === buildingId &&
    ...
    [...rows].sort((a, b) => compareHouseNumber(a.houseNumber, b.houseNumber)).forEach((item) => {
        ...
        const waterPrev = waterBill && Number.isFinite(Number(waterBill.currentReading))
            ? Number(waterBill.currentReading)
            : undefined;
        const electricityPrev = electricityBill && Number.isFinite(Number(electricityBill.currentReading))
            ? Number(electricityBill.currentReading)
            : undefined;
        const waterMeterValue = meterNumberForHouse("water", buildingId, houseNumber, item.waterMeterNumber);
        const electricityMeterValue = meterNumberForHouse("electricity", buildingId, houseNumber, item.electricityMeterNumber);
        const configuredWaterMeter = findConfiguredMeter("water", buildingId, houseNumber);
        const configuredElectricityMeter = findConfiguredMeter("electricity", buildingId, houseNumber);
        const transferredWaterReading = isCombinedChargeBuilding && !hasUsableMeterNumber(configuredWaterMeter?.meterNumber)
            ? numericValueFromString(item.waterMeterNumber)
            : undefined;
        const transferredElectricityReading = isCombinedChargeBuilding &&
            !hasUsableMeterNumber(configuredElectricityMeter?.meterNumber)
            ? numericValueFromString(item.electricityMeterNumber)
            : undefined;
        const waterMeterNumber = transferredWaterReading != null ? "" : normalizeUtilityMeterNumber(waterMeterValue);
        const electricityMeterNumber = transferredElectricityReading != null
            ? ""
            : normalizeUtilityMeterNumber(electricityMeterValue);
        const hasBothMeters = hasUsableMeterNumber(waterMeterNumber) && hasUsableMeterNumber(electricityMeterNumber);
        const roomWaterFixedCharge = Number.isFinite(Number(item.waterFixedChargeKsh)) && Number(item.waterFixedChargeKsh) > 0
            ? Number(item.waterFixedChargeKsh)
            : undefined;
        const roomElectricityFixedCharge = Number.isFinite(Number(item.electricityFixedChargeKsh)) &&
            Number(item.electricityFixedChargeKsh) > 0
            ? Number(item.electricityFixedChargeKsh)
            : undefined;
        const buildingWaterFixedCharge = getBuildingUtilityFixedChargeDefault("water", buildingId);
        const buildingElectricityFixedCharge = getBuildingUtilityFixedChargeDefault("electricity", buildingId);
        const latestWaterFixedCharge = Number.isFinite(Number(waterBill?.fixedChargeKsh)) && Number(waterBill?.fixedChargeKsh) > 0
            ? Number(waterBill?.fixedChargeKsh)
            : undefined;
        const latestElectricityFixedCharge = Number.isFinite(Number(electricityBill?.fixedChargeKsh)) &&
        ...
        const autoWaterFixedCharge = roomWaterFixedCharge != null ? undefined : resolvedWaterFixedDefault;
        const autoElectricityFixedCharge = roomElectricityFixedCharge != null ? undefined : resolvedElectricityFixedDefault;
        const row = document.createElement("tr");
...
function buildUtilitySheetRegistryPayload() {
    ...
    trList.forEach((tr) => {
        ...
        const electricityInput = tr.querySelector('input[data-field="electricityMeterNumber"]');
        const waterFixedInput = tr.querySelector('input[data-field="waterFixedChargeKsh"]');
        const electricityFixedInput = tr.querySelector('input[data-field="electricityFixedChargeKsh"]');
        if (!(waterInput instanceof HTMLInputElement) ||
        ...
        const roomElectricityFixedCharge = numericValueFromString(tr.dataset.roomElectricityFixedCharge);
        const autoWaterFixedCharge = numericValueFromString(tr.dataset.autoWaterFixedCharge);
        const autoElectricityFixedCharge = numericValueFromString(tr.dataset.autoElectricityFixedCharge);
        const waterFixedChargeKsh = waterFixedChargeInput == null
            ? 0
            : roomWaterFixedCharge != null && roomWaterFixedCharge > 0
                ...
                : autoWaterFixedCharge != null &&
                    ...
                    ? 0
                    : waterFixedChargeInput;
        const electricityFixedChargeKsh = electricityFixedChargeInput == null
            ? 0
            : roomElectricityFixedCharge != null && roomElectricityFixedCharge > 0
                ? electricityFixedChargeInput
                : autoElectricityFixedCharge != null &&
                    utilityPricingNumbersEqual(electricityFixedChargeInput, autoElectricityFixedCharge)
                    ? 0
                    : electricityFixedChargeInput;
        rows.push({
            houseNumber,
            ...
            electricityMeterNumber: normalizeUtilityMeterNumber(electricityInput.value) || undefined,
            waterFixedChargeKsh,
...
function buildUtilitySheetAuditRows() {
    ...
    trList.forEach((tr) => {
        ...
        const waterPreviousInput = tr.querySelector('input[data-field="waterPreviousReading"]');
        const waterCurrentInput = tr.querySelector('input[data-field="waterCurrentReading"]');
        const waterFixedInput = tr.querySelector('input[data-field="waterFixedChargeKsh"]');
        const electricityMeterInput = tr.querySelector('input[data-field="electricityMeterNumber"]');
        const electricityPreviousInput = tr.querySelector('input[data-field="electricityPreviousReading"]');
        const electricityCurrentInput = tr.querySelector('input[data-field="electricityCurrentReading"]');
        const electricityFixedInput = tr.querySelector('input[data-field="electricityFixedChargeKsh"]');
        rows.push({
            houseNumber,
            householdMembers: Number.isInteger(householdMembers) ? householdMembers : 0,
            hasActiveResident: tr.dataset.hasActiveResident === "true",
            waterMeterNumber: waterMeterInput instanceof HTMLInputElement
                ? waterMeterInput.value.trim() || undefined
                : undefined,
            waterPreviousReading: waterPreviousInput instanceof HTMLInputElement
                ? toOptionalNumber(waterPreviousInput.value)
                : undefined,
            waterCurrentReading: waterCurrentInput instanceof HTMLInputElement
                ? toOptionalNumber(waterCurrentInput.value)
                : undefined,
            waterFixedChargeKsh: waterFixedInput instanceof HTMLInputElement
                ? toOptionalNumber(waterFixedInput.value)
                : undefined,
            electricityMeterNumber: electricityMeterInput instanceof HTMLInputElement
                ? electricityMeterInput.value.trim() || undefined
                : undefined,
            electricityPreviousReading: electricityPreviousInput instanceof HTMLInputElement
                ? toOptionalNumber(electricityPreviousInput.value)
                : undefined,
            electricityCurrentReading: electricityCurrentInput instanceof HTMLInputElement
                ? toOptionalNumber(electricityCurrentInput.value)
                : undefined,
            electricityFixedChargeKsh: electricityFixedInput instanceof HTMLInputElement
...
async function finalizeUtilityBulkAudit(buildingId, auditId, payload) {
    ...
    await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/utility-bulk-audits/${encodeURIComponent(auditId)}`, {
        method: "PATCH",
        headers: {
...
function buildUtilitySheetBillRequests(buildingId, billingMonth, dueDateIso, note, combinedUtilityChargeKsh) {
    if (!(utilitySheetBodyEl instanceof HTMLElement)) {
        return [];
    }
    const waterRatePerUnitKsh = toOptionalNumber(utilitySheetWaterRateEl?.value) ??
        getUtilityRateDefault("water", buildingId);
    const electricityRatePerUnitKsh = toOptionalNumber(utilitySheetElectricRateEl?.value) ??
    ...
    trList.forEach((tr) => {
        ...
        const electricityCurrentInput = tr.querySelector('input[data-field="electricityCurrentReading"]');
        const electricityFixedInput = tr.querySelector('input[data-field="electricityFixedChargeKsh"]');
        const waterPreviousReading = waterPreviousInput instanceof HTMLInputElement
            ? toOptionalNumber(waterPreviousInput.value)
            : undefined;
        const waterCurrentReading = waterCurrentInput instanceof HTMLInputElement
            ? toOptionalNumber(waterCurrentInput.value)
            : undefined;
        const waterFixedChargeKsh = waterFixedInput instanceof HTMLInputElement
            ? toOptionalNumber(waterFixedInput.value) ?? 0
            : 0;
        const electricityPreviousReading = electricityPreviousInput instanceof HTMLInputElement
            ? toOptionalNumber(electricityPreviousInput.value)
            : undefined;
        const electricityCurrentReading = electricityCurrentInput instanceof HTMLInputElement
            ? toOptionalNumber(electricityCurrentInput.value)
            : undefined;
        const electricityFixedChargeKsh = electricityFixedInput instanceof HTMLInputElement
            ? toOptionalNumber(electricityFixedInput.value) ?? 0
            : 0;
        const hasRoomSpecificUtilityEntry = waterPreviousReading != null ||
        ...
        ["water", "electricity"].forEach((utilityType) => {
            const meterInput = tr.querySelector(`input[data-field="${utilityType}MeterNumber"]`);
            const previousInput = tr.querySelector(`input[data-field="${utilityType}PreviousReading"]`);
            const currentInput = tr.querySelector(`input[data-field="${utilityType}CurrentReading"]`);
            const fixedInput = tr.querySelector(`input[data-field="${utilityType}FixedChargeKsh"]`);
            if (!(meterInput instanceof HTMLInputElement) ||
            ...
            const ratePerUnitKsh = utilityType === "water" ? waterRatePerUnitKsh : electricityRatePerUnitKsh;
            ...
            if (currentReading != null && ratePerUnitKsh == null) {
                throw new Error(`${utilityType} for ${houseNumber} requires a building rate per unit.`);
            }
            if (previousReading != null && currentReading == null) {
                throw new Error(`${utilityType} for ${houseNumber} requires current reading when previous reading is provided.`);
            }
            if (previousReading != null &&
                currentReading != null &&
                currentReading < previousReading) {
                throw new Error(`${utilityType} for ${houseNumber} has current reading lower than previous reading.`);
            }
            const resolvedRatePerUnitKsh = currentReading != null ? ratePerUnitKsh : undefined;
            const payload = {
                buildingId,
                billingMonth,
...
function getSelectedUtilityBuildingId() {
    return String(registryBuildingSelectEl?.value || state.selectedRegistryBuildingId || "").trim();
}
...
function findConfiguredMeter(utilityType, buildingId, houseNumber) {
    return (state.meterByKey.get(utilityBuildingHouseLookupKey(utilityType, buildingId, houseNumber)) ??
...
function syncUtilityBillInputMode() {
    ...
    if (!hasMeter) {
        ...
        const defaultFixedCharge = getRoomUtilityFixedChargeDefault(utilityType, buildingId, houseNumber);
...
async function requestJson(url, options = {}) {
    ...
    if (!response.ok) {
        ...
        const err = new Error(issueMessage ?? payload.error ?? `Request failed (${response.status})`);
...
function renderLandlordPushControls() {
    ...
    landlordPushAlertsBtnEl.classList.toggle("hidden", !owner || !supported || !serverEnabled || hasSubscription);
...
async function getLandlordPushSubscription() {
    ...
    return registration.pushManagement.getSubscription();
}
...
async function syncLandlordPushState({ subscribeIfAllowed = false } = {}) {
    ...
    if (!subscription && subscribeIfAllowed && Notification.permission === "granted") {
        ...
        try {
            const created = await registration.pushManagement.subscribe({
...
async function enableLandlordPushAlerts() {
    clearError();
    if (!isOwnerAlertRole()) {
        showError("Owner alerts are only available for landlord or staff accounts.");
        return;
    }
    if (!supportsLandlordPush()) {
    ...
    try {
        const existing = await registration.pushManagement.getSubscription();
        const subscription = existing ??
            (await registration.pushManagement.subscribe({
                userVisibleOnly: true,
                applicationServerKey: urlBase64ToUint8Array(config.publicKey)
            }));
        await registerLandlordPushSubscription(subscription);
        ...
        const message = error instanceof Error ? error.message : "Unable to enable owner browser alerts.";
        showError(message);
    }
    finally {
        renderLandlordPushControls();
...
async function loadOwnerNotifications() {
    ...
    state.ownerNotifications = Array.isArray(payload.data?.notifications)
        ...
        : [];
    state.ownerNotificationsUnreadCount = Number.isFinite(Number(payload.data?.unreadCount))
...
async function loadMessageCenter() {
    ...
    if (state.selectedMessageBuildingId) {
        ...
        state.selectedTicketBuildingId = state.selectedMessageBuildingId;
        if (landlordTicketBuildingSelectEl instanceof HTMLSelectElement) {
            landlordTicketBuildingSelectEl.value = state.selectedMessageBuildingId;
        }
    }
...
async function loadAutomaticMessageRules(buildingId = state.selectedMessageBuildingId) {
    ...
    const payload = await requestJson(`/api/landlord/messages/rules?buildingId=${encodeURIComponent(normalizedBuildingId)}`, { cache: "no-store" });
...
function syncBuildingPhotoPreview() {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : "Unable to preview selected photo.";
        showError(message);
        buildingPhotoFileEl.value = "";
    }
    const gallery = createUploadedImageGallery(getBuildingPhotoUrls(String(buildingPhotoBuildingSelectEl?.value ?? "").trim()).slice(0, 1), { linkLabel: "Open building photo" });
    if (gallery) {
...
function renderBuildingPhotoOptions() {
    ...
    if (!Array.isArray(state.buildings) || state.buildings.length === 0) {
        ...
        replaceUploadPreview(buildingPhotoPreviewEl, null, "No building photo selected.");
        return;
    }
...
function renderGlobalSearchBuildingOptions() {
    ...
    const preferredBuilding = state.selectedResidentsBuildingId && state.selectedResidentsBuildingId !== "all"
        ? state.selectedResidentsBuildingId
...
async function ensureSession() {
    try {
        ...
        handleLandlordError(error, "Management session is not available.");
        return false;
    }
}
function getRecordBuildingId(item) {
    return normalizeLookupBuildingId(item?.buildingId || item?.building?.id || "");
}
function getRowsForFocusedBuilding(rows) {
    const allRows = Array.isArray(rows) ? rows : [];
    const focusedBuildingId = getFocusedBuildingId();
    if (!focusedBuildingId) {
        return allRows;
    }
    return allRows.filter((item) => getRecordBuildingId(item) === focusedBuildingId);
}
function getOpenIssueRowsForFocusedBuilding() {
    return getRowsForFocusedBuilding(state.tickets).filter((item) => String(item?.status || "").trim() !== "resolved");
}
function getPendingApplicationRowsForFocusedBuilding() {
    return getRowsForFocusedBuilding(state.applications).filter((item) => String(item?.status || "pending").trim() === "pending");
}
function renderDailyDashboard() {
    const requiredEls = [
        ...
        overviewDailyIssuesEl,
        overviewDailyRequestsEl,
        overviewDailyVacantEl
    ];
    if (requiredEls.some((item) => !(item instanceof HTMLElement))) {
        return;
    }
    const focusedBuildingId = getFocusedBuildingId();
    const buildingLabel = focusedBuildingId
        ? getBuildingDisplayNameById(focusedBuildingId, "Focused building")
        : "Portfolio";
    const residentRows = dedupeResidentDirectoryRows(getRowsForFocusedBuilding(state.residentDirectory));
    const rentRows = getRowsForFocusedBuilding(state.rentStatus);
    const utilityBills = getActionableUtilityBills(getRowsForFocusedBuilding(state.bills));
    const currentMonthKey = currentBillingMonth();
    const rentCollectedThisMonth = residentRows.reduce((sum, item) => sum + getResidentCurrentMonthRentPaidKsh(item), 0);
    const utilityCollectedThisMonth = getRowsForFocusedBuilding(state.payments)
    ...
    const outstanding = residentRows.length > 0
        ? residentRows.reduce((sum, item) => sum + getResidentOperationalOutstandingKsh(item, getResidentUtilityRoomSummary(item)), 0)
        : rentRows.reduce((sum, item) => sum + Math.max(0, Number(item.balanceKsh ?? 0)), 0) +
            utilityBills.reduce((sum, item) => sum + utilityAmount(item.balanceKsh), 0);
    const unpaidRoomKeys = new Set();
    rentRows.forEach((item) => {
        const balanceKsh = Math.max(0, Number(item.currentMonthOutstandingKsh ?? item.balanceKsh ?? 0));
        if (balanceKsh > 0) {
            unpaidRoomKeys.add(`${getRecordBuildingId(item)}::${normalizeHouse(item.houseNumber)}`);
        }
    });
    residentRows.forEach((item) => {
        if (getResidentOutstandingBalanceKsh(item) > 0) {
            unpaidRoomKeys.add(`${getRecordBuildingId(item)}::${normalizeHouse(item.houseNumber)}`);
        }
    });
    utilityBills.forEach((item) => {
        if (utilityAmount(item.balanceKsh) > 0) {
            unpaidRoomKeys.add(`${getRecordBuildingId(item)}::${normalizeHouse(item.houseNumber)}`);
        }
    });
    const vacantRooms = residentRows.filter((item) => !(item?.hasActiveResident || item?.residentUserId || item?.residentName)).length;
    ...
    overviewDailyIssuesEl.textContent = String(openIssues);
    overviewDailyRequestsEl.textContent = String(pendingRequests);
    overviewDailyVacantEl.textContent = String(vacantRooms);
}
function getFocusedBuildingSummary(buildingId) {
    ...
    const residentRows = dedupeResidentDirectoryRows(state.residentDirectory).filter((item) => normalizeLookupBuildingId(item.buildingId) === buildingId);
    const utilityBills = getActionableUtilityBills(state.bills).filter((item) => normalizeLookupBuildingId(item.buildingId) === buildingId);
    let openBills = 0;
    const outstanding = residentRows.length > 0
        ? residentRows.reduce((sum, item) => sum + getResidentOperationalOutstandingKsh(item, getResidentUtilityRoomSummary(item)), 0)
        : utilityBills.reduce((sum, item) => sum + utilityAmount(item.balanceKsh), 0);
...
function renderLandlordFocusPanel() {
    if (!(landlordFocusBuildingSelectEl instanceof HTMLSelectElement) ||
...
function renderBuildingManagementSummary(totalCount, visibleCount) {
    ...
    const focusCopy = focusedBuildingId && focusedBuildingName
...
function renderBuildings(rows) {
    ...
    if (filteredRows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = `<td colspan="8">No buildings match "${escapeHtml(state.buildingManagementQuery)}".</td>`;
        buildingsBodyEl.append(row);
        return;
    }
    ...
    `;
      ...
      <td>${primaryPhoto
            ...
            : '<span class="ticket-details muted">No photo</span>'}</td>
      <td>
...
function handleDeleteBuildingClick(target, buildingId, buildingName) {
    ...
    const shouldProceed = window.confirm(`Delete ${buildingName || getBuildingDisplayNameById(buildingId)}?\nThis permanently removes the building, rooms, active tenancy links, and linked unit records.`);
    if (!shouldProceed) {
        return;
    }
...
function handleSettingsClearCacheClick() {
    if (isCaretakerRole()) {
        showError("House manager accounts cannot clear building cache.");
        return;
    }
    ...
    const buildingName = getBuildingDisplayNameById(buildingId, buildingId);
    const confirmation = window.prompt(`Type CLEAR CACHE to refresh workspace cache for ${buildingName}. Live rent, utility, room, and resident data will stay intact.`, "");
    if (confirmation !== "CLEAR CACHE") {
        return;
    }
    if (settingsClearCacheBtnEl instanceof HTMLButtonElement) {
        settingsClearCacheBtnEl.disabled = true;
    }
    clearError();
    void (async () => {
        try {
            await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/clear-cache`, {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
            ...
            setStatus(`Cleared workspace cache for ${buildingName}.`);
            await loadData({ quick: false });
            setActiveLandlordView("settings");
            if (settingsBuildingSelectEl instanceof HTMLSelectElement) {
                settingsBuildingSelectEl.value = buildingId;
            }
            renderSettingsPanel();
        }
        catch (error) {
            handleLandlordError(error, "Failed to clear building cache.");
        }
        finally {
            renderSettingsPanel();
        }
...
function setPreferredBuildingSelection(buildingId, options = {}) {
    ...
    if (settingsBuildingSelectEl instanceof HTMLSelectElement) {
        settingsBuildingSelectEl.value = normalizedBuildingId;
    }
    if (options.includeResidents !== false &&
...
function getSelectedSettingsBuildingId() {
    return String(settingsBuildingSelectEl?.value || state.selectedRegistryBuildingId || "").trim();
}
function renderSettingsPanel() {
    if (!(settingsSummaryEl instanceof HTMLElement)) {
        return;
    }
    const buildingId = getSelectedSettingsBuildingId();
    const building = state.buildings.find((item) => item.id === buildingId) ?? null;
    const hasBuildings = Array.isArray(state.buildings) && state.buildings.length > 0;
    const actionsDisabled = isCaretakerRole() || !building;
    if (!hasBuildings) {
        settingsSummaryEl.textContent =
            "No buildings are available yet. Add a building before using workspace settings.";
    }
    else if (isCaretakerRole()) {
        settingsSummaryEl.textContent =
            "House manager accounts can view this page but cannot run cache-clearing or destructive building actions.";
    }
    else {
        settingsSummaryEl.textContent = `Settings are focused on ${building?.name || getBuildingDisplayNameById(buildingId, "the selected building")}. Clear cache refreshes workspace state only. Delete data permanently removes the whole building.`;
    }
    if (settingsClearCacheBtnEl instanceof HTMLButtonElement) {
        settingsClearCacheBtnEl.disabled = actionsDisabled;
    }
    if (settingsClearDataBtnEl instanceof HTMLButtonElement) {
        settingsClearDataBtnEl.disabled = actionsDisabled;
    }
}
function syncSettingsBuildingOptions() {
    if (!(settingsBuildingSelectEl instanceof HTMLSelectElement)) {
        return;
    }
    settingsBuildingSelectEl.replaceChildren();
    if (!Array.isArray(state.buildings) || state.buildings.length === 0) {
        const option = document.createElement("option");
        option.value = "";
        option.textContent = "No buildings available";
        settingsBuildingSelectEl.append(option);
        settingsBuildingSelectEl.disabled = true;
        renderSettingsPanel();
        return;
    }
    const selected = state.selectedRegistryBuildingId &&
        state.buildings.some((item) => item.id === state.selectedRegistryBuildingId)
        ? state.selectedRegistryBuildingId
        : state.buildings[0].id;
    settingsBuildingSelectEl.disabled = false;
    state.buildings.forEach((building) => {
        const option = document.createElement("option");
        option.value = building.id;
        option.textContent = getBuildingDisplayName(building);
...
function renderRoomBuildingOptions() {
    ...
    const selected = state.selectedRoomBuildingId &&
...
function syncRentPaymentBuildingOptions() {
    ...
    const rentEnabledBuildings = (Array.isArray(state.buildings) ? state.buildings : []).filter((building) => getPaymentAccessRecord(building.id)?.rentEnabled !== false);
    ...
    const selected = state.selectedRentPaymentBuildingId &&
...
function getRentEnabledBuildings() {
    return (Array.isArray(state.buildings) ? state.buildings : []).filter((building) => getPaymentAccessRecord(building.id)?.rentEnabled !== false);
}
function syncRentSheetBuildingOptions() {
    ...
    const selected = state.selectedRentSheetBuildingId &&
...
function getSelectedRentSheetBuildingId() {
    return String(rentSheetBuildingSelectEl?.value || state.selectedRentSheetBuildingId || "").trim();
}
...
function setRentSheetStatus(message) {
    if (rentSheetStatusEl instanceof HTMLElement) {
        rentSheetStatusEl.textContent = message;
    }
}
...
function setRentSheetDefaultInputs(data) {
    state.rentSheetDefaults = {
        ...
        depositKsh: data?.buildingDefaultDepositKsh ?? null,
        dueDay: data?.buildingDefaultDueDay ?? null,
        graceDays: data?.buildingDefaultGraceDays ?? 0,
        chargeStartDate: String(data?.chargeStartDate ?? "")
    };
    if (rentSheetDefaultMonthlyRentEl instanceof HTMLInputElement) {
        rentSheetDefaultMonthlyRentEl.value = optionalInputValue(data?.buildingDefaultMonthlyRentKsh);
    }
    if (rentSheetDefaultDepositEl instanceof HTMLInputElement) {
        rentSheetDefaultDepositEl.value = optionalInputValue(data?.buildingDefaultDepositKsh);
    }
    ...
    if (rentSheetDefaultGraceDaysEl instanceof HTMLInputElement) {
        rentSheetDefaultGraceDaysEl.value = optionalInputValue(data?.buildingDefaultGraceDays ?? 0);
    }
    if (rentSheetChargeStartDateEl instanceof HTMLInputElement) {
        rentSheetChargeStartDateEl.value = String(data?.chargeStartDate ?? "");
    }
    setRentSheetStatus("Current month paid cannot exceed the resolved monthly rent. Due day controls the monthly due date.");
}
function renderRentSheetRows(rows) {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = '<td colspan="13">No rooms found for this building.</td>';
        rentSheetBodyEl.append(row);
        return;
    }
    [...rows].sort((a, b) => compareHouseNumber(a.houseNumber, b.houseNumber)).forEach((item) => {
        const houseNumber = normalizeHouse(item.houseNumber);
        const resolvedMonthlyRentKsh = Math.max(0, Math.round(Number(item.resolvedMonthlyRentKsh ?? item.monthlyRentKsh ?? 0)));
        const balanceKsh = Math.max(0, Math.round(Number(item.balanceKsh ?? 0)));
        const currentMonthPaidKsh = Math.max(0, Math.round(Number(item.currentMonthPaidKsh ?? 0)));
        ...
        const depositKsh = Math.max(0, Math.round(Number(item.depositKsh ?? 0))) > 0
            ? optionalInputValue(item.depositKsh)
            : "";
        ...
        const rentPlaceholder = rentDefaultPlaceholder(state.rentSheetDefaults.monthlyRentKsh, formatCurrency);
        const depositPlaceholder = rentDefaultPlaceholder(state.rentSheetDefaults.depositKsh, formatCurrency);
        const dueDayPlaceholder = rentDefaultPlaceholder(state.rentSheetDefaults.dueDay, formatRentDueDay);
        const graceDaysPlaceholder = rentDefaultPlaceholder(state.rentSheetDefaults.graceDays, formatRentGraceDays);
        const sourceLabel = rentSetupSourceLabel(item.rentSetupSource);
        const resolvedDueLabel = item.resolvedDueDay == null ? "No due day" : `Day ${Math.round(Number(item.resolvedDueDay))}`;
        const resolvedGraceLabel = `${Math.max(0, Math.round(Number(item.resolvedGraceDays ?? 0)))} grace`;
        const residentLabel = String(item.residentName ?? "").trim() ||
        ...
        row.dataset.houseNumber = houseNumber;
        row.dataset.hasActiveResident = item.hasActiveResident ? "true" : "false";
        row.dataset.resolvedMonthlyRentKsh = String(resolvedMonthlyRentKsh);
        row.dataset.originalCurrentMonthPaidKsh = String(currentMonthPaidKsh);
        row.innerHTML = `
      <td><strong>${escapeHtml(houseNumber)}</strong></td>
      <td>
      ...
      <td>
        <input
          ...
          min="0"
          step="1"
          value="${escapeHtml(depositKsh)}"
          placeholder="${escapeHtml(depositPlaceholder)}"
        />
      </td>
      <td>
        <input
      ...
      <td>
        <input
          data-field="active"
          type="checkbox"
          title="Charge rent for this room when occupied"
          ${roomDefaultActive ? "checked" : ""}
        />
      </td>
      ...
      <td>
        <input
          class="registry-table-input utility-sheet-input"
          data-field="currentMonthPaidKsh"
          type="number"
          min="0"
          step="1"
          value="${currentMonthPaidKsh > 0 ? escapeHtml(currentMonthPaidKsh) : ""}"
          placeholder="${escapeHtml(formatCurrency(currentMonthPaidKsh))}"
        />
      </td>
...
function buildRentSheetPayload() {
    const buildingDefaultMonthlyRentKsh = rentSheetDefaultMonthlyRentEl instanceof HTMLInputElement
        ? toOptionalNumber(rentSheetDefaultMonthlyRentEl.value)
        : null;
    const buildingDefaultDepositKsh = rentSheetDefaultDepositEl instanceof HTMLInputElement
        ? toOptionalNumber(rentSheetDefaultDepositEl.value)
        : null;
    const buildingDefaultDueDay = rentSheetDefaultDueDayEl instanceof HTMLInputElement
        ? toOptionalNumber(rentSheetDefaultDueDayEl.value)
        : null;
    const buildingDefaultGraceDays = rentSheetDefaultGraceDaysEl instanceof HTMLInputElement
        ? toOptionalNumber(rentSheetDefaultGraceDaysEl.value)
        : 0;
    const chargeStartDate = rentSheetChargeStartDateEl instanceof HTMLInputElement
        ? String(rentSheetChargeStartDateEl.value || "").trim()
        : "";
    ...
    if (buildingDefaultDepositKsh != null && buildingDefaultDepositKsh < 0) {
        throw new Error("Default deposit cannot be negative.");
    }
    if (buildingDefaultDueDay != null &&
        (buildingDefaultDueDay < 1 || buildingDefaultDueDay > 31)) {
        throw new Error("Building Default due day must be from 1 to 31.");
    }
    if (buildingDefaultGraceDays != null &&
    ...
    trList.forEach((tr) => {
        ...
        const currentMonthPaidInput = tr.querySelector('input[data-field="currentMonthPaidKsh"]');
        ...
        const depositKsh = depositInput instanceof HTMLInputElement ? toOptionalNumber(depositInput.value) : null;
        if (depositKsh != null && depositKsh < 0) {
            throw new Error(`Deposit for ${houseNumber} cannot be negative.`);
        }
        const paymentDueDay = dueDayInput instanceof HTMLInputElement ? toOptionalNumber(dueDayInput.value) : null;
        if (paymentDueDay != null && (paymentDueDay < 1 || paymentDueDay > 31)) {
            throw new Error(`Due day for ${houseNumber} must be from 1 to 31.`);
        }
        const graceDays = graceDaysInput instanceof HTMLInputElement
        ...
        const currentMonthPaidKsh = currentMonthPaidInput instanceof HTMLInputElement
            ? toOptionalNumber(currentMonthPaidInput.value)
            : null;
        if (currentMonthPaidKsh != null && currentMonthPaidKsh < 0) {
            throw new Error(`Paid this month for ${houseNumber} cannot be negative.`);
        }
        const originalCurrentMonthPaidKsh = Math.max(0, Math.round(Number(tr.dataset.originalCurrentMonthPaidKsh ?? 0)));
        const resolvedMonthlyRentKsh = Math.max(0, Math.round(Number(monthlyRentKsh ??
            buildingDefaultMonthlyRentKsh ??
            tr.dataset.resolvedMonthlyRentKsh ??
            0)));
        const paidChanged = currentMonthPaidKsh != null &&
            Math.round(currentMonthPaidKsh) !== originalCurrentMonthPaidKsh;
        if (paidChanged && Math.round(currentMonthPaidKsh) > resolvedMonthlyRentKsh) {
            throw new Error(`Paid this month for ${houseNumber} cannot be more than ${formatCurrency(resolvedMonthlyRentKsh)}.`);
        }
        rows.push({
            houseNumber,
            monthlyRentKsh: monthlyRentKsh == null ? null : Math.round(monthlyRentKsh),
            depositKsh: depositKsh == null ? null : Math.round(depositKsh),
            currentMonthPaidKsh: paidChanged ? Math.round(currentMonthPaidKsh) : null,
    ...
    return {
        buildingDefaultMonthlyRentKsh: buildingDefaultMonthlyRentKsh == null ? null : Math.round(buildingDefaultMonthlyRentKsh),
        buildingDefaultDepositKsh: buildingDefaultDepositKsh == null ? null : Math.round(buildingDefaultDepositKsh),
        buildingDefaultDueDay: buildingDefaultDueDay == null ? null : Math.round(buildingDefaultDueDay),
        buildingDefaultGraceDays: buildingDefaultGraceDays == null ? 0 : Math.round(buildingDefaultGraceDays),
        chargeStartDate: chargeStartDate || null,
...
async function loadRentSheetRows() {
    ...
    const payload = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/rent-setup-sheet`);
...
function focusTenantsSection(buildingId) {
    if (buildingId) {
...
function renderRegistryBuildingOptions() {
    ...
    const knownSelection = state.selectedRegistryBuildingId &&
        state.buildings.some((item) => item.id === state.selectedRegistryBuildingId)
        ? state.selectedRegistryBuildingId
        : state.buildings[0].id;
...
function renderResidentsBuildingOptions() {
    ...
    const validSelection = state.selectedResidentsBuildingId === "all" ||
...
function syncOverviewLookupBuildingOptions() {
    ...
    const validSelection = state.selectedOverviewRoomBuildingId === "all" ||
...
function syncCaretakerBuildingOptions() {
    ...
    const selected = state.selectedCaretakerBuildingId &&
...
function renderOwnerStaff() {
    ...
    if (ownerStaffSummaryEl instanceof HTMLElement) {
        ...
        ownerStaffSummaryEl.textContent = `${activeCount} of ${limit} staff accounts active. ${remaining} ${slotText} available.`;
    }
    if (ownerStaffSubmitBtnEl instanceof HTMLButtonElement) {
        ownerStaffSubmitBtnEl.disabled = !ownerAccess || remaining <= 0;
    }
...
function syncLandlordTicketBuildingOptions() {
    ...
    if (state.selectedTicketBuildingId &&
...
function renderLandlordTickets(tickets) {
    ...
    tickets.forEach((ticket) => {
      ...
      <td>
        <div class="landlord-ticket-response">
          <span class="landlord-ticket-response-label">Resident update</span>
          <select data-action="status">${createLandlordTicketStatusOptions(ticket.status)}</select>
          <textarea data-action="note" rows="3" maxlength="500" placeholder="What did you send or tell the resident?">${escapeHtml(replyText || "")}</textarea>
          <button data-action="save" type="button">Save Update</button>
        </div>
      </td>
    `;
        ...
        const noteInput = row.querySelector('textarea[data-action="note"]');
        ...
        saveButton.addEventListener("click", () => {
            ...
            void (async () => {
                try {
                    await requestJson(`/api/landlord/tickets/${encodeURIComponent(ticket.id)}/status`, {
                        method: "PATCH",
                        headers: {
                            "content-type": "application/json"
                        },
...
function renderMoveOutSettlementReport(rows) {
    ...
    const debtRows = reportRows.filter((item) => item?.action === "transfer_to_resident_debt");
    ...
    const sumBy = (items, field) => items.reduce((sum, item) => sum + Math.max(0, Number(item?.[field] ?? 0)), 0);
    ...
    [...reportRows]
        ...
        .forEach((item) => {
        const row = document.createElement("tr");
        const buildingLabel = item.buildingName || getBuildingDisplayNameById(item.buildingId, "-");
        ...
        const canCollectDebt = !isCaretakerRole() &&
            item.action === "transfer_to_resident_debt" &&
            item.status === "resident_debt_open";
        const canRecordRefund = !isCaretakerRole() &&
            Number(item.depositRefundKsh ?? 0) > 0 &&
            item.status === "deposit_refund_due";
        const actionCell = canCollectDebt
            ? `<button type="button" class="ghost-btn" data-action="collect-resident-debt" data-settlement-id="${escapeHtml(item.id)}" data-resident-name="${escapeHtml(residentName)}" data-amount-ksh="${escapeHtml(String(Math.max(0, Number(item.amountKsh ?? 0))))}">Record Collection</button>`
            : canRecordRefund
                ? `<button type="button" class="ghost-btn" data-action="record-deposit-refund" data-settlement-id="${escapeHtml(item.id)}" data-resident-name="${escapeHtml(residentName)}" data-amount-ksh="${escapeHtml(String(Math.max(0, Number(item.depositRefundKsh ?? 0))))}">Mark Refunded</button>`
                : "-";
        row.innerHTML = `
        <td>${formatDateTime(item.createdAt)}</td>
        <td>${escapeHtml(buildingLabel)}</td>
        ...
        <td>
          <span class="settlement-outcome-pill ${escapeHtml(outcomeClass)}">${escapeHtml(formatSettlementAction(item.action))}</span>
          <small class="settlement-status-text">${escapeHtml(formatSettlementStatus(item.status))}</small>
        </td>
        <td>${escapeHtml(formatCurrency(item.amountKsh))}</td>
...
function handleCollectResidentDebtClick(target, settlementId, residentName, amountKsh) {
    ...
    const shouldProceed = window.confirm(`Record ${formatCurrency(amount)} collected from ${residentName || "this resident"}?`);
    if (!shouldProceed) {
        return;
    }
    target.disabled = true;
    ...
    void (async () => {
        try {
            const response = await requestJson(`/api/landlord/move-out-settlements/${encodeURIComponent(settlementId)}/collect`, {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
            ...
            setStatus(`${formatCurrency(collected)} resident debt marked as collected for ${response?.data?.residentName || residentName || "resident"}.`);
...
function handleRecordDepositRefundClick(target, settlementId, residentName, amountKsh) {
    ...
    const amount = Math.max(0, Number(amountKsh ?? 0));
    const shouldProceed = window.confirm(`Mark ${formatCurrency(amount)} deposit refund paid to ${residentName || "this resident"}?`);
    if (!shouldProceed) {
        return;
    }
    target.disabled = true;
    ...
    void (async () => {
        try {
            const response = await requestJson(`/api/landlord/move-out-settlements/${encodeURIComponent(settlementId)}/refund`, {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
            ...
            setStatus(`${formatCurrency(refunded)} deposit refund marked as paid for ${response?.data?.residentName || residentName || "resident"}.`);
            await loadMoveOutSettlements();
        }
        catch (error) {
...
function handleDeleteExpenditureClick(target, expenditureId, title) {
    ...
    const shouldProceed = window.confirm(`Delete expenditure "${title || "Untitled expenditure"}"?`);
    if (!shouldProceed) {
        return;
    }
    target.disabled = true;
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/expenditures/${encodeURIComponent(expenditureId)}`, {
                method: "DELETE"
            });
...
function getMoveOutSettlementAction() {
    const selected = moveOutSettlementFormEl?.querySelector('input[name="moveOutSettlementAction"]:checked');
...
function renderMoveOutSettlement(summary, context) {
    ...
    if (moveOutSettlementSummaryEl instanceof HTMLElement) {
        moveOutSettlementSummaryEl.textContent = `${summary?.resident?.fullName || context.residentName} • ${summary?.building?.name || getBuildingDisplayNameById(context.buildingId)} • House ${summary?.houseNumber || context.houseNumber}`;
    }
    ...
    const collectInput = moveOutSettlementFormEl?.querySelector('input[name="moveOutSettlementAction"][value="collect_before_move_out"]');
    const writeOffInput = moveOutSettlementFormEl?.querySelector('input[name="moveOutSettlementAction"][value="write_off"]');
    const transferInput = moveOutSettlementFormEl?.querySelector('input[name="moveOutSettlementAction"][value="transfer_to_resident_debt"]');
...
async function openMoveOutSettlement(target, context) {
    ...
    try {
        const response = await requestJson(`/api/landlord/buildings/${encodeURIComponent(context.buildingId)}/users/${encodeURIComponent(context.userId)}/move-out-settlement`);
...
function renderRegistryRows(rows) {
    ...
    rows.forEach((item) => {
        const houseNumber = normalizeHouse(item.houseNumber);
        const waterReadingBill = getUtilityBillForMonth("water", buildingId, houseNumber, billingMonth);
        const electricityReadingBill = getUtilityBillForMonth("electricity", buildingId, houseNumber, billingMonth);
        const hasBothMeters = hasUsableMeterNumber(item.waterMeterNumber) &&
            hasUsableMeterNumber(item.electricityMeterNumber);
        const waterMeterNumber = normalizeUtilityMeterNumber(item.waterMeterNumber);
        const electricityMeterNumber = normalizeUtilityMeterNumber(item.electricityMeterNumber);
        const row = document.createElement("tr");
        row.dataset.houseNumber = houseNumber;
        row.dataset.hasBothMeters = hasBothMeters ? "true" : "false";
        row.innerHTML = `
...
function buildRegistrySavePayload() {
    ...
    trList.forEach((tr) => {
        ...
        const waterFixedInput = tr.querySelector('input[data-field="waterFixedChargeKsh"]');
        const electricityFixedInput = tr.querySelector('input[data-field="electricityFixedChargeKsh"]');
        const combinedUtilityInput = tr.querySelector('input[data-field="combinedUtilityChargeKsh"]');
        if (!(membersInput instanceof HTMLInputElement) ||
        ...
        if (!Number.isInteger(members) || members < 0 || members > 20) {
            throw new Error(`Members for ${houseNumber} must be a whole number between 0 and 20.`);
        }
        ...
        const electricityFixedChargeKsh = toOptionalNumber(electricityFixedInput.value) ?? 0;
        const combinedUtilityChargeKsh = toOptionalNumber(combinedUtilityInput.value) ?? 0;
        if (waterFixedChargeKsh < 0 || waterFixedChargeKsh > 200000) {
            throw new Error(`Water fixed charge for ${houseNumber} must be between 0 and 200,000.`);
        }
        if (electricityFixedChargeKsh < 0 || electricityFixedChargeKsh > 200000) {
            throw new Error(`Electric fixed charge for ${houseNumber} must be between 0 and 200,000.`);
        }
        if (combinedUtilityChargeKsh < 0 || combinedUtilityChargeKsh > 200000) {
            throw new Error(`Room utility amount for ${houseNumber} must be between 0 and 200,000.`);
        }
        rows.push({
            houseNumber,
            householdMembers: members,
            waterMeterNumber: normalizeUtilityMeterNumber(waterMeterNumber) || undefined,
            electricityMeterNumber: normalizeUtilityMeterNumber(electricityMeterNumber) || undefined,
            waterFixedChargeKsh,
            electricityFixedChargeKsh,
...
function renderApplications(rows) {
    ...
    rows.forEach((item) => {
      ...
      <td>${escapeHtml(occupationSummary.title)}${occupationSummary.details
...
function renderRentStatus(rows) {
    ...
    visibleRows.forEach((item) => {
        ...
        const latestPaymentBillingMonth = String(item.latestPaymentBillingMonth || "").trim();
        const latestPaymentRecordedAt = String(item.latestPaymentRecordedAt || item.latestPaymentAt || "").trim();
        const canRecordPayment = quickPaymentAmountKsh > 0;
        const rentVisualState = getRentCollectionVisualState(item);
        row.className = tableRowToneClass(rentVisualState.toneClass);
        row.innerHTML = `
      <td>${escapeHtml(buildingLabel)}</td>
      <td>${item.houseNumber}</td>
      <td>${renderTableStatusPill(rentVisualState.label, rentVisualState.toneClass)}</td>
      ...
      <td>${escapeHtml(formatBillingMonth(billingMonth))}</td>
      <td>${formatDateTime(item.dueDate)}</td>
      <td>${escapeHtml(latestPaymentBillingMonth ? formatBillingMonth(latestPaymentBillingMonth) : "-")}</td>
      <td>${escapeHtml(latestPaymentRecordedAt ? formatDateTime(latestPaymentRecordedAt) : "-")}</td>
...
function renderOverviewCollections(rows) {
    ...
    if (visibleRows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = '<td colspan="12">No rent collection records for the current building.</td>';
        overviewCollectionsBodyEl.append(row);
        return;
    }
    const rankedRows = [...visibleRows].sort(compareStableRoomOrder);
    rankedRows.forEach((item) => {
        const row = document.createElement("tr");
        const buildingLabel = getBuildingDisplayNameById(item.buildingId, "-");
        const resident = findResidentDirectoryEntry(item.buildingId, item.houseNumber);
        const utilitySummary = resident ? getResidentUtilityRoomSummary(resident) : null;
        const monthlyRentKsh = resident
            ? getResidentMonthlyRentKsh(resident)
            : Number(item.monthlyRentKsh ?? 0);
        const paidThisMonthKsh = resident
            ? getResidentCurrentMonthRentPaidKsh(resident)
            : Number(item.currentMonthPaidKsh ?? item.paidAmountKsh ?? 0);
        const latePenaltyKsh = resident
            ? getResidentCurrentLatePenaltyKsh(resident)
            : Math.max(0, Number(item.currentMonthLatePenaltyKsh ?? 0));
        const dueThisMonthKsh = resident
            ? getResidentOperationalCurrentDueKsh(resident, utilitySummary)
            : Number(item.currentMonthOutstandingKsh ?? item.balanceKsh ?? 0);
        const totalOutstandingKsh = resident
            ? getResidentOperationalOutstandingKsh(resident, utilitySummary)
            : Number(item.balanceKsh ?? dueThisMonthKsh ?? 0);
        const rentVisualState = resident
            ? {
                label: getResidentBillingStatusLabel(resident),
                toneClass: getResidentBillingToneClass(resident, utilitySummary)
            }
            : getRentCollectionVisualState(item);
        const latestPayment = Number(item.latestPaymentAmountKsh ?? 0) > 0
            ? formatCurrency(item.latestPaymentAmountKsh)
            : "-";
        const latestPaymentBillingMonth = String(item.latestPaymentBillingMonth || "").trim();
        const latestPaymentRecordedAt = String(item.latestPaymentRecordedAt || item.latestPaymentAt || "").trim();
        row.className = tableRowToneClass(rentVisualState.toneClass);
      ...
      <td>${escapeHtml(item.houseNumber)}</td>
      <td>${renderTableStatusPill(rentVisualState.label, rentVisualState.toneClass)}</td>
      <td>${escapeHtml(formatCurrency(monthlyRentKsh))}</td>
      <td>${escapeHtml(formatCurrency(paidThisMonthKsh))}</td>
      <td>${escapeHtml(formatCurrency(latePenaltyKsh))}</td>
      <td>${escapeHtml(formatCurrency(dueThisMonthKsh))}</td>
...
function renderRoomLedgerActions(resident, totalBalanceKsh) {
    ...
    const hasResident = Boolean(residentUserId ||
        resident?.hasActiveResident ||
        String(resident?.residentName ?? "").trim());
    const buttons = [
        `<button type="button" data-action="open-room-account" data-building-id="${escapeHtml(buildingId)}" data-house-number="${escapeHtml(houseNumber)}">Manage</button>`
    ];
    if (!hasResident && !isCaretakerRole()) {
        buttons.push(`<button type="button" data-action="open-direct-tenant-drawer" data-building-id="${escapeHtml(buildingId)}" data-house-number="${escapeHtml(houseNumber)}">Add Tenant</button>`);
    }
    if (hasResident && residentUserId) {
        buttons.push(`<button type="button" class="btn-danger" data-action="remove-resident" data-building-id="${escapeHtml(buildingId)}" data-house-number="${escapeHtml(houseNumber)}" data-user-id="${escapeHtml(residentUserId)}" data-resident-name="${escapeHtml(residentName)}">Clear</button>`);
    }
    if (!hasResident && !isCaretakerRole() && totalBalanceKsh > 0) {
        buttons.push(`<button type="button" class="btn-danger" data-action="write-off-room-balance" data-building-id="${escapeHtml(buildingId)}" data-house-number="${escapeHtml(houseNumber)}" data-amount-ksh="${escapeHtml(totalBalanceKsh)}">Clear Balance</button>`);
    }
    if (!hasResident && !isCaretakerRole() && totalBalanceKsh <= 0) {
        buttons.push(`<button type="button" class="btn-danger" data-action="remove-room" data-building-id="${escapeHtml(buildingId)}" data-house-number="${escapeHtml(houseNumber)}">Remove Room</button>`);
    }
...
function renderRoomLedger(rows) {
    ...
    const totals = visibleRows.reduce((summary, resident) => {
        const hasResident = resident?.hasActiveResident || resident?.residentUserId || resident?.residentName;
        const utilitySummary = getResidentUtilityRoomSummary(resident);
        const totalBalanceKsh = getResidentOperationalOutstandingKsh(resident, utilitySummary);
        const latestSettlement = getLatestRoomSettlement(resident?.buildingId, resident?.houseNumber);
    ...
    if (visibleRows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = `<td class="table-cell-full" colspan="${roomLedgerColumnCount}">No rooms matched "${escapeHtml(state.residentSearchQuery)}".</td>`;
        roomLedgerBodyEl.append(row);
        return;
    }
    visibleRows.forEach((resident) => {
        ...
        const billingToneClass = getResidentBillingToneClass(resident, utilitySummary);
        const rentCurrentDueKsh = getResidentCurrentRentDueKsh(resident);
        const rentArrearsKsh = getResidentRentArrearsKsh(resident);
        const rentBalanceKsh = Math.max(rentCurrentDueKsh + rentArrearsKsh, Math.max(0, utilityAmount(resident.rentBalanceKsh)));
        const monthlyRentKsh = getResidentMonthlyRentKsh(resident);
        const utilityOpenKsh = Math.max(getResidentUtilityBalanceKsh(resident), utilityAmount(utilitySummary?.totalOpenBalanceKsh));
      ...
      <td>
        ...
        <span class="table-status-pill room-ledger-billing-note ${billingToneClass}">${escapeHtml(billingStatus)}</span>
      </td>
      <td>
...
function renderResidentDirectory(rows) {
    ...
    if (filteredRows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = `<td class="table-cell-full" colspan="${residentDirectoryColumnCount}">No rooms matched "${escapeHtml(state.residentSearchQuery)}".</td>`;
        residentsBodyEl.append(row);
        return;
    }
    filteredRows.forEach((resident) => {
        ...
        const latePenalty = hasResident
            ...
            : "-";
        const outstandingBalanceKsh = getResidentOperationalOutstandingKsh(resident, utilitySummary);
      ...
      >
        ...
        if (!hasResident && !isCaretakerRole()) {
          ...
          type="button"
          data-action="open-direct-tenant-drawer"
          data-building-id="${escapeHtml(resident.buildingId)}"
          data-house-number="${escapeHtml(resident.houseNumber)}"
        >
          Add Tenant
      ...
      <td>${renderTableStatusPill(billingStatus, billingToneClass)}</td>
...
function renderResidentDrawer(resident) {
    ...
    const billingStatus = hasResident ? getResidentBillingStatusLabel(resident) : "-";
    const outstandingBalanceKsh = getResidentOutstandingBalanceKsh(resident);
    const totalOutstanding = hasResident ? formatCurrency(outstandingBalanceKsh) : "-";
    const nextDueDate = getResidentNextDueDate(resident);
    const nextDue = hasResident && nextDueDate ? formatDateTime(nextDueDate) : "-";
    const latestReceipt = hasResident ? resident.latestRentPaymentReference ?? "-" : "-";
    const latestPaidAt = hasResident && resident.latestRentPaymentAt
    ...
    const electricityMeter = normalizeUtilityMeterNumber(resident.electricityMeterNumber) || "Missing";
    ...
    const roomIssues = Array.isArray(state.tickets)
        ? state.tickets.filter((ticket) => ticket.buildingId === resident.buildingId &&
            normalizeHouse(ticket.houseNumber) === normalizeHouse(resident.houseNumber))
        : [];
    const roomIssuesSummary = roomIssues.length === 0
        ? '<p class="status-text">No room issues recorded for this room.</p>'
        : `<div class="stack-list">${roomIssues
            .slice(0, 4)
            .map((ticket) => `
              <article class="package-card">
                <p class="status-text">${escapeHtml(ticket.queue)} • ${escapeHtml(ticket.status)} • ${formatDateTime(ticket.createdAt)}</p>
    ...
    const roomUtilityPaidKsh = roomUtilityPayments.reduce((sum, item) => sum + Number(item?.amountKsh ?? 0), 0);
    const roomExpenditureTotalKsh = roomExpenditures.reduce((sum, item) => sum + Number(item?.amountKsh ?? 0), 0);
    ...
    if (overappliedUtilityBills.length > 0) {
        roomLedgerFlags.push(`${overappliedUtilityBills.length} utility bill${overappliedUtilityBills.length === 1 ? "" : "s"} ${overappliedUtilityBills.length === 1 ? "has" : "have"} payments above the posted charge. Review allocation history.`);
    }
    if (roomExpenditureTotalKsh > 0) {
        roomLedgerFlags.push(`Room-specific charges total ${formatCurrency(roomExpenditureTotalKsh)} and should count toward the resident outstanding balance.`);
    }
    const agreementPayload = sameResidentKey(state.selectedResident, resident) && state.selectedResidentAgreement
    ...
    const agreementError = sameResidentKey(state.selectedResident, resident) && state.selectedResidentAgreementError
        ? state.selectedResidentAgreementError
        : "";
    ...
    const workSchoolSummary = agreement?.organizationName
        ? `${agreement.organizationName}${agreement.organizationLocation ? ` • ${agreement.organizationLocation}` : ""}`
        : "Not recorded";
    const leaseSummary = agreement?.leaseStartDate || agreement?.leaseEndDate
        ? `${agreement?.leaseStartDate ? formatDateOnly(agreement.leaseStartDate) : "open"} -> ${agreement?.leaseEndDate ? formatDateOnly(agreement.leaseEndDate) : "ongoing"}`
        : "Not recorded";
    const monthlyRentKsh = getResidentMonthlyRentKsh(resident, agreement);
    const currentRentDueKsh = getResidentCurrentRentDueKsh(resident, agreement);
    ...
    const monthlyRent = hasResident && monthlyRentKsh > 0 ? formatCurrency(monthlyRentKsh) : "-";
    const currentRentDue = hasResident && (rentEnabled || currentRentDueKsh > 0)
        ? formatCurrency(currentRentDueKsh)
        : "-";
    const rentArrears = hasResident && (rentEnabled || rentArrearsKsh > 0)
        ? formatCurrency(rentArrearsKsh)
        : "-";
    const currentLatePenalty = hasResident && (rentEnabled || currentMonthLatePenaltyKsh > 0)
    ...
    const compactDrawer = typeof window !== "undefined" &&
    ...
    const canEditRentProfile = rentEnabled &&
        !isCaretakerRole() &&
  ...
  `;
    ...
    <div class="resident-summary">
      <p class="status-text">${escapeHtml(buildingLabel)} • House ${escapeHtml(resident.houseNumber)}</p>
    ...
    <div class="resident-grid resident-grid-primary">
      ...
      ${rentEnabled
        ? `<div class="resident-grid-card-highlight"><span>Monthly Rent</span><strong>${escapeHtml(monthlyRent)}</strong></div>
      <div class="resident-grid-card-highlight"><span>Current Rent Due</span><strong>${escapeHtml(currentRentDue)}</strong></div>
    ...
    <details class="resident-drawer-panel resident-agreement-card" ${agreementOpenAttr}>
      ...
      <div class="resident-drawer-panel-body">
        ...
        <div class="resident-agreement-overview">
          ...
          <div><span>Occupation</span><strong>${escapeHtml(formatAgreementOccupationStatus(agreement?.occupationStatus))}</strong></div>
          <div><span>Work / School</span><strong>${escapeHtml(workSchoolSummary)}</strong></div>
          <div><span>Emergency Contact</span><strong>${escapeHtml(agreement?.emergencyContactName
        ? `${agreement.emergencyContactName}${agreement?.emergencyContactPhone ? ` • ${agreement.emergencyContactPhone}` : ""}`
        : "Not recorded")}</strong></div>
          <div><span>Lease</span><strong>${escapeHtml(leaseSummary)}</strong></div>
          <div><span>Deposit</span><strong>${escapeHtml(formatCurrency(Number(agreement?.depositKsh ?? resident.depositKsh ?? 0)))}</strong></div>
        </div>
        ${agreementResident
        ? `<p class="status-text resident-agreement-note">Active resident on this agreement: ${escapeHtml(agreementResident.fullName ?? residentName)} • ${escapeHtml(agreementResident.phone ?? residentPhone)}</p>`
        : ""}
        ${agreementError
    ...
    <details class="resident-drawer-panel" ${roomLedgerOpenAttr}>
      ...
      <div class="resident-drawer-panel-body">
        <div class="resident-agreement-overview resident-ledger-overview">
          ...
          <div><span>Utility Paid</span><strong>${escapeHtml(formatCurrency(roomUtilityPaidKsh))}</strong></div>
          <div><span>Room Charges</span><strong>${escapeHtml(formatCurrency(roomExpenditureTotalKsh))}</strong></div>
          <div><span>Latest Bill Month</span><strong>${escapeHtml(latestLedgerBillingLabel)}</strong></div>
        </div>
  ...
  `;
    ...
    <div class="resident-grid resident-grid-primary">
      ...
      ${rentEnabled
        ? `<div class="resident-grid-card-highlight"><span>Monthly Rent</span><strong>${escapeHtml(monthlyRent)}</strong></div>
      <div class="resident-grid-card-highlight"><span>Current Rent Due</span><strong>${escapeHtml(currentRentDue)}</strong></div>
      <div class="resident-grid-card-highlight"><span>Rent Arrears</span><strong>${escapeHtml(rentArrears)}</strong></div>
      <div><span>Utility Balance</span><strong>${escapeHtml(utilityBalance)}</strong></div>`
        : `<div class="resident-grid-card-highlight"><span>Current Utility Due</span><strong>${escapeHtml(currentUtilityDue)}</strong></div>
      <div class="resident-grid-card-highlight"><span>Utility Arrears</span><strong>${escapeHtml(utilityArrears)}</strong></div>
      <div><span>Utility Balance</span><strong>${escapeHtml(utilityBalance)}</strong></div>`}
    ...
    <details class="resident-drawer-panel" ${roomProfileOpenAttr}>
      ...
      <div class="resident-drawer-panel-body">
        ...
        : `<div><span>Current Utility Due</span><strong>${escapeHtml(currentUtilityDue)}</strong></div>
          ...
          <div><span>Occupation</span><strong>${escapeHtml(agreement?.occupationLabel ||
        formatAgreementOccupationStatus(agreement?.occupationStatus))}</strong></div>
          <div><span>Work / School</span><strong>${escapeHtml(workSchoolSummary)}</strong></div>
          <div><span>Emergency Contact</span><strong>${escapeHtml(agreement?.emergencyContactName
        ? `${agreement.emergencyContactName}${agreement?.emergencyContactPhone ? ` • ${agreement.emergencyContactPhone}` : ""}`
        : "Not recorded")}</strong></div>
    ...
    <details class="resident-drawer-panel" ${roomLedgerOpenAttr}>
      ...
      <div class="resident-drawer-panel-body">
        ...
        ? `<div class="stack-list">${roomUtilityBills
            ...
            return `
                        <article class="package-card resident-ledger-card">
                          <p class="status-text">${escapeHtml(utilityTypeLabel(bill?.utilityType))} • ${escapeHtml(formatBillingMonth(bill?.billingMonth))} • Due ${escapeHtml(dueLabel)}</p>
                          <h4>${escapeHtml(formatCurrency(billBalanceKsh))} open of ${escapeHtml(formatCurrency(billAmountKsh))}</h4>
                          <p class="status-text">Status ${escapeHtml(billStatus)} • Paid ${escapeHtml(formatCurrency(paidAmountKsh))}</p>
                          ${bill?.note
        ...
        ? `<div class="stack-list">${roomUtilityPayments
            ...
            .map((payment) => `
                        <article class="package-card resident-ledger-card">
                          <p class="status-text">${escapeHtml(formatPaymentProvider(payment?.provider))} • ${escapeHtml(payment?.paidAt ? formatDateTime(payment.paidAt) : "-")}</p>
                          <h4>${escapeHtml(formatCurrency(Number(payment?.amountKsh ?? 0)))}</h4>
                          <p class="status-text">${escapeHtml(utilityTypeLabel(payment?.utilityType))} • ${escapeHtml(formatUtilityPaymentCoverage(payment, payment?.billingMonth) || "-")}</p>
                          <p class="status-text">${escapeHtml(payment?.providerReference || payment?.note || "No reference recorded.")}</p>
                        </article>
                      `)
            .join("")}</div>
        ...
        ? `<div class="stack-list">${roomExpenditures
            ...
            .map((item) => `
                        <article class="package-card resident-ledger-card">
                          <p class="status-text">${escapeHtml(formatExpenditureCategory(item?.category))} • ${escapeHtml(item?.createdAt ? formatDateTime(item.createdAt) : "-")}</p>
                          <h4>${escapeHtml(formatCurrency(Number(item?.amountKsh ?? 0)))}</h4>
    ...
    ${rentEnabled
      ...
      <summary>
        ...
        <small>${canRecordCashPayment
            ? "Posts to this room immediately"
            : hasResident
                ? "Read only"
                : "No active resident"}</small>
      </summary>
      <div class="resident-drawer-panel-body">
        <div class="resident-grid resident-grid-secondary">
          <div><span>Current Rent Due</span><strong>${escapeHtml(currentRentDue)}</strong></div>
    ...
    ${rentEnabled
      ...
      <summary>
        ...
        <small>${canEditRentProfile ? "Management can edit" : "Read only"}</small>
      </summary>
      <div class="resident-drawer-panel-body">
        <div class="resident-grid resident-grid-secondary">
          <div><span>Current Due Date</span><strong>${escapeHtml(resident.rentDueDate ? formatDateTime(resident.rentDueDate) : "-")}</strong></div>
          <div><span>Overdue Starts</span><strong>${escapeHtml(overdueStartSummary)}</strong></div>
          <div><span>Grace Days</span><strong>${escapeHtml(String(rentGraceDays))}</strong></div>
          <div><span>Current Balance</span><strong>${escapeHtml(formatCurrency(Number(resident.rentBalanceKsh ?? 0)))}</strong></div>
        </div>
        ${canEditRentProfile
            ? `<p class="status-text resident-agreement-note">
    ...
    <details class="resident-drawer-panel resident-agreement-card" ${agreementOpenAttr}>
      <summary>
        ...
        <small>${canEditAgreement ? "Management can edit" : hasResident ? "Read only" : "No active resident"}</small>
      </summary>
      <div class="resident-drawer-panel-body">
        <p class="status-text">${escapeHtml(agreementStatusText)}</p>
      <div class="resident-agreement-overview">
        ...
        <div class="inline-fields compact-fields resident-agreement-grid">
          <label>
            ...
            <select name="identityType" ${disabledAttr}>
              ...
              <option value="national_id" ${agreement?.identityType === "national_id" ? "selected" : ""}>National ID</option>
              <option value="passport" ${agreement?.identityType === "passport" ? "selected" : ""}>Passport</option>
              <option value="alien_id" ${agreement?.identityType === "alien_id" ? "selected" : ""}>Alien ID</option>
          ...
          <label>
            ...
            <select name="occupationStatus" ${disabledAttr}>
              <option value="">Select</option>
              <option value="employed" ${agreement?.occupationStatus === "employed" ? "selected" : ""}>Employed</option>
              <option value="self_employed" ${agreement?.occupationStatus === "self_employed" ? "selected" : ""}>Self-employed</option>
              <option value="student" ${agreement?.occupationStatus === "student" ? "selected" : ""}>Student</option>
              <option value="sponsored" ${agreement?.occupationStatus === "sponsored" ? "selected" : ""}>Sponsored</option>
              <option value="unemployed" ${agreement?.occupationStatus === "unemployed" ? "selected" : ""}>Unemployed</option>
              <option value="other" ${agreement?.occupationStatus === "other" ? "selected" : ""}>Other</option>
            </select>
          </label>
          <label>
            Role / Course / Trade
...
function renderPaymentAccess(rows) {
    ...
    rows.forEach((item) => {
        const row = document.createElement("tr");
        const safeBuildingName = getBuildingDisplayNameById(item.buildingId, item.buildingName ?? "Building");
        const canEdit = !isCaretakerRole();
        const rentGraceDays = Math.max(0, Math.round(Number(item.rentGraceDays ?? 0)));
        const lateRentPenaltyAmountKsh = Math.max(0, Math.round(Number(item.lateRentPenaltyAmountKsh ?? 0)));
        row.innerHTML = `
...
function renderPaymentProfiles() {
    ...
    if (paymentProfilesSummaryEl instanceof HTMLElement) {
        ...
        paymentProfilesSummaryEl.textContent = `${profiles.length} payment profile${profiles.length === 1 ? "" : "s"} available. ${configuredCount} configured for STK.`;
    }
    ...
    const profileOptions = profiles
        ...
        return `<option value="${escapeHtml(profile.id)}">${escapeHtml(`${profile.name} (${status})`)}</option>`;
    })
        .join("");
...
function renderPaymentInstructions() {
    ...
    if (paymentInstructionsSummaryEl instanceof HTMLElement) {
        ...
        paymentInstructionsSummaryEl.textContent = `${configuredCount} of ${rows.length} building${rows.length === 1 ? "" : "s"} have visible payment details.`;
    }
    if (rows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML =
    ...
    rows.forEach((item) => {
      ...
      <td>
        <div class="payment-instruction-fields">
          <input data-setting="mpesaBusinessNumber" type="text" maxlength="40" value="${escapeHtml(item.mpesaBusinessNumber || "")}" placeholder="${escapeHtml(effective.mpesaBusinessNumber || "Paybill or Till")}" ${canEdit ? "" : "disabled"} />
          <input data-setting="mpesaAccountReference" type="text" maxlength="80" value="${escapeHtml(item.mpesaAccountReference || "")}" placeholder="${escapeHtml(effective.mpesaAccountReference || "Account reference")}" ${canEdit ? "" : "disabled"} />
          <input data-setting="mpesaAccountName" type="text" maxlength="120" value="${escapeHtml(item.mpesaAccountName || "")}" placeholder="${escapeHtml(effective.mpesaAccountName || "Account name")}" ${canEdit ? "" : "disabled"} />
        </div>
      </td>
      <td>
        <div class="payment-instruction-fields">
          <input data-setting="bankName" type="text" maxlength="120" value="${escapeHtml(item.bankName || "")}" placeholder="Bank name" ${canEdit ? "" : "disabled"} />
          <input data-setting="bankAccountName" type="text" maxlength="120" value="${escapeHtml(item.bankAccountName || "")}" placeholder="Account name" ${canEdit ? "" : "disabled"} />
          <input data-setting="bankAccountNumber" type="text" maxlength="80" value="${escapeHtml(item.bankAccountNumber || "")}" placeholder="Account number" ${canEdit ? "" : "disabled"} />
          <input data-setting="bankBranch" type="text" maxlength="120" value="${escapeHtml(item.bankBranch || "")}" placeholder="Branch" ${canEdit ? "" : "disabled"} />
          <input data-setting="bankSwiftCode" type="text" maxlength="40" value="${escapeHtml(item.bankSwiftCode || "")}" placeholder="SWIFT or bank code" ${canEdit ? "" : "disabled"} />
        </div>
      </td>
      <td>
        <div class="payment-instruction-fields">
          <input data-setting="cashLocation" type="text" maxlength="160" value="${escapeHtml(item.cashLocation || "")}" placeholder="Office or contact" ${canEdit ? "" : "disabled"} />
          <textarea data-setting="instructions" maxlength="800" placeholder="Resident payment notes" ${canEdit ? "" : "disabled"}>${escapeHtml(item.instructions || "")}</textarea>
          <textarea data-setting="proofInstructions" maxlength="800" placeholder="Receipt or proof notes" ${canEdit ? "" : "disabled"}>${escapeHtml(item.proofInstructions || "")}</textarea>
        </div>
      </td>
      <td>${formatDateTime(item.updatedAt)}${item.updatedByRole ? `<br /><small>${escapeHtml(item.updatedByRole)}</small>` : ""}</td>
...
function renderUtilityBills(rows) {
    ...
    if (visibleRows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = '<td colspan="10">No utility bills posted.</td>';
    ...
    visibleRows.forEach((item) => {
        const row = document.createElement("tr");
        const displayStatus = getUtilityDisplayStatus(item);
        const toneClass = utilityStatusToneClass(displayStatus);
        row.className = tableRowToneClass(toneClass);
        row.innerHTML = `
      <td>${item.utilityType}</td>
      <td>${item.houseNumber}</td>
...
function renderUtilityRoomSummary(rows) {
    ...
    utilityRoomSummaryBodyEls.forEach((bodyEl) => {
        ...
        if (summaryRows.length === 0) {
            const row = document.createElement("tr");
            const emptyText = allSummaryRows.length === 0
        ...
        summaryRows.forEach((item) => {
            const accountBuildingId = item.buildingId ||
                getSelectedUtilityBuildingId() ||
                state.selectedResidentsBuildingId ||
                state.selectedRegistryBuildingId ||
                "";
            const row = document.createElement("tr");
            row.className = ["account-drilldown-row", tableRowToneClass(utilityStatusToneClass(item.status))]
                .filter(Boolean)
                .join(" ");
            row.dataset.action = "open-room-account-row";
...
function canCorrectPayment(item) {
    return !isCaretakerRole() && normalizePaymentProvider(item?.provider) !== "mpesa";
}
function buildCorrectionActionButtons(item, type) {
    if (!canCorrectPayment(item)) {
        return normalizePaymentProvider(item?.provider) === "mpesa" ? "M-PESA locked" : "-";
    }
    const action = type === "rent" ? "rent" : "utility";
    const utilityType = String(item?.utilityType ?? "").trim();
    const paymentId = escapeHtml(item?.id ?? item?.paymentId ?? "");
    const buildingId = escapeHtml(item?.buildingId ?? "");
    const houseNumber = escapeHtml(item?.houseNumber ?? "");
    const billingMonth = escapeHtml(item?.billingMonth ?? "");
    const paidAt = escapeHtml(item?.paidAt ?? "");
    const provider = escapeHtml(item?.provider ?? "cash");
    const providerReference = escapeHtml(item?.providerReference ?? "");
    const amountKsh = escapeHtml(Number(item?.amountKsh ?? 0));
    const note = escapeHtml(item?.note ?? "");
    const utilityAttribute = utilityType
        ? ` data-utility-type="${escapeHtml(utilityType)}"`
        : "";
    return `
    <div class="resident-row-actions">
      <button
        type="button"
        data-action="edit-${action}-payment"
        data-payment-id="${paymentId}"
        data-building-id="${buildingId}"
        data-house-number="${houseNumber}"
        data-billing-month="${billingMonth}"
        data-paid-at="${paidAt}"
        data-provider="${provider}"
        data-provider-reference="${providerReference}"
        data-amount-ksh="${amountKsh}"
        data-note="${note}"${utilityAttribute}
      >Edit</button>
      <button
        type="button"
        class="btn-danger"
        data-action="unrecord-${action}-payment"
        data-payment-id="${paymentId}"
        data-building-id="${buildingId}"
        data-house-number="${houseNumber}"
        data-amount-ksh="${amountKsh}"${utilityAttribute}
      >Unrecord</button>
    </div>
  `;
}
function renderPaymentCorrections() {
    renderCorrectionRentPayments(state.rentPayments);
    renderCorrectionUtilityPayments(state.payments);
}
function renderCorrectionRentPayments(rows) {
    if (!(correctionRentPaymentsBodyEl instanceof HTMLElement)) {
        return;
    }
    correctionRentPaymentsBodyEl.replaceChildren();
    const visibleRows = filterCorrectionPaymentRows(rows);
    if (visibleRows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = '<td colspan="9">No rent payments found for the current filters.</td>';
        correctionRentPaymentsBodyEl.append(row);
        return;
    }
    visibleRows.forEach((item) => {
        const row = document.createElement("tr");
        row.innerHTML = `
      <td>${escapeHtml(getBuildingDisplayNameById(item.buildingId, item.buildingId || "-"))}</td>
      <td>${escapeHtml(item.houseNumber ?? "-")}</td>
      ...
      <td>${buildCorrectionActionButtons(item, "rent")}</td>
    `;
        correctionRentPaymentsBodyEl.append(row);
    });
}
function renderCorrectionUtilityPayments(rows) {
    if (!(correctionUtilityPaymentsBodyEl instanceof HTMLElement)) {
        return;
    }
    correctionUtilityPaymentsBodyEl.replaceChildren();
    const visibleRows = filterCorrectionPaymentRows(rows).filter((item) => {
        const selectedUtility = String(paymentCorrectionUtilityTypeEl?.value || "").trim();
        return !selectedUtility || String(item.utilityType ?? "") === selectedUtility;
    });
    if (visibleRows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = '<td colspan="10">No utility payments found for the current filters.</td>';
        correctionUtilityPaymentsBodyEl.append(row);
        return;
    }
    visibleRows.forEach((item) => {
        const row = document.createElement("tr");
      ...
      <td>${escapeHtml(formatCurrency(item.amountKsh))}</td>
      <td>${escapeHtml(formatDateTime(item.paidAt))}</td>
      <td>${buildCorrectionActionButtons(item, "utility")}</td>
    `;
        correctionUtilityPaymentsBodyEl.append(row);
    });
}
function filterCorrectionPaymentRows(rows) {
    const buildingId = getSelectedPaymentCorrectionBuildingId();
    const houseNumber = normalizeHouse(paymentCorrectionHouseEl?.value);
    return (Array.isArray(rows) ? rows : []).filter((item) => {
        const matchesBuilding = !buildingId || normalizeLookupBuildingId(item.buildingId) === buildingId;
        const matchesHouse = !houseNumber || normalizeHouse(item.houseNumber) === houseNumber;
        return matchesBuilding && matchesHouse;
    });
}
function getSelectedPaymentCorrectionBuildingId() {
    const value = String(paymentCorrectionBuildingSelectEl?.value || state.selectedPaymentCorrectionBuildingId || "all").trim();
    return value === "all" ? "" : value;
}
function syncPaymentCorrectionBuildingOptions() {
    const selects = [paymentCorrectionBuildingSelectEl, billingHoldBuildingSelectEl].filter((item) => item instanceof HTMLSelectElement);
    selects.forEach((select) => {
        const previous = String(select.value || "").trim();
        const includeAll = select === paymentCorrectionBuildingSelectEl;
        select.replaceChildren();
        if (includeAll) {
            const option = document.createElement("option");
            option.value = "all";
            option.textContent = "All buildings";
        ...
        const fallback = includeAll ? "all" : state.selectedRegistryBuildingId || state.buildings[0]?.id || "";
        select.value = previous && [...select.options].some((option) => option.value === previous)
            ? previous
            : fallback;
    });
}
async function loadPaymentCorrections() {
    const buildingId = getSelectedPaymentCorrectionBuildingId();
    const houseNumber = normalizeHouse(paymentCorrectionHouseEl?.value);
    const utilityType = String(paymentCorrectionUtilityTypeEl?.value || "").trim();
    ...
    const rentPayload = await requestJson(`/api/landlord/rent-payments?${params.toString()}`);
    state.rentPayments = Array.isArray(rentPayload.data) ? rentPayload.data : [];
    const utilityParams = new URLSearchParams(params);
    if (utilityType) {
        utilityParams.set("utilityType", utilityType);
    }
    const utilityPayload = await requestJson(`/api/landlord/utilities/payments?${utilityParams.toString()}`);
    state.payments = Array.isArray(utilityPayload.data) ? utilityPayload.data : [];
    renderUtilityPayments(state.payments);
    renderPaymentCorrections();
    renderKeyedPrices();
    renderMetrics();
}
...
function formatPromptDateTime(value) {
    ...
    const parsed = new Date(raw);
    return Number.isNaN(parsed.getTime()) ? raw : toDateTimeLocalInputValue(parsed);
}
function promptForLandlordPaymentEdit(button) {
    const currentAmount = Number(button?.dataset?.amountKsh ?? 0);
    const currentProvider = normalizePaymentProvider(button?.dataset?.provider || "cash");
    ...
    const providerRaw = window.prompt("Provider (cash, bank, or card)", currentProvider || "cash");
    if (providerRaw === null) {
        return null;
    }
    ...
    const referenceRaw = window.prompt("Reference (optional for cash, required for bank/card)", currentReference);
    ...
    const paidAtRaw = window.prompt("Paid at (YYYY-MM-DDTHH:MM or full ISO time)", formatPromptDateTime(currentPaidAt));
    if (paidAtRaw === null) {
        return null;
    }
    ...
    const billingMonthRaw = window.prompt("Billing month (YYYY-MM). Leave blank to keep current coverage month.", currentBillingMonth);
    ...
    const payload = {
        buildingId: String(button?.dataset?.buildingId ?? "").trim(),
        amountKsh,
        provider,
    ...
    if (String(button?.dataset?.action ?? "") === "edit-utility-payment") {
...
async function handleCorrectionPaymentAction(button) {
    const action = String(button?.dataset?.action ?? "").trim();
    const paymentId = String(button?.dataset?.paymentId ?? "").trim();
    const buildingId = String(button?.dataset?.buildingId ?? "").trim();
    const houseNumber = normalizeHouse(button?.dataset?.houseNumber);
    const utilityType = String(button?.dataset?.utilityType ?? "").trim();
    if (!paymentId || !buildingId || !houseNumber) {
        showError("Payment details are missing. Refresh and try again.");
        return;
    }
    const isRent = action.includes("rent");
    const isEdit = action.startsWith("edit-");
    const isUnrecord = action.startsWith("unrecord-");
    if (!isEdit && !isUnrecord) {
        return;
    }
    if (!isRent && !utilityType) {
    ...
    try {
        if (isEdit) {
            const payload = promptForLandlordPaymentEdit(button);
            if (!payload) {
                return;
            }
            const url = isRent
                ? `/api/landlord/rent/${encodeURIComponent(houseNumber)}/payments/${encodeURIComponent(paymentId)}`
                : `/api/landlord/utilities/${encodeURIComponent(utilityType)}/${encodeURIComponent(houseNumber)}/payments/${encodeURIComponent(paymentId)}`;
            ...
            const amount = formatCurrency(button?.dataset?.amountKsh);
            const label = isRent ? "rent" : `${utilityType} utility`;
            const confirmed = window.confirm(`Unrecord ${amount} ${label} payment for ${houseNumber}? The room balance will reopen.`);
            if (!confirmed) {
                return;
            }
            const url = isRent
                ? `/api/landlord/rent/${encodeURIComponent(houseNumber)}/payments/${encodeURIComponent(paymentId)}/unrecord`
                : `/api/landlord/utilities/${encodeURIComponent(utilityType)}/${encodeURIComponent(houseNumber)}/payments/${encodeURIComponent(paymentId)}/unrecord`;
            await requestJson(url, {
                method: "POST",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({ buildingId })
            });
            setStatus("Payment unrecorded.");
        }
        await Promise.all([loadPaymentCorrections(), loadRentStatus(), loadBills(), loadResidents()]);
    }
    catch (error) {
        handleLandlordError(error, isEdit ? "Failed to edit payment." : "Failed to unrecord payment.");
    }
    finally {
        button.disabled = false;
    }
}
function renderKeyedPrices() {
    if (!(keyedPricesBodyEl instanceof HTMLElement)) {
        return;
    }
    keyedPricesBodyEl.replaceChildren();
    const selectedBuildingId = getSelectedPaymentCorrectionBuildingId();
    const selectedHouse = normalizeHouse(paymentCorrectionHouseEl?.value);
    const rowsByKey = new Map();
    (Array.isArray(state.rentStatus) ? state.rentStatus : []).forEach((item) => {
        if (selectedBuildingId && normalizeLookupBuildingId(item.buildingId) !== selectedBuildingId) {
            return;
        }
        if (selectedHouse && normalizeHouse(item.houseNumber) !== selectedHouse) {
            return;
        }
        const key = buildingHouseLookupKey(item.buildingId, item.houseNumber);
        if (key) {
            rowsByKey.set(key, { buildingId: item.buildingId, houseNumber: item.houseNumber, rent: item });
        }
    });
    (Array.isArray(state.registryRows) ? state.registryRows : []).forEach((item) => {
        const buildingId = state.selectedRegistryBuildingId || selectedBuildingId || "";
        if (selectedBuildingId && normalizeLookupBuildingId(buildingId) !== selectedBuildingId) {
            return;
        }
        if (selectedHouse && normalizeHouse(item.houseNumber) !== selectedHouse) {
            return;
        }
        const key = buildingHouseLookupKey(buildingId, item.houseNumber);
        if (!key) {
            return;
        }
        const existing = rowsByKey.get(key) ?? { buildingId, houseNumber: item.houseNumber };
        existing.registry = item;
        rowsByKey.set(key, existing);
    });
    const rows = [...rowsByKey.values()].sort(compareStableRoomOrder);
    if (rows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = '<td colspan="9">No keyed prices found for the current filters.</td>';
        keyedPricesBodyEl.append(row);
        return;
    }
    rows.forEach((item) => {
        ...
        const row = document.createElement("tr");
        const dueDay = registry.configuredPaymentDueDay ?? registry.roomDefaultRentDueDay ?? rent.paymentDueDay ?? "-";
        row.innerHTML = `
      <td>${escapeHtml(getBuildingDisplayNameById(item.buildingId, item.buildingId || "-"))}</td>
      <td>${escapeHtml(item.houseNumber ?? "-")}</td>
      <td>${escapeHtml(formatCurrency(rent.monthlyRentKsh ?? registry.configuredMonthlyRentKsh ?? registry.monthlyRentKsh ?? 0))}</td>
      <td>${registry.depositKsh == null ? "-" : escapeHtml(formatCurrency(registry.depositKsh))}</td>
      <td>${escapeHtml(dueDay)}</td>
      <td>${escapeHtml(formatCurrency(registry.waterFixedChargeKsh ?? 0))}</td>
      <td>${escapeHtml(formatCurrency(registry.electricityFixedChargeKsh ?? 0))}</td>
      <td>${escapeHtml(formatCurrency(registry.combinedUtilityChargeKsh ?? 0))}</td>
      <td>${escapeHtml(registry.rentSetupSource ?? rent.rentSetupSource ?? "current setup")}</td>
    `;
        keyedPricesBodyEl.append(row);
    });
}
async function submitBillingHoldCorrection(event) {
    event.preventDefault();
    if (isCaretakerRole()) {
        showError("House manager accounts cannot pause room billing.");
        return;
    }
    const buildingId = String(billingHoldBuildingSelectEl?.value || "").trim();
    const houseNumber = normalizeHouse(billingHoldHouseEl?.value);
    const scope = String(billingHoldScopeEl?.value || "all").trim();
    const utilityType = String(billingHoldUtilityTypeEl?.value || "").trim();
    const startMonth = toBillingMonth(billingHoldStartMonthEl?.value);
    const endMonth = toBillingMonth(billingHoldEndMonthEl?.value);
    const reason = String(billingHoldReasonEl?.value || "").trim() || "Backdated billing pause correction";
    if (!buildingId || !houseNumber || !startMonth || !endMonth) {
        showError("Billing pause requires building, house, start month, and end month.");
        return;
    }
    if (endMonth < startMonth) {
        showError("End month must be the same as or after the start month.");
        return;
    }
    if (utilityType && scope !== "utilities") {
        showError("Choose utilities only before selecting a specific utility.");
        return;
    }
    const submitButton = billingHoldCorrectionFormEl?.querySelector("button[type='submit']");
    if (submitButton instanceof HTMLButtonElement) {
        submitButton.disabled = true;
    }
    clearError();
    try {
        const payload = { scope, startMonth, endMonth, reason };
        if (utilityType) {
            payload.utilityType = utilityType;
        }
        await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/rooms/${encodeURIComponent(houseNumber)}/billing-holds`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify(payload)
        });
        billingHoldCorrectionFormEl?.reset();
        syncPaymentCorrectionBuildingOptions();
        if (billingHoldHelpEl instanceof HTMLElement) {
            billingHoldHelpEl.textContent = `Billing pause saved for ${houseNumber} from ${startMonth} to ${endMonth}.`;
        }
        setStatus("Billing pause saved.");
        await Promise.all([loadRentStatus(), loadBills(), loadResidents()]);
    }
    catch (error) {
        handleLandlordError(error, "Failed to save billing pause.");
    }
    finally {
        if (submitButton instanceof HTMLButtonElement) {
            submitButton.disabled = false;
...
function renderUtilityPayments(rows) {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
        const row = document.createElement("tr");
        row.innerHTML = '<td colspan="9">No utility payments found.</td>';
    ...
    rows.forEach((item) => {
      ...
      <td>${item.houseNumber}</td>
      <td>${formatBillingMonth(item.billingMonth)}</td>
      <td>${formatDateTime(item.createdAt || item.paidAt)}</td>
      <td>${item.provider}</td>
...
function renderUtilityRoomSummaryActions(item, accountBuildingId) {
    const houseNumber = normalizeHouse(item?.houseNumber);
    const resident = findResidentDirectoryEntry(accountBuildingId, houseNumber) ??
...
function renderMetrics() {
    ...
    const actionableBills = getActionableUtilityBills(state.bills).filter((item) => !metricBuildingId || normalizeLookupBuildingId(item.buildingId) === metricBuildingId);
    const meters = state.meters.filter((item) => !metricBuildingId || normalizeLookupBuildingId(item.buildingId) === metricBuildingId).length;
    const residentRows = dedupeResidentDirectoryRows(state.residentDirectory).filter((item) => !metricBuildingId || normalizeLookupBuildingId(item.buildingId) === metricBuildingId);
    ...
    const utilityCollectedThisMonth = (Array.isArray(state.payments) ? state.payments : [])
        .filter((item) => (!metricBuildingId || normalizeLookupBuildingId(item.buildingId) === metricBuildingId) &&
    ...
    const outstanding = residentRows.length > 0
        ? residentRows.reduce((sum, item) => sum + getResidentOutstandingBalanceKsh(item), 0)
...
function createRentPaymentPayload() {
    const buildingId = String(rentPaymentBuildingSelectEl?.value || state.selectedRentPaymentBuildingId || "").trim();
    return {
        buildingId,
...
async function loadBuildings() {
    ...
    state.residentUsersCount = state.buildings.reduce((sum, item) => sum + Number(item.residentUsers ?? 0), 0);
    renderBuildings(state.buildings);
...
async function loadApplications() {
    ...
    const payload = await requestJson(`/api/landlord/tenant-applications?status=${encodeURIComponent(status)}`);
...
async function refreshPendingApplicationsIndicator() {
    ...
    if (previousCount > 0 && pendingRows.length > previousCount) {
        ...
        setStatus(`${newItems} new tenant application${newItems === 1 ? "" : "s"} waiting for review.`);
    }
...
async function loadResidents() {
    ...
    const selection = String(residentsBuildingSelectEl.value || state.selectedResidentsBuildingId || "").trim();
    ...
    const buildingIds = state.selectedResidentsBuildingId === "all"
    ...
    const payload = await requestJson(`/api/landlord/resident-directory${query.size > 0 ? `?${query.toString()}` : ""}`);
...
async function loadCaretakers() {
    ...
    const payload = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/caretakers`);
...
async function loadCaretakerAccessRequests() {
    ...
    const payload = await requestJson(`/api/landlord/caretaker-access-requests?status=pending&buildingId=${encodeURIComponent(buildingId)}`);
...
async function loadRegistryRows() {
    const buildingId = String(registryBuildingSelectEl.value || state.selectedRegistryBuildingId || "").trim();
    state.selectedRegistryBuildingId = buildingId;
    if (!buildingId) {
        setRegistryRows([]);
    ...
    const payload = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/utility-registry`);
    setRegistryRows(payload.data ?? []);
    setUtilityPricingState(payload.buildingConfiguration ?? null, payload.rateDefaults ?? { buildingId }, buildingId);
...
async function loadMeters() {
    const buildingId = getSelectedUtilityBuildingId();
    const payload = await requestJson(withBuildingQuery("/api/landlord/utilities/meters", buildingId));
...
async function loadBills() {
    ...
    const payload = await requestJson(withBuildingQuery("/api/landlord/utilities/bills", buildingId, "limit=600"));
...
async function loadRegistryReadingBills() {
    ...
    const payload = await requestJson(withBuildingQuery("/api/landlord/utilities/bills", buildingId, new URLSearchParams({
        billingMonth,
        limit: "600"
    }).toString()));
...
async function loadPayments() {
    const buildingId = getSelectedUtilityBuildingId();
    const payload = await requestJson(withBuildingQuery("/api/landlord/utilities/payments", buildingId, "limit=600"));
...
async function loadExpenditures() {
    const buildingId = getSelectedUtilityBuildingId();
    const payload = await requestJson(withBuildingQuery("/api/landlord/expenditures", buildingId));
...
async function loadMoveOutSettlements() {
    const buildingId = getSelectedUtilityBuildingId();
    try {
        const payload = await requestJson(withBuildingQuery("/api/landlord/move-out-settlements", buildingId, "limit=500"));
...
function applyLandlordStartupData(startup) {
    ...
    const hasDeepLinkBuilding = state.buildings.some((item) => item.id === deepLinkBuildingId);
    state.selectedRoomBuildingId = String((hasDeepLinkBuilding ? deepLinkBuildingId : selection.roomBuildingId) ||
        state.buildings[0]?.id ||
        "").trim();
    state.selectedRegistryBuildingId = String((hasDeepLinkBuilding ? deepLinkBuildingId : selection.registryBuildingId) ||
        state.buildings[0]?.id ||
        "").trim();
    state.selectedCaretakerBuildingId = String(selection.caretakerBuildingId || state.selectedRegistryBuildingId || "").trim();
    ...
    state.selectedRentPaymentBuildingId = String((hasDeepLinkBuilding ? deepLinkBuildingId : selection.rentPaymentBuildingId) ||
        state.selectedRegistryBuildingId ||
        "").trim();
    state.selectedRentSheetBuildingId = String((hasDeepLinkBuilding ? deepLinkBuildingId : selection.rentSheetBuildingId) ||
        state.selectedRegistryBuildingId ||
        "").trim();
    state.selectedMessageBuildingId = String(selection.messageBuildingId || state.selectedRegistryBuildingId || state.buildings[0]?.id || "").trim();
    state.residentUsersCount = state.buildings.reduce((sum, item) => sum + Number(item.residentUsers ?? 0), 0);
    ...
    setUtilityPricingState(startup?.utilityBuildingConfiguration ?? null, startup?.utilityRateDefaults ?? null, state.selectedRegistryBuildingId);
    state.utilitySheetMonthlyCombinedCharge = null;
    ...
    state.ownerNotifications = Array.isArray(startup?.ownerNotifications?.notifications)
        ...
        : [];
    state.ownerNotificationsUnreadCount = Number.isFinite(Number(startup?.ownerNotifications?.unreadCount))
    ...
    setResidentDirectory(dedupeResidentDirectoryRows(Array.isArray(startup?.residentDirectory) ? startup.residentDirectory : []));
...
async function loadDataLegacy() {
    ...
    try {
        ...
        setStatus("Management data load failed.");
    }
}
async function hydrateDeferredLandlordData() {
    ...
    landlordDeferredHydrationPromise = Promise.allSettled(loaders.map((loader) => loader()))
        ...
        if (unauthorized?.status === "rejected") {
            handleLandlordError(unauthorized.reason, "Management session is not available.");
            return;
        }
...
landlordNavButtons.forEach((button) => {
    ...
    button.addEventListener("click", () => {
        const targetView = button.dataset.landlordView || button.dataset.landlordFocusTargetView;
        setActiveLandlordView(targetView);
        const sectionTarget = String(button.dataset.landlordSectionTarget ||
...
dashboardActionButtons.forEach((button) => {
    ...
    button.addEventListener("click", () => {
        const action = String(button.dataset.dashboardAction || "").trim();
        const focusedBuildingId = getFocusedBuildingId();
        switch (action) {
            case "rooms":
                setActiveLandlordView("tenants");
...
buildingExistingRoomsEl?.addEventListener("click", (event) => {
...
openDirectTenantDrawerBtnEl?.addEventListener("click", () => {
    openDirectTenantDrawer();
});
closeDirectTenantDrawerBtnEl?.addEventListener("click", () => {
...
residentsOpenRentSheetBtnEl?.addEventListener("click", () => {
    void openRentSheetModal();
});
...
directTenantFormEl?.addEventListener("submit", (event) => {
    ...
    const billingStartInput = window.prompt(`Billing start date for ${buildingLabel} ${houseNumber} (YYYY-MM-DD). Billing stays paused until this date if it falls in a future month.`, defaultBillingStartDate);
    ...
    void (async () => {
        try {
            ...
            closeDirectTenantDrawer();
            setStatus(`${residentName} added to ${buildingName} ${data.houseNumber || houseNumber}. They can sign in with their phone number and ID number as the temporary password.${billingPauseText}${smsText}`);
            await Promise.all([
                loadBuildings(),
                loadApplications(),
                loadRentStatus(),
...
ownerStaffFormEl?.addEventListener("submit", (event) => {
    ...
    void (async () => {
        try {
            ...
            renderOwnerStaff();
            setStatus("Staff account added.");
        }
        catch (error) {
            handleLandlordError(error, "Failed to add staff account.");
        }
        finally {
            renderOwnerStaff();
        }
...
ownerStaffBodyEl?.addEventListener("click", (event) => {
    ...
    const shouldProceed = window.confirm("Disable this staff account?");
    if (!shouldProceed) {
        return;
    }
    button.disabled = true;
    ...
    void (async () => {
        try {
            const payload = await requestJson(`/api/landlord/staff/${encodeURIComponent(userId)}`, {
                method: "DELETE",
                headers: {
                    "content-type": "application/json"
                },
            ...
            setOwnerStaffData(payload.data?.ownerStaff ?? payload.data);
            renderOwnerStaff();
            setStatus("Staff account disabled.");
        }
        catch (error) {
            handleLandlordError(error, "Failed to disable staff account.");
        }
        finally {
            button.disabled = false;
        }
...
caretakerFormEl?.addEventListener("submit", (event) => {
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/caretakers`, {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
...
caretakersBodyEl?.addEventListener("click", (event) => {
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/caretakers/${encodeURIComponent(userId)}`, {
                method: "DELETE"
            });
...
caretakerRequestsBodyEl?.addEventListener("click", (event) => {
    ...
    const shouldProceed = window.confirm(approved
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/caretaker-access-requests/${encodeURIComponent(requestId)}`, {
                method: "PATCH",
                headers: {
                    "content-type": "application/json"
                },
...
paymentAccessBodyEl.addEventListener("click", (event) => {
    ...
    const electricityInput = row.querySelector('input[data-setting="electricityEnabled"]');
    ...
    const latePenaltyAmountInput = row.querySelector('input[data-setting="lateRentPenaltyAmountKsh"]');
    if (!(rentInput instanceof HTMLInputElement) ||
        !(waterInput instanceof HTMLInputElement) ||
        !(electricityInput instanceof HTMLInputElement) ||
    ...
    const lateRentPenaltyAmountKsh = Math.max(0, Math.round(Number(latePenaltyAmountInput.value || 0)));
    const nextValue = {
    ...
    if (nextValue.rentEnabled !== Boolean(current.rentEnabled)) {
        changes.push(`Rent payments will be ${nextValue.rentEnabled ? "enabled" : "disabled"}`);
    }
    ...
    if (nextValue.lateRentPenaltyEnabled !== Boolean(current.lateRentPenaltyEnabled)) {
        changes.push(`Fixed late fee will be ${nextValue.lateRentPenaltyEnabled ? "enabled" : "disabled"}`);
    }
    if (nextValue.lateRentPenaltyAmountKsh !==
    ...
    if (nextValue.waterEnabled !== Boolean(current.waterEnabled)) {
        changes.push(`Water payments will be ${nextValue.waterEnabled ? "enabled" : "disabled"}`);
    }
    if (nextValue.electricityEnabled !== Boolean(current.electricityEnabled)) {
        changes.push(`Electricity payments will be ${nextValue.electricityEnabled ? "enabled" : "disabled"}`);
    }
    if (changes.length === 0) {
    ...
    const noteRaw = window.prompt("Optional note for this change (visible in audit details). Leave blank to skip.");
    const note = noteRaw == null || String(noteRaw).trim().length === 0
        ? undefined
        : String(noteRaw).trim();
    target.disabled = true;
    clearError();
    void (async () => {
        try {
            await requestJson(`/api/landlord/payment-access-controls/${encodeURIComponent(buildingId)}`, {
                method: "PATCH",
                headers: {
                    "content-type": "application/json"
                },
...
paymentProfilesBodyEl?.addEventListener("click", (event) => {
    ...
    const assignment = state.buildingPaymentProfileByBuildingId.get(normalizeLookupBuildingId(buildingId));
    ...
    if (!profile.isConfigured) {
        ...
        const confirmation = window.confirm(`${profile.name} is not fully configured (${missing}). Save this routing anyway? Residents will not be able to initialize STK until backend env is updated.`);
        if (!confirmation) {
            return;
        }
    }
    ...
    const noteRaw = window.prompt("Optional note for this routing change. Leave blank to skip.");
    const note = noteRaw == null || String(noteRaw).trim().length === 0
        ? undefined
        : String(noteRaw).trim();
    target.disabled = true;
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/payment-profiles/${encodeURIComponent(buildingId)}`, {
                method: "PATCH",
                headers: {
                    "content-type": "application/json"
                },
...
paymentInstructionsBodyEl?.addEventListener("click", (event) => {
    ...
    const current = state.buildingPaymentInstructionByBuildingId.get(normalizeLookupBuildingId(buildingId));
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/payment-instructions/${encodeURIComponent(buildingId)}`, {
                method: "PATCH",
                headers: {
                    "content-type": "application/json"
                },
...
buildingFormEl.addEventListener("submit", (event) => {
    ...
    void (async () => {
        try {
            const payload = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/houses`, {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
...
buildingsBodyEl.addEventListener("click", (event) => {
    ...
    if (target.dataset.action === "delete-building") {
        const buildingName = String(target.dataset.buildingName || getBuildingDisplayNameById(buildingId)).trim();
...
function handleRemoveRoomClick(target, buildingId, houseNumber) {
    ...
    const shouldProceed = window.confirm(`Remove room ${houseNumber} from building ${buildingId}?\nThis cannot be undone if the room has no tenancy history.`);
    if (!shouldProceed) {
        return;
    }
    target.disabled = true;
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/houses/${encodeURIComponent(houseNumber)}/remove`, {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
...
function handleWriteOffRoomBalanceClick(target, buildingId, houseNumber, amountKsh) {
    ...
    const shouldProceed = window.confirm(`Clear the open balance for room ${houseNumber} in ${buildingId}?\n${amountLabel} will be recorded as landlord loss and the room will stay available.`);
    if (!shouldProceed) {
        return;
    }
    target.disabled = true;
    ...
    void (async () => {
        try {
            const response = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/houses/${encodeURIComponent(houseNumber)}/write-off-balances`, {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
            ...
            setStatus(`Cleared room ${houseNumber}. ${formatCurrency(settled)} recorded as landlord loss.`);
            await Promise.all([
                loadBills(),
                loadPayments(),
                loadRentStatus(),
...
function handleRemoveResidentClick(target, buildingId, userId, houseNumber, residentName) {
    if (!userId) {
...
async function submitMoveOutSettlement(event) {
    ...
    const residentName = String(moveOutSettlementFormEl?.dataset.residentName ?? "").trim() || "Resident";
    ...
    if (action === "collect_before_move_out" && total > 0) {
        closeMoveOutSettlementModal();
        setStatus(`${residentName} remains active in house ${houseNumber}. Collect ${formatCurrency(total)} before clearing the resident.`);
        return;
    }
    setMoveOutSettlementLoading(true);
    clearError();
    try {
        const response = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/users/${encodeURIComponent(userId)}/remove`, {
            method: "POST",
            headers: {
                "content-type": "application/json"
            },
        ...
        if (action === "transfer_to_resident_debt" && settled > 0) {
            setStatus(`Removed ${residentName} from house ${houseNumber}. ${formatCurrency(settled)} transferred to resident debt.`);
        }
        else if (action === "write_off" && settled > 0) {
            setStatus(`Removed ${residentName} from house ${houseNumber}. ${formatCurrency(settled)} written off.`);
        }
        else if (refundDue > 0) {
            setStatus(`Removed ${residentName} from house ${houseNumber}. ${formatCurrency(refundDue)} deposit refund recorded as due.`);
        }
        else if (depositApplied > 0) {
            setStatus(`Removed ${residentName} from house ${houseNumber}. ${formatCurrency(depositApplied)} deposit applied to close the room balance.`);
        }
        else {
...
registryBodyEl.addEventListener("click", (event) => {
    ...
    if (action === "remove-resident") {
        ...
        handleRemoveResidentClick(target, buildingId, userId, houseNumber, residentName);
        return;
    }
    ...
    if (action === "write-off-room-balance") {
        ...
        handleWriteOffRoomBalanceClick(target, buildingId, houseNumber, Number(target.dataset.amountKsh ?? 0));
    }
});
residentsBodyEl?.addEventListener("click", (event) => {
    ...
    if (action === "open-direct-tenant-drawer") {
        openDirectTenantDrawer({ buildingId, houseNumber });
        return;
    }
...
roomLedgerBodyEl?.addEventListener("click", (event) => {
    ...
    if (button instanceof HTMLButtonElement) {
        ...
        if (action === "write-off-room-balance") {
            handleWriteOffRoomBalanceClick(button, buildingId, houseNumber, Number(button.dataset.amountKsh ?? 0));
        }
        return;
    }
...
applicationsBodyEl.addEventListener("click", (event) => {
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/tenant-applications/${encodeURIComponent(applicationId)}`, {
                method: "PATCH",
                headers: {
                    "content-type": "application/json"
                },
...
utilityMeterFormEl.addEventListener("submit", (event) => {
    ...
    void (async () => {
        try {
            await requestJson(withBuildingQuery(`/api/landlord/utilities/${encodeURIComponent(utilityType)}/${encodeURIComponent(houseNumber)}/meter`, buildingId), {
                method: "PUT",
                headers: {
                    "content-type": "application/json"
                },
...
registrySaveBtnEl.addEventListener("click", () => {
    ...
    void (async () => {
        try {
            await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/utility-registry`, {
                method: "PUT",
                headers: {
                    "content-type": "application/json"
                },
...
utilitySheetFormEl?.addEventListener("submit", (event) => {
    ...
    const buildingDefaultWaterFixedChargeKsh = toOptionalNumber(utilitySheetWaterFixedDefaultEl?.value);
    const buildingDefaultElectricityFixedChargeKsh = toOptionalNumber(utilitySheetElectricFixedDefaultEl?.value);
    const buildingDefaultCombinedUtilityChargeKsh = toOptionalNumber(utilitySheetBuildingCombinedChargeEl?.value);
    const normalizedBuildingDefaultWaterFixedChargeKsh = buildingDefaultWaterFixedChargeKsh == null
        ? null
        : Math.max(0, buildingDefaultWaterFixedChargeKsh);
    const normalizedBuildingDefaultElectricityFixedChargeKsh = buildingDefaultElectricityFixedChargeKsh == null
        ? null
        : Math.max(0, buildingDefaultElectricityFixedChargeKsh);
    const normalizedBuildingDefaultCombinedUtilityChargeKsh = buildingDefaultCombinedUtilityChargeKsh == null
        ? null
        : Math.max(0, Math.round(buildingDefaultCombinedUtilityChargeKsh));
    const normalizedWaterRatePerUnitKsh = toOptionalNumber(utilitySheetWaterRateEl?.value) == null
        ? null
        : Math.max(0, Number(toOptionalNumber(utilitySheetWaterRateEl?.value)));
    const normalizedElectricityRatePerUnitKsh = toOptionalNumber(utilitySheetElectricRateEl?.value) == null
        ? null
        : Math.max(0, Number(toOptionalNumber(utilitySheetElectricRateEl?.value)));
    const currentBuildingDefaultWaterFixedChargeKsh = state.utilitySheetBuildingConfiguration?.defaultWaterFixedChargeKsh == null
        ? null
        : Math.max(0, Number(state.utilitySheetBuildingConfiguration.defaultWaterFixedChargeKsh));
    const currentBuildingDefaultElectricityFixedChargeKsh = state.utilitySheetBuildingConfiguration?.defaultElectricityFixedChargeKsh == null
        ? null
        : Math.max(0, Number(state.utilitySheetBuildingConfiguration.defaultElectricityFixedChargeKsh));
    const currentWaterRatePerUnitKsh = state.utilitySheetBuildingConfiguration?.defaultWaterRatePerUnitKsh == null
        ? null
        : Math.max(0, Number(state.utilitySheetBuildingConfiguration.defaultWaterRatePerUnitKsh));
    const currentElectricityRatePerUnitKsh = state.utilitySheetBuildingConfiguration?.defaultElectricityRatePerUnitKsh == null
        ? null
        : Math.max(0, Number(state.utilitySheetBuildingConfiguration.defaultElectricityRatePerUnitKsh));
    const currentBuildingDefaultCombinedUtilityChargeKsh = state.utilitySheetBuildingConfiguration?.defaultCombinedUtilityChargeKsh == null
        ? null
        : Math.max(0, Math.round(state.utilitySheetBuildingConfiguration.defaultCombinedUtilityChargeKsh));
    const normalizedMonthlyCombinedUtilityChargeKsh = combinedUtilityChargeKsh == null ? null : Math.max(0, Math.round(combinedUtilityChargeKsh));
    const rateDefaults = {
    ...
    try {
        ...
        billRequests = buildUtilitySheetBillRequests(buildingId, billingMonth, dueDate, bulkNote, combinedUtilityChargeKsh);
    }
    catch (error) {
    ...
    void (async () => {
        try {
            const auditPayload = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/utility-bulk-audits`, {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
                body: JSON.stringify({
                    ...
                    dueDate,
                    note: bulkNote,
                    defaultWaterFixedChargeKsh: normalizedBuildingDefaultWaterFixedChargeKsh,
                    defaultElectricityFixedChargeKsh: normalizedBuildingDefaultElectricityFixedChargeKsh,
                    defaultCombinedUtilityChargeKsh: normalizedBuildingDefaultCombinedUtilityChargeKsh,
            ...
            try {
                downloadUtilityBulkAuditCsv(auditPayload?.data ?? {
                    id: auditId,
            ...
            if (!utilityPricingNumbersEqual(normalizedWaterRatePerUnitKsh, currentWaterRatePerUnitKsh) ||
                !utilityPricingNumbersEqual(normalizedElectricityRatePerUnitKsh, currentElectricityRatePerUnitKsh) ||
                !utilityPricingNumbersEqual(normalizedBuildingDefaultWaterFixedChargeKsh, currentBuildingDefaultWaterFixedChargeKsh) ||
                !utilityPricingNumbersEqual(normalizedBuildingDefaultElectricityFixedChargeKsh, currentBuildingDefaultElectricityFixedChargeKsh) ||
                normalizedBuildingDefaultCombinedUtilityChargeKsh !==
                    currentBuildingDefaultCombinedUtilityChargeKsh) {
                const configurationPayload = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/configuration`, {
                    method: "PATCH",
                    headers: {
                        "content-type": "application/json"
                    },
                    body: JSON.stringify({
                        ...
                        defaultElectricityRatePerUnitKsh: normalizedElectricityRatePerUnitKsh,
                        defaultWaterFixedChargeKsh: normalizedBuildingDefaultWaterFixedChargeKsh,
                        defaultElectricityFixedChargeKsh: normalizedBuildingDefaultElectricityFixedChargeKsh,
                        defaultCombinedUtilityChargeKsh: normalizedBuildingDefaultCombinedUtilityChargeKsh,
                        acknowledgeImpact: true
            ...
            if (combinedUtilityChargeKsh != null && combinedUtilityChargeKsh > 0) {
                await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/monthly-combined-utility-charge`, {
                    method: "PUT",
                    headers: {
                        "content-type": "application/json"
                    },
            ...
            for (const billRequest of billRequests) {
                try {
                    await requestJson(withBuildingQuery(`/api/landlord/utilities/${encodeURIComponent(billRequest.utilityType)}/${encodeURIComponent(billRequest.houseNumber)}/bills`, buildingId), {
                        method: "POST",
                        headers: {
                            "content-type": "application/json"
                        },
                    ...
                    failures.push(`${billRequest.utilityType} ${billRequest.houseNumber}: ${error instanceof Error ? error.message : "failed"}`);
                }
            }
            await Promise.all([
                loadRegistryRows(),
            ...
            if (failures.length > 0) {
                ...
                showError(`Saved meter updates. Posted ${postedCount}/${billRequests.length} bills. Failed: ${preview}${failures.length > 3 ? " ..." : ""}`);
                setStatus(`Bulk save completed for ${buildingId} with ${failures.length} bill error(s).`);
            }
            else {
                setStatus(`Saved bulk utility entry for ${buildingId}. Posted ${postedCount} bill(s).`);
                closeUtilitySheetModal();
            }
        }
        catch (error) {
...
utilityBillFormEl.addEventListener("submit", (event) => {
    ...
    if (!utility.buildingId ||
        !utility.houseNumber ||
        !utility.payload.dueDate) {
        showError("Utility bill requires house and due date.");
        return;
    }
    const configuredMeter = findConfiguredMeter(utility.utilityType, utility.buildingId, utility.houseNumber);
    if (configuredMeter) {
        if (utility.payload.currentReading == null ||
            utility.payload.ratePerUnitKsh == null) {
            showError(`House ${utility.houseNumber} has meter ${configuredMeter.meterNumber}. Enter current reading and rate per unit.`);
            return;
        }
    }
    else if (Number(utility.payload.fixedChargeKsh ?? 0) <= 0) {
        showError(`House ${utility.houseNumber} has no ${utility.utilityType} meter. Enter a fixed charge greater than zero.`);
        return;
    }
    ...
    void (async () => {
        try {
            const response = await requestJson(withBuildingQuery(`/api/landlord/utilities/${encodeURIComponent(utility.utilityType)}/${encodeURIComponent(utility.houseNumber)}/bills`, utility.buildingId), {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
            ...
            setStatus(`${utility.utilityType} bill posted for ${utility.houseNumber}${postedBillingMonth ? ` (${postedBillingMonth})` : ""} in ${utility.buildingId}.`);
...
utilityPaymentFormEl?.addEventListener("submit", (event) => {
    ...
    void (async () => {
        try {
            const response = await requestJson(withBuildingQuery(`/api/landlord/utilities/${encodeURIComponent(utility.utilityType)}/${encodeURIComponent(utility.houseNumber)}/payments`, utility.buildingId), {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
            ...
            const coverage = formatUtilityPaymentCoverage(response?.data, utility.payload.billingMonth);
            setStatus(`${utility.utilityType} payment recorded for ${utility.houseNumber}${coverage ? ` covering ${coverage}` : ""}.`);
...
overviewUtilityPaymentFormEl?.addEventListener("submit", (event) => {
    ...
    const statusLabel = String(overviewUtilityPaymentFormEl.dataset.statusLabel ?? "Actionable").trim() ||
        "Actionable";
    ...
    void (async () => {
        try {
            const response = await requestJson(withBuildingQuery(`/api/landlord/utilities/${encodeURIComponent(utilityType)}/${encodeURIComponent(houseNumber)}/payments`, buildingId), {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
            ...
            closeOverviewUtilityPaymentModal();
            setStatus(`${utilityTypeLabel(utilityType)} payment recorded for ${houseNumber}${coverage ? ` covering ${coverage}` : ""}.`);
            await Promise.all([loadBills(), loadPayments(), loadResidents()]);
        }
        catch (error) {
...
rentSheetFormEl?.addEventListener("submit", (event) => {
    ...
    try {
        ...
        setRentSheetStatus(error instanceof Error ? error.message : "Invalid values in rent setup sheet.");
    ...
    setRentSheetStatus("Saving rent setup...");
    void (async () => {
        try {
            const response = await requestJson(`/api/landlord/buildings/${encodeURIComponent(buildingId)}/rent-setup-sheet`, {
                method: "PUT",
                headers: {
                    "content-type": "application/json"
                },
            ...
            setStatus(`Saved rent setup for ${getBuildingDisplayNameById(buildingId, buildingId)}.`);
            setRentSheetStatus("Rent setup saved.");
            closeRentSheetModal();
            focusTenantsSection(buildingId);
        }
        catch (error) {
            setRentSheetStatus(error instanceof Error ? error.message : "Failed to save rent setup sheet.");
...
rentPaymentFormEl?.addEventListener("submit", (event) => {
    ...
    void (async () => {
        try {
            await requestJson(withBuildingQuery(`/api/landlord/rent/${encodeURIComponent(rentPayment.houseNumber)}/payments`, rentPayment.buildingId), {
                method: "POST",
                headers: {
                    "content-type": "application/json"
                },
            ...
            syncRentPaymentBuildingOptions();
            setStatus(`Rent payment posted for ${rentPayment.houseNumber} in ${rentPayment.buildingId}.`);
            await Promise.all([loadRentStatus(), loadResidents()]);
...
refreshOwnerStaffBtnEl?.addEventListener("click", () => {
    void loadOwnerStaff().catch((error) => {
        handleLandlordError(error, "Unable to refresh staff access.");
    });
});
...
residentsOverviewEl?.addEventListener("click", (event) => {
    ...
    renderResidentDirectory(state.residentDirectory);
    scrollToLandlordSection(String(card.dataset.residentTargetSection || getResidentOverviewTargetSection(filter)));
});
residentsOpenMatchBtnEl?.addEventListener("click", () => {
    openResidentSearchMatch();
});
overviewRoomBuildingSelectEl?.addEventListener("change", () => {
    state.selectedOverviewRoomBuildingId = String(overviewRoomBuildingSelectEl.value || "all").trim() || "all";
...
overviewRoomSearchInputEl?.addEventListener("keydown", (event) => {
    ...
    event.preventDefault();
    void openResidentLookup(overviewRoomSearchInputEl.value, state.selectedOverviewRoomBuildingId).catch((error) => {
...
landlordGlobalSearchFormEl?.addEventListener("submit", (event) => {
    event.preventDefault();
    const buildingId = landlordGlobalSearchBuildingEl instanceof HTMLSelectElement
        ? landlordGlobalSearchBuildingEl.value
        : state.selectedOverviewRoomBuildingId;
    void openResidentLookup(landlordGlobalSearchInputEl instanceof HTMLInputElement
        ? landlordGlobalSearchInputEl.value
        : "", buildingId).catch((error) => {
        handleLandlordError(error, "Unable to open room lookup.");
    });
});
...
refreshPaymentCorrectionsBtnEl?.addEventListener("click", () => {
    void loadPaymentCorrections().catch((error) => {
        handleLandlordError(error, "Unable to refresh payment corrections.");
    });
});
paymentCorrectionBuildingSelectEl?.addEventListener("change", () => {
    state.selectedPaymentCorrectionBuildingId = String(paymentCorrectionBuildingSelectEl.value || "all");
    renderPaymentCorrections();
    renderKeyedPrices();
});
paymentCorrectionHouseEl?.addEventListener("input", () => {
    renderPaymentCorrections();
    renderKeyedPrices();
});
paymentCorrectionUtilityTypeEl?.addEventListener("change", () => {
    renderPaymentCorrections();
});
[correctionRentPaymentsBodyEl, correctionUtilityPaymentsBodyEl, utilityPaymentsBodyEl].forEach((bodyEl) => {
    bodyEl?.addEventListener("click", (event) => {
        const target = event.target;
        if (!(target instanceof HTMLElement)) {
            return;
        }
        const button = target.closest("button[data-action$='-payment']");
        if (!(button instanceof HTMLButtonElement)) {
            return;
        }
        void handleCorrectionPaymentAction(button);
    });
});
billingHoldCorrectionFormEl?.addEventListener("submit", (event) => {
    void submitBillingHoldCorrection(event);
});
refreshOverviewDashboardBtnEl?.addEventListener("click", () => {
    void Promise.all([
        ...
        handleLandlordError(error, "Unable to refresh dashboard.");
    });
});
...
moveOutSettlementsBodyEl?.addEventListener("click", (event) => {
    ...
    if (target.dataset.action === "collect-resident-debt") {
        handleCollectResidentDebtClick(target, String(target.dataset.settlementId || "").trim(), String(target.dataset.residentName || "Resident").trim(), Number(target.dataset.amountKsh ?? 0));
        return;
    }
    if (target.dataset.action === "record-deposit-refund") {
        handleRecordDepositRefundClick(target, String(target.dataset.settlementId || "").trim(), String(target.dataset.residentName || "Resident").trim(), Number(target.dataset.amountKsh ?? 0));
    }
});
expendituresBodyEl?.addEventListener("click", (event) => {
    ...
    handleDeleteExpenditureClick(target, String(target.dataset.expenditureId || "").trim(), String(target.dataset.title || "").trim());
});
expenditureFormEl?.addEventListener("submit", (event) => {
    ...
    const chargeableToResident = expenditureChargeableEl instanceof HTMLInputElement
