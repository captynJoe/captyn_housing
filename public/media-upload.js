...
export function validateImageFiles(fileList, { maxFiles = 4, maxSizeMb = 10 } = {}) {
...
export function renderSelectedImagePreviews(container, fileList, { emptyText = DEFAULT_EMPTY_TEXT } = {}) {
    if (!(container instanceof HTMLElement)) {
        return;
    }
    ...
    files.forEach((file, index) => {
        ...
        container.append(createPreviewImage(objectUrl, file.name || `Selected photo ${index + 1}`, file.name || `Photo ${index + 1}`));
    });
...
export function createUploadedImageGallery(urls, { linkLabel = "Open photo" } = {}) {
    ...
    list.forEach((url, index) => {
        ...
        anchor.append(createPreviewImage(url, `${linkLabel} ${index + 1}`, `Photo ${index + 1}`));
...
export async function uploadImageFiles(files, { createUploadRequest, getSignature } = {}) {
    if (!Array.isArray(files) || files.length === 0) {
        return [];
    }
    const buildUploadRequest = typeof createUploadRequest === "function"
        ? createUploadRequest
    ...
    for (const file of files) {
        ...
        if (!response.ok) {
            const uploadError = payload?.issues?.[0]?.message ??
                payload?.error ??
                "Photo upload failed.";
