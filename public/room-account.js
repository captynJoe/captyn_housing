...
const roomRentDueDayDefaultActionEl = document.getElementById("room-rent-due-day-default-action");
...
const roomBalanceConfirmBackdropEl = document.getElementById("room-balance-confirm-backdrop");
const roomBalanceConfirmModalEl = document.getElementById("room-balance-confirm-modal");
const roomBalanceConfirmMessageEl = document.getElementById("room-balance-confirm-message");
const roomBalanceConfirmValuesEl = document.getElementById("room-balance-confirm-values");
const roomBalanceConfirmCancelEl = document.getElementById("room-balance-confirm-cancel");
...
function requestJson(url, options = {}) {
    return fetch(url, {
        ...
        if (!response.ok) {
            ...
            const error = new Error(issueMessage ?? payload.error ?? `Request failed (${response.status})`);
...
function isGenericRouteNotFound(error) {
    return (error?.status === 404 &&
        /request failed \(404\)|api route not found|cannot (post|delete)/i.test(String(error?.message ?? "")));
}
...
function confirmBalanceAdjustment({ currentBalanceKsh, nextBalanceKsh, targetLabel }) {
    const fallbackMessage = `Update current balance for ${targetLabel} from ${formatCurrency(currentBalanceKsh)} to ${formatCurrency(nextBalanceKsh)}? This will immediately update the rent ledger.`;
    if (!(roomBalanceConfirmModalEl instanceof HTMLElement) ||
...
function syncRentDueDayDefaultAction(room) {
    ...
    const roomDefaultDay = toOptionalNumber(room?.roomDefaultRentDueDay ?? room?.roomDefaultDueDay);
    ...
    if (buildingDefaultDay != null) {
        roomRentDueDayDefaultActionEl.textContent = `Building default: ${formatDueDayLabel(buildingDefaultDay)}`;
...
function renderPaymentActionButtons(actionPrefix, payment, extraAttributes = {}) {
    ...
    const deletable = canUnrecordPayments() && paymentId && (provider === "cash" || source === "manual");
...
function describeChargeSource(source, utilityBillingMode) {
    switch (String(source ?? "").trim()) {
        ...
        default:
            return {
                label: String(utilityBillingMode ?? "").trim() === "combined_charge"
...
function getBillingModeLabel(room, payload) {
    ...
    const hasRent = Number(room?.monthlyRentKsh ?? 0) > 0 ||
...
function getNextDueLabel(payload) {
    ...
    const includeRentDue = rentDueDate &&
...
function renderBillingHolds(payload) {
    ...
    roomBillingHoldsEl.innerHTML = sorted
        ...
        <article class="package-card room-billing-hold-card ${active ? "is-active" : "is-canceled"}">
          <div>
            <p class="status-text">${escapeHtml(active ? "Active" : "Ended")} • ${escapeHtml(formatBillingHoldRange(hold))}</p>
          ...
          ${active && canManageBillingHolds()
            ? `<button type="button" class="ghost-btn" data-action="cancel-billing-hold" data-hold-id="${escapeHtml(hold?.id)}" data-hold-label="${escapeHtml(scopeLabel)}">Resume</button>`
            : ""}
        </article>
      `;
    })
...
function renderIdentityDocumentPreview() {
    ...
    renderSelectedImagePreviews(selectedPreview, roomIdentityDocumentEl?.files, {
        emptyText: existingUrls.length > 0
...
function renderManagementForms(payload) {
    ...
    setManagementStatus(hasActiveResident
    ...
    setPillText(roomAgreementStateEl, hasActiveResident ? (canEditAgreement ? "Editable" : "Read only") : "Vacant");
    ...
    if (roomAgreementFormEl instanceof HTMLFormElement) {
        ...
        setFormFieldValue(roomAgreementFormEl, "organizationLocation", agreement.organizationLocation ?? "");
        setFormFieldValue(roomAgreementFormEl, "studentRegistrationNumber", agreement.studentRegistrationNumber ?? "");
        ...
        setFormFieldValue(roomAgreementFormEl, "emergencyContactName", agreement.emergencyContactName ?? "");
        setFormFieldValue(roomAgreementFormEl, "emergencyContactPhone", agreement.emergencyContactPhone ?? "");
        setFormFieldValue(roomAgreementFormEl, "leaseStartDate", toDateInputValue(agreement.leaseStartDate));
        setFormFieldValue(roomAgreementFormEl, "leaseEndDate", toDateInputValue(agreement.leaseEndDate));
    ...
    if (roomRentSetupFormEl instanceof HTMLFormElement) {
        setFormFieldValue(roomRentSetupFormEl, "monthlyRentKsh", optionalNumberToInputString(room.roomDefaultMonthlyRentKsh));
        setFormFieldPlaceholder(roomRentSetupFormEl, "monthlyRentKsh", formatRoomDefaultPlaceholder(room.buildingDefaultMonthlyRentKsh, formatCurrency));
        setFormFieldValue(roomRentSetupFormEl, "balanceKsh", numberToInputString(room.rentBalanceKsh));
        setFormFieldValue(roomRentSetupFormEl, "dueDate", toDateInputValue(room.rentDueDate));
        setFormFieldValue(roomRentSetupFormEl, "paymentDueDay", optionalNumberToInputString(room.roomDefaultRentDueDay ?? room.roomDefaultDueDay));
        setFormFieldPlaceholder(roomRentSetupFormEl, "paymentDueDay", formatRoomDefaultPlaceholder(room.buildingDefaultRentDueDay, formatDueDayLabel));
        setFormFieldValue(roomRentSetupFormEl, "rentGraceDays", optionalNumberToInputString(room.roomDefaultGraceDays));
        setFormFieldPlaceholder(roomRentSetupFormEl, "rentGraceDays", formatRoomDefaultPlaceholder(room.buildingDefaultRentGraceDays ?? buildingConfiguration.rentGraceDays, formatGraceDaysLabel));
        setFormFieldValue(roomRentSetupFormEl, "depositKsh", numberToInputString(agreement.depositKsh ?? room.depositKsh));
        setFormFieldValue(roomRentSetupFormEl, "depositPaidKsh", numberToInputString(agreement.depositPaidKsh));
...
function renderAnomalies(payload) {
    ...
    if (missingMonths.length > 0) {
        anomalies.push(`Recurring billing looks incomplete through ${formatBillingMonth(anomalyData.visibleThroughBillingMonth)}. Missing ${missingMonths.map((item) => formatBillingMonth(item)).join(", ")}. Estimated backfill ${formatCurrency(anomalyData.estimatedRecurringBackfillKsh ?? 0)}.`);
    }
    overappliedBills.forEach((item) => {
        anomalies.push(`${utilityTypeLabel(item.utilityType)} ${formatBillingMonth(item.billingMonth)} has ${formatCurrency(item.paidKsh)} paid against ${formatCurrency(item.amountKsh)}. Review duplicate or misallocated payments.`);
    });
    if (payload?.building?.houseRegistered === false || anomalyData?.overstatedOrphanedRoom) {
        anomalies.push("This room has utility or charge activity but is not in the building house list. Confirm whether it should be archived or registered.");
    }
    if (payload?.building?.visibleToLandlord === false) {
        anomalies.push("This room is not normally visible to the current landlord scope. Access is limited to direct route review.");
    }
    if (String(room?.verificationStatus ?? "").trim() === "pending_review") {
        anomalies.push("Resident verification is still pending review. Resident-facing billing may stay hidden until approval is complete.");
    }
    if (String(chargeSetup?.source ?? "").trim() === "unconfigured") {
        anomalies.push("This room still needs a reliable utility charge rule. Add meters, a room default amount, or a building default before future months post.");
    }
    if (Number(summary.expenseChargesKsh ?? 0) > 0 &&
        Number(summary.displayedOutstandingKsh ?? 0) < Number(summary.projectedOutstandingKsh ?? 0)) {
        anomalies.push(`Room charges are already part of the visible outstanding. Projected outstanding also includes missing recurring utility months.`);
    }
...
function renderRoomAuditEvents(payload) {
    ...
    roomAuditEventsEl.innerHTML = events
        ...
        <article class="package-card room-account-audit-card">
          <p class="status-text">${escapeHtml(formatDateTime(event.createdAt))} • ${escapeHtml(formatAuditActor(event.actor))}</p>
...
function renderRoomIssues(payload) {
    ...
    roomIssuesEl.innerHTML = tickets
        ...
        <article class="package-card room-account-issue-card">
          <p class="status-text">${escapeHtml(ticket.queue || "support")} • ${escapeHtml(ticket.status || "open")} • ${escapeHtml(formatDateTime(ticket.createdAt))}</p>
...
function renderRoomAccount(payload) {
    ...
    if (roomAccountTitleEl instanceof HTMLElement) {
        roomAccountTitleEl.textContent = `${buildingName} • Room ${normalizeHouse(room.houseNumber || state.houseNumber)}`;
    }
...
async function createBillingHold(event) {
    ...
    try {
        await requestJson(`/api/landlord/buildings/${encodeURIComponent(state.buildingId)}/rooms/${encodeURIComponent(state.houseNumber)}/billing-holds`, {
            method: "POST",
            headers: {
...
async function cancelBillingHold(button) {
    ...
    const url = `/api/landlord/buildings/${encodeURIComponent(state.buildingId)}/rooms/${encodeURIComponent(state.houseNumber)}/billing-holds/${encodeURIComponent(holdId)}/cancel`;
    setLoading(true);
    showError("");
...
async function buildAgreementPayloadFromForm(form) {
    ...
    return buildAgreementPayloadFromRecord({
        ...
        studentRegistrationNumber: normalizeOptionalString(getFormFieldValue(form, "studentRegistrationNumber")),
...
async function saveRoomAgreement(event) {
    ...
    try {
        ...
        await requestJson(`/api/landlord/buildings/${encodeURIComponent(state.buildingId)}/houses/${encodeURIComponent(state.houseNumber)}/agreement`, {
            method: "PUT",
            headers: {
                "content-type": "application/json"
            },
...
async function saveRoomRentSetup(event) {
    ...
    const room = getRoom();
    const roomDefaultMonthlyRentKsh = toOptionalNumber(getFormFieldValue(form, "monthlyRentKsh"));
    const fallbackMonthlyRentKsh = roomDefaultMonthlyRentKsh ??
    ...
    const roomDefaultPaymentDueDay = toOptionalNumber(getFormFieldValue(form, "paymentDueDay"));
    const paymentDueDay = roomDefaultPaymentDueDay ??
    ...
    if (requestedBalanceKsh != null && requestedBalanceKsh !== currentBalanceKsh) {
        const targetLabel = String(room.residentName ?? "").trim() ||
    ...
    try {
        const rentSetupResponse = await requestJson(`/api/landlord/buildings/${encodeURIComponent(state.buildingId)}/rent-setup-sheet`, {
            method: "PUT",
            headers: {
                "content-type": "application/json"
            },
            body: JSON.stringify({
                ...
                rows: [
                    {
                        houseNumber: state.houseNumber,
                        monthlyRentKsh: roomDefaultMonthlyRentKsh == null
                            ? null
                            : Math.round(roomDefaultMonthlyRentKsh),
                        paymentDueDay: roomDefaultPaymentDueDay == null ? null : Math.round(roomDefaultPaymentDueDay),
                        graceDays: roomDefaultGraceDays == null ? null : Math.round(roomDefaultGraceDays),
                        active: true,
                        note
                    }
                ]
        ...
        const savedSetupRow = Array.isArray(rentSetupResponse?.data?.rows)
            ? rentSetupResponse.data.rows.find((item) => normalizeHouse(item.houseNumber) === normalizeHouse(state.houseNumber))
            : null;
        const monthlyRentKsh = toOptionalNumber(savedSetupRow?.resolvedMonthlyRentKsh) ?? fallbackMonthlyRentKsh;
        const resolvedPaymentDueDay = toOptionalNumber(savedSetupRow?.resolvedDueDay) ?? paymentDueDay;
        ...
        if (getAgreementState().hasActiveResident) {
            await requestJson(`/api/landlord/buildings/${encodeURIComponent(state.buildingId)}/houses/${encodeURIComponent(state.houseNumber)}/agreement`, {
                ...
                body: JSON.stringify(buildAgreementPayloadFromRecord({
                    monthlyRentKsh,
                    depositKsh,
                    depositPaidKsh,
...
async function loadRoomAccount() {
    ...
    showError("");
    try {
        const payload = await requestJson(`/api/landlord/buildings/${encodeURIComponent(state.buildingId)}/rooms/${encodeURIComponent(state.houseNumber)}/ledger`);
        state.role = payload?.role || state.role;
        state.data = payload?.data ?? null;
        renderRoomAccount(state.data);
        setStatus(`Signed in as ${formatRoleLabel(state.role)}. Room account refreshed ${formatDateTime(new Date().toISOString())}.`);
    }
    catch (error) {
        if (error?.status === 401) {
            redirectToLogin();
        ...
        const message = error instanceof Error ? error.message : "Unable to load the room account.";
        showError(message);
...
async function unrecordPayment(button) {
    ...
    const confirmed = window.confirm(`Unrecord ${amount} ${paymentLabel} payment? The room balance will reopen by that amount.`);
    if (!confirmed) {
        return;
    }
    ...
    const postUrl = action === "unrecord-rent-payment"
        ? `/api/landlord/rent/${roomPath}/payments/${paymentPath}/unrecord`
        : `/api/landlord/utilities/${encodeURIComponent(utilityType)}/${roomPath}/payments/${paymentPath}/unrecord`;
    const deleteUrl = action === "unrecord-rent-payment"
        ? `/api/landlord/rent/${roomPath}/payments/${paymentPath}?${buildingQuery}`
        : `/api/landlord/utilities/${encodeURIComponent(utilityType)}/${roomPath}/payments/${paymentPath}?${buildingQuery}`;
    setLoading(true);
    showError("");
    button.disabled = true;
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : "Unable to unrecord this payment.";
        showError(message);
...
async function editPayment(button) {
    ...
    const roomPath = encodeURIComponent(state.houseNumber);
    const paymentPath = encodeURIComponent(paymentId);
    const url = action === "edit-rent-payment"
        ? `/api/landlord/rent/${roomPath}/payments/${paymentPath}`
        : `/api/landlord/utilities/${encodeURIComponent(utilityType)}/${roomPath}/payments/${paymentPath}`;
    setLoading(true);
    showError("");
    button.disabled = true;
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : "Unable to edit this payment.";
        showError(message);
...
async function init() {
    try {
        ...
        const message = error instanceof Error ? error.message : "Unable to open this room account.";
        showError(message);
...
document.addEventListener("click", (event) => {
    ...
    if (target.closest("#room-balance-confirm-apply") instanceof HTMLElement) {
        event.preventDefault();
        event.stopPropagation();
        closeBalanceConfirmModal(true);
        return;
    }
    if (target.closest("#room-balance-confirm-cancel") instanceof HTMLElement ||
