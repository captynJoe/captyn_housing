...
const profileIdentityDocumentPreviewEl = document.getElementById("profile-identity-document-preview");
...
const profileStudentRegistrationNumberEl = document.getElementById("profile-student-registration-number");
...
const profileEmergencyContactNameEl = document.getElementById("profile-emergency-contact-name");
const profileEmergencyContactPhoneEl = document.getElementById("profile-emergency-contact-phone");
...
async function apiRequest(url, init = {}) {
    ...
    if (!response.ok) {
        ...
        throw new Error(issueMessage || payload.error || `Request failed with status ${response.status}`);
    }
    return payload;
}
...
function renderIdentityNotice(profile = state.profile) {
    ...
    profileIdNoticeEl.textContent = `${overdue ? "ID required now." : "ID required."} Add your ID type and ID number. You may attach an ID photo if management asks for one. ${remainingCopy} ${formatIdentityDeadline(requirement)}`;
...
function renderProfile(profile) {
    ...
    residentSessionSummaryEl.textContent = `House ${session.houseNumber} (${session.phoneMask}) • ${formatVerificationLabel(session)} • Expires ${formatDateTime(session.expiresAt)}`;
    ...
    profileSessionCopyEl.textContent = session.mustChangePassword
        ...
        : isPendingReview(session)
            ...
            : `Signed in for house ${session.houseNumber}. Session expires ${formatDateTime(session.expiresAt)}.`;
...
async function handleSave(event) {
    ...
    try {
        ...
        if ((identityType || identityNumber || willHaveIdentityPhotos) &&
        ...
        const payload = {
            ...
            studentRegistrationNumber: optionalTrimmedValue(profileStudentRegistrationNumberEl.value),
...
profileIdentityDocumentEl?.addEventListener("change", () => {
    try {
        ...
        renderIdentityDocumentPreview();
        showFeedback(error instanceof Error ? error.message : "Invalid ID photo.", "error");
    }
});
