...
const residentPasswordChangePanelEl = document.getElementById("resident-password-change-panel");
const residentPasswordChangeFormEl = document.getElementById("resident-password-change-form");
...
const residentPasswordChangeBtnEl = document.getElementById("resident-password-change-btn");
...
const wifiPlansEl = document.getElementById("wifi-plans");
const wifiCurrentCardEl = document.getElementById("wifi-current-card");
const wifiCurrentPlanNameEl = document.getElementById("wifi-current-plan-name");
const wifiCurrentExpiryEl = document.getElementById("wifi-current-expiry");
const wifiCurrentUsageEl = document.getElementById("wifi-current-usage");
const wifiBuyLinkEl = document.getElementById("wifi-buy-link");
...
const state = {
    ...
    rememberResidentDevice: INITIAL_RESIDENT_STORAGE.token !== ""
...
function renderResidentIdentityNotice() {
    ...
    const needsIdentity = Boolean(state.residentSession) &&
    ...
    residentIdNoticeEl.replaceChildren(lead, document.createTextNode(`Add your ID type and ID number. ${remainingCopy} ${formatIdentityDeadline(requirement)} `), action, document.createTextNode("."));
...
function appendLatestUtilityReadingCards(container, latestReadings = [], meters = []) {
    ...
    ["water", "electricity"].forEach((utilityType) => {
        ...
        appendedCount += 1;
        const meterNumber = normalizeUtilityMeterDisplay(reading?.meterNumber) ||
        ...
        details.textContent = reading
            ? `${meterNumber ? `Meter ${meterNumber} • ` : ""}Reading ${formatReadingValue(reading.previousReading)} -> ${formatReadingValue(reading.currentReading)} for ${reading.billingMonth}.`
            : `${meterNumber ? `Meter ${meterNumber} is on file. ` : ""}Waiting for the first recorded reading.`;
...
function setPaymentAmountValue(inputEl, balance, amount) {
    ...
    const nextAmount = total > 0 ? Math.min(total, requestedAmount || total) : requestedAmount;
...
function listOutstandingUtilityBills(utilityType) {
    const bills = Array.isArray(state.utilityBills) ? state.utilityBills : [];
    return bills
        .filter((item) => item.utilityType === utilityType && Number(item.balanceKsh) > 0)
        .sort((a, b) => a.billingMonth.localeCompare(b.billingMonth));
}
function getTotalOutstandingUtilityBalanceForType(utilityType) {
    return listOutstandingUtilityBills(utilityType).reduce((sum, bill) => sum + toPositiveNumber(bill.balanceKsh), 0);
}
function getSelectedUtilityBillMonth(utilityType) {
    return normalizeBillingMonthInput(state.utilitySelectedBillMonthByType?.[utilityType] ?? "");
}
...
function listSelectableRentBillingMonths() {
    ...
    const estimatedOpenMonthCount = balance > 0 && monthlyRent > 0 ? Math.max(1, Math.ceil(balance / monthlyRent)) : 0;
    const estimatedLastOpenMonth = estimatedOpenMonthCount > 0
        ? shiftBillingMonth(referenceMonth, estimatedOpenMonthCount - 1)
        : referenceMonth;
    const rangeEnd = compareBillingMonths(currentMonth, estimatedLastOpenMonth) > 0
        ? currentMonth
...
function getActiveRentBillingMonth() {
    return (getSelectedRentBillingMonth() ??
...
function syncRentBillingMonthOptions() {
    ...
    const nextSelectedMonth = options.find((billingMonth) => billingMonth === currentSelectedMonth) ??
        billingMonths[0] ??
        fallbackMonth ??
        null;
    ...
    options.forEach((billingMonth, index) => {
        ...
        option.textContent = formatRentBillingMonthOptionLabel(billingMonth, index, options.length);
...
function resolveShortcutUtilityType(preferredType) {
    const explicitType = preferredType === "electricity" || preferredType === "water" ? preferredType : "";
...
function updatePaymentsSummaryCard() {
    ...
    paymentsSummaryActionEl.textContent =
        totalOutstanding > 0
            ? "Pay any amount toward your balance using the instructions below."
...
function updateRentPaymentGuidance() {
    ...
    if (enteredAmount <= 0) {
        ...
        rentPaymentRemainingEl.textContent = `Rent balance open: ${formatCurrency(balance)}. Suggested start today: ${formatCurrency(suggestedStarter)}. Enter any amount to preview what remains.${monthCopy}`;
        return;
    }
    if (enteredAmount >= balance) {
        rentPaymentRemainingEl.textContent = `This payment clears the full rent balance of ${formatCurrency(balance)}.${monthCopy}`;
        return;
    }
    const remaining = computeRemainingBalance(balance, enteredAmount);
    rentPaymentRemainingEl.textContent = `After paying ${formatCurrency(enteredAmount)}, you will still have ${formatCurrency(remaining)} remaining on rent.`;
    if (monthCopy) {
...
function updateUtilityPaymentGuidance() {
    ...
    const bill = findOutstandingUtilityBill(utilityType, getSelectedUtilityBillMonth(utilityType));
    if (!bill || toPositiveNumber(bill.balanceKsh) <= 0) {
        utilityPaymentRemainingEl.textContent = `No ${utilityLabel(utilityType).toLowerCase()} balance is open right now.`;
        return;
    }
    ...
    if (enteredAmount <= 0) {
        ...
        utilityPaymentRemainingEl.textContent =
            billCount > 1
                ? `${utilityLabel(utilityType)} payments start with ${bill.billingMonth} and can cover ${formatCurrency(totalOutstanding)} across ${billCount} open bills. Suggested start today: ${formatCurrency(suggestedStarter)}.`
                : `${utilityLabel(utilityType)} ${bill.billingMonth} is open for ${formatCurrency(balance)}. Suggested start today: ${formatCurrency(suggestedStarter)}. Enter any amount to preview what remains.`;
        return;
    }
    if (enteredAmount >= totalOutstanding) {
        utilityPaymentRemainingEl.textContent =
            billCount > 1
                ? `This payment clears the full ${utilityLabel(utilityType).toLowerCase()} balance of ${formatCurrency(totalOutstanding)} across ${billCount} bills.`
    ...
    if (enteredAmount >= balance) {
        ...
        utilityPaymentRemainingEl.textContent =
            carryForward > 0
                ? `This clears ${bill.billingMonth} and carries ${formatCurrency(carryForward)} into the next open ${utilityLabel(utilityType).toLowerCase()} bill.`
                : `This payment clears the full ${utilityLabel(utilityType).toLowerCase()} balance for ${bill.billingMonth}.`;
        return;
    }
    const remaining = computeRemainingBalance(balance, enteredAmount);
    utilityPaymentRemainingEl.textContent = `After paying ${formatCurrency(enteredAmount)}, you will still have ${formatCurrency(remaining)} remaining on this ${bill.billingMonth} ${utilityLabel(utilityType).toLowerCase()} bill.`;
}
...
function applyResidentPaymentShortcut(shortcut) {
    ...
    if (shortcut === "rent-full") {
        ...
        if (!RESIDENT_ONLINE_RENT_PAYMENT_ENABLED) {
            showFeedback(`Rent balance is ${formatCurrency(rentOutstanding)}. Use the payment instructions and include your house reference.`, "info");
    ...
    const largestUtilityType = electricityOutstanding > waterOutstanding ? "electricity" : "water";
    ...
    if (utilityOutstanding >= rentOutstanding && utilityOutstanding > 0) {
        ...
        applyPaymentAccessUi();
        syncUtilityPaymentFormFromBalances();
        utilityPaymentAmountEl.value = formatAmountValue(computeSuggestedStarterAmount(utilityOutstanding));
        updateUtilityPaymentGuidance();
        focusResidentPaymentSection(utilityPaymentSectionEl, utilityPaymentAmountEl);
        return;
    }
    if (rentOutstanding > 0) {
        if (!RESIDENT_ONLINE_RENT_PAYMENT_ENABLED) {
            showFeedback(`Suggested rent starter: ${formatCurrency(computeSuggestedStarterAmount(rentOutstanding))}. Use the payment instructions and include your house reference.`, "info");
            focusResidentPaymentSection(paymentInstructionsCardEl, null);
            return;
        }
...
function setResidentPayMode(mode, options = {}) {
    ...
    if (residentPaySubmitEl instanceof HTMLButtonElement) {
        ...
        residentPaySubmitEl.disabled = Boolean(state.residentSession && !canResidentAccessBilling()) || (!canPayUtility && !canPayRentOnline && !(isRent && !RESIDENT_ONLINE_RENT_PAYMENT_ENABLED));
    }
}
...
function renderPaymentInstructions() {
    if (!(paymentInstructionsCardEl instanceof HTMLElement) ||
    ...
    if (method === "bank") {
        hasDetail =
            appendPaymentInstructionDetail("Bank Name", effective.bankName || instructions.bankName) ||
                hasDetail;
        hasDetail =
            appendPaymentInstructionDetail("Bank Account Name", effective.bankAccountName || instructions.bankAccountName) || hasDetail;
        hasDetail =
            appendPaymentInstructionDetail("Bank Account Number", effective.bankAccountNumber || instructions.bankAccountNumber) || hasDetail;
        hasDetail =
            appendPaymentInstructionDetail("Bank Branch", effective.bankBranch || instructions.bankBranch) ||
                hasDetail;
        hasDetail =
            appendPaymentInstructionDetail("SWIFT / Bank Code", effective.bankSwiftCode || instructions.bankSwiftCode) || hasDetail;
    }
    else if (method === "cash") {
        hasDetail =
            appendPaymentInstructionDetail("Cash Payment Point", effective.cashLocation || instructions.cashLocation) || hasDetail;
    }
    else if (method === "manual") {
        hasDetail =
            appendPaymentInstructionDetail("Payment Instructions", effective.instructions || instructions.instructions) || hasDetail;
    }
    else {
        hasDetail =
            appendPaymentInstructionDetail("Buy Goods Till Number", effective.mpesaBusinessNumber || instructions.mpesaBusinessNumber) || hasDetail;
        hasDetail =
            appendPaymentInstructionDetail("M-PESA Account Name", effective.mpesaAccountName || instructions.mpesaAccountName) || hasDetail;
        hasDetail =
            appendPaymentInstructionDetail("House / Account Reference", effective.mpesaAccountReference || instructions.mpesaAccountReference) || hasDetail;
    }
    const notes = [
    ...
    if (!hasDetail) {
        appendPaymentInstructionDetail("Status", "Payment details are not set yet. Contact management before sending rent.");
    }
}
...
function getSelectedResidentBuildingName() {
    const selectedBuildingId = authBuildingIdEl instanceof HTMLSelectElement
        ? String(authBuildingIdEl.value || "").trim()
        : "";
    return (state.buildings.find((item) => item.id === selectedBuildingId)?.name ?? "");
}
...
function syncModalBodyState() {
    const hasOpenModal = !mpesaStatusModalEl.classList.contains("hidden") ||
...
function renderMetaGrid(containerEl, rows) {
    containerEl.innerHTML = rows
        ...
        .map((row) => `<div><dt>${escapeHtml(row.label)}</dt><dd>${escapeHtml(String(row.value))}</dd></div>`)
        .join("");
}
function renderMpesaStatusModal() {
    ...
    mpesaStatusCopyEl.textContent =
        flow.copy ??
            `An M-PESA prompt has been sent. Enter your PIN on your phone to finish this ${describeMpesaTarget(flow)}.`;
...
<html lang="en">
  ...
  <body>
    <article class="card">
      <div class="head">
        <div class="brand">
          <img src="data:image/svg+xml;utf8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><circle cx="32" cy="32" r="32" fill="#0AA83A"/><path d="M9 39V26.5h4.2l3.4 6.7 3.4-6.7H24V39h-4v-5.6l-2.5 4.9h-1.9l-2.5-4.9V39z" fill="#fff"/><path d="M37.5 39V26.5h5.1c3.6 0 5.3 1.8 5.3 4.6 0 2.8-1.8 4.6-5.3 4.6h-1.2V39zm4-6.8h1.1c1 0 1.5-0.4 1.5-1.1 0-0.7-0.5-1.1-1.5-1.1h-1.1z" fill="#fff"/></svg>')}" alt="" width="42" height="42" />
          <div>
...
function findLatestUtilityPayment(criteria) {
    const sorted = [...(state.utilityPayments ?? [])].sort((a, b) => new Date(b.paidAt).getTime() - new Date(a.paidAt).getTime());
    return (sorted.find((payment) => criteria.providerReference &&
        String(payment.providerReference ?? "").trim().toUpperCase() ===
            String(criteria.providerReference).trim().toUpperCase()) ??
        sorted.find((payment) => payment.utilityType === criteria.utilityType &&
            payment.billingMonth === criteria.billingMonth &&
            Math.round(Number(payment.amountKsh ?? 0)) === Math.round(Number(criteria.amountKsh ?? 0))) ?? null);
}
function findLatestRentPayment(criteria) {
    const sorted = [...(state.rentPayments ?? [])].sort((a, b) => new Date(b.paidAt).getTime() - new Date(a.paidAt).getTime());
    return (sorted.find((payment) => criteria.providerReference &&
        String(payment.providerReference ?? "").trim().toUpperCase() ===
            String(criteria.providerReference).trim().toUpperCase()) ??
        sorted.find((payment) => payment.billingMonth === criteria.billingMonth &&
            Math.round(Number(payment.amountKsh ?? 0)) === Math.round(Number(criteria.amountKsh ?? 0))) ?? null);
}
...
function updateResidentNavDots() {
    ...
    if (supportDot instanceof HTMLElement) {
        supportDot.classList.toggle("hidden", !canResidentAccessSupport() || openReports === 0);
    }
    if (paymentsDot instanceof HTMLElement) {
        paymentsDot.classList.toggle("hidden", rentOutstanding + utilityOutstanding <= 0);
    }
}
...
function applyPaymentAccessUi() {
    ...
    if (bothDisabled) {
        ...
        utilityPaymentStateEl.textContent = `${utilityLabel(selectedType)} payments are disabled by your landlord.`;
    }
    else {
        utilityPaymentStateEl.textContent =
...
function saveResidentToken(token, options = {}) {
    const rememberDevice = typeof options.rememberDevice === "boolean"
...
function setActiveResidentView(nextView, { scroll = false } = {}) {
    ...
    const targetView = requestedView === "support" && state.residentSession && !canResidentAccessSupport()
        ? "overview"
        : requestedView;
    ...
    if (targetView === "wifi" && !state.wifiStatusLoaded) {
        void loadResidentWifiStatus();
    }
}
function formatDataUsage(octets) {
    const bytes = Number(octets ?? 0);
...
function formatWifiDuration(durationSeconds) {
    const hours = Math.round(Number(durationSeconds ?? 0) / 3600);
    if (hours < 1) {
        return "Short access";
    }
    if (hours < 24) {
...
function renderResidentWifiStatus() {
    const status = state.wifiStatus;
    const plans = status?.plans ?? [];
    if (wifiBuyLinkEl instanceof HTMLAnchorElement && status?.buyUrl) {
        wifiBuyLinkEl.href = status.buyUrl;
    }
    if (status?.current) {
        wifiCurrentCardEl.classList.remove("hidden");
        wifiCurrentPlanNameEl.textContent = status.current.planName;
        wifiCurrentExpiryEl.textContent = `Expires ${new Date(status.current.expiresAt).toLocaleString()}`;
        const usedBytes = Number(status.current.totalInputOctets ?? 0) + Number(status.current.totalOutputOctets ?? 0);
        wifiCurrentUsageEl.textContent = `Data used so far: ${formatDataUsage(usedBytes)}`;
    }
    else {
        wifiCurrentCardEl.classList.add("hidden");
    }
    wifiEmptyEl.classList.toggle("hidden", Boolean(status?.current) || plans.length > 0);
    wifiPlansEl.replaceChildren();
    plans.forEach((plan) => {
        const card = document.createElement("div");
        card.className = "wifi-package-card";
        if (plan.isFree) {
            const badge = document.createElement("span");
            badge.className = "wifi-package-badge";
            ...
            const badge = document.createElement("span");
            badge.className = "wifi-package-badge";
            badge.textContent = `${plan.discountPercent}% off`;
            card.append(badge);
        }
        ...
        price.textContent = plan.isFree ? "Free" : formatCurrency(plan.priceKsh);
        const duration = document.createElement("div");
        duration.className = "wifi-package-meta";
        duration.textContent = formatWifiDuration(plan.durationSeconds);
        card.append(name, price, duration);
        wifiPlansEl.append(card);
    });
}
async function loadResidentWifiStatus() {
    wifiErrorEl.classList.add("hidden");
    try {
        const payload = await requestJson("/api/resident/wifi/status", {}, { auth: true });
        state.wifiStatus = payload.data ?? null;
        state.wifiStatusLoaded = true;
        renderResidentWifiStatus();
    }
    catch (error) {
        wifiErrorEl.textContent =
            error instanceof Error ? error.message : "Unable to load Wi-Fi status.";
...
function scrollToResidentPanel(targetView) {
    ...
    const panel = residentViewPanels.find((item) => item.dataset.residentViewPanel === targetView);
    if (!panel) {
        return;
    }
...
function renderOverviewSession() {
    ...
    overviewSessionExpiryEl.textContent = `${formatResidentVerificationLabel(session.verificationStatus)} account • Expires ${formatDateTime(session.expiresAt)}.`;
    syncResidentPaySummary();
}
...
function renderPwaControls() {
    ...
    if (installAppBtnEl instanceof HTMLButtonElement) {
        installAppBtnEl.classList.toggle("hidden", state.pwaInstalled || !deferredInstallPrompt);
    }
    ...
    if (pushEnableBtnEl instanceof HTMLButtonElement) {
        ...
        pushEnableBtnEl.classList.toggle("hidden", hasSubscription || pushPermission === "denied");
    }
...
function renderSmsControls() {
    ...
    if (smsStatusTextEl instanceof HTMLElement) {
        if (!hasSession) {
            ...
            smsStatusTextEl.textContent =
                categories.length > 0
                    ? `SMS updates are active for ${phoneMask} for ${categories.join(" and ")} notices.${senderId ? ` Sender ID: ${senderId}.` : ""}`
                    : `SMS updates are active for ${phoneMask}, but no billing category is selected yet.${senderId ? ` Sender ID: ${senderId}.` : ""}`;
        }
    }
...
async function loadResidentSmsConfig() {
    ...
    try {
        const payload = await requestJson("/api/user/notification-preferences", {}, { auth: true });
...
async function updateResidentNotificationPreferences(patch, { successMessage = "" } = {}) {
    if (!state.residentSession || isPasswordChangeRequired()) {
        showFeedback("Sign in with an active resident session before updating SMS alerts.");
        return null;
    }
    const payload = await requestJson("/api/user/notification-preferences", {
        method: "PATCH",
        headers: {
...
async function enableResidentSmsNotifications() {
    ...
    try {
        await updateResidentNotificationPreferences({ smsEnabled: true }, { successMessage: "SMS alerts enabled for this resident account." });
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Unable to enable SMS alerts.";
...
async function disableResidentSmsNotifications() {
    clearFeedback();
    try {
        await updateResidentNotificationPreferences({ smsEnabled: false }, { successMessage: "SMS alerts paused for this resident account." });
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Unable to pause SMS alerts.";
        showFeedback(message);
    }
}
async function registerResidentPushSubscription(subscription) {
    await requestJson("/api/user/push-subscriptions", {
        method: "POST",
        headers: {
            "content-type": "application/json"
        },
...
function showNonResidentSessionState(sessionInfo) {
    ...
    residentSessionSummaryEl.textContent = `Another ${role} session is active${formatSessionExpirySuffix(sessionInfo?.expiresAt)} Sign out before resident sign up/sign in.`;
}
...
function renderReports(reports) {
    ...
    reports.forEach((report) => {
        ...
        const guidance = report.status === "resolved"
...
function renderNotifications(notifications) {
    ...
    notificationCountEl.textContent = `${notifications.length} alert${notifications.length === 1 ? "" : "s"}`;
    ...
    notifications.forEach((notification) => {
        ...
        fragment.querySelector(".item-meta").textContent = formatDateTime(notification.createdAt);
...
function renderRentDue(rentDue, fallbackMessage) {
  ...
  `;
    ...
    if (!String(rentPaymentAmountEl.value ?? "").trim() && rentDue.balanceKsh > 0) {
        rentPaymentAmountEl.value = formatAmountValue(computeSuggestedStarterAmount(rentDue.balanceKsh));
    }
    syncRentBillingMonthOptions();
    updateRentPaymentGuidance();
}
function renderUtilityBills(bills, meters = [], fallbackMessage, latestReadings = []) {
    ...
    const latestReadingCount = appendLatestUtilityReadingCards(utilityBillsListEl, latestReadings, meters);
    ...
    if (outstanding > 0) {
        utilityBillsSummaryEl.innerHTML = `Outstanding utility balance: <strong>${formatCurrency(outstanding)}</strong>${outstandingCount > 1 ? ` across ${outstandingCount} open bills.` : " ready to pay now."}`;
    }
    else {
    ...
    const focusBill = selectedOutstandingBills.find((bill) => bill.billingMonth === selectedMonth) ??
    ...
    if (focusBill) {
        ...
        const details = document.createElement("p");
        details.className = "item-details";
        const readingDetails = focusBill.meterNumber === "NO-METER"
            ? `Fixed charge ${formatCurrency(focusBill.fixedChargeKsh)}`
            : `Reading ${Number(focusBill.previousReading ?? 0).toLocaleString("en-US")} -> ${Number(focusBill.currentReading ?? 0).toLocaleString("en-US")}`;
        details.innerHTML = `Paying against <strong>${formatCurrency(focusBill.balanceKsh)}</strong> of ${formatCurrency(focusBill.amountKsh)} • Due <strong>${formatDateTime(focusBill.dueDate)}</strong> • ${readingDetails}`;
        const meta = document.createElement("dl");
        meta.className = "utility-bill-meta";
        meta.innerHTML = `
      <div>
    ...
    const otherOutstandingBills = outstandingBills.filter((bill) => !focusBill ||
    ...
    if (otherOutstandingBills.length > 0) {
        ...
        const title = document.createElement("strong");
        title.className = "item-title";
        title.textContent = `${otherOutstandingBills.length} other open utility bill${otherOutstandingBills.length === 1 ? "" : "s"}`;
        const chip = document.createElement("span");
        chip.className = "item-chip chip-due_soon";
        chip.textContent = "switch in form";
        top.append(title, chip);
        const utilityCounts = otherOutstandingBills.reduce((counts, bill) => {
            counts[bill.utilityType] = (counts[bill.utilityType] ?? 0) + 1;
            return counts;
        }, { water: 0, electricity: 0 });
        const otherOutstandingTotal = otherOutstandingBills.reduce((sum, bill) => sum + Number(bill.balanceKsh), 0);
        const details = document.createElement("p");
        details.className = "item-details";
        details.textContent = `Water ${utilityCounts.water} • Electricity ${utilityCounts.electricity} • Remaining ${formatCurrency(otherOutstandingTotal)}. Payments continue across these open bills automatically.`;
...
function syncUtilityBillingMonthOptions() {
    ...
    const nextSelectedMonth = outstandingBills.find((bill) => bill.billingMonth === currentSelectedMonth)?.billingMonth ??
...
function syncUtilityBillCardSelection() {
    ...
    utilityBillsListEl
        ...
        const isSelected = node.dataset.utilityType === selectedType &&
...
function syncUtilityPaymentFormFromBalances() {
    ...
    const selectedMonth = utilityPaymentMonthEl instanceof HTMLSelectElement &&
    ...
    renderUtilityBills(state.utilityBills, state.utilityMeters, undefined, state.utilityLatestReadings);
    syncUtilityBillCardSelection();
    if (!bill) {
        utilityPaymentBalanceEl.textContent = `No ${utilityLabel(utilityType).toLowerCase()} balance is open right now.`;
        updateUtilityPaymentGuidance();
        return;
    }
    const totalOutstanding = getTotalOutstandingUtilityBalanceForType(utilityType);
    ...
    utilityPaymentBalanceEl.textContent =
        outstandingCount > 1
            ? `${utilityLabel(utilityType)} payments auto-apply from ${bill.billingMonth}. Total open balance: ${formatCurrency(totalOutstanding)} across ${outstandingCount} bills.`
            : `${utilityLabel(utilityType)} ${bill.billingMonth} balance ready: ${formatCurrency(bill.balanceKsh)}.`;
...
function renderUtilityPayments(payments, fallbackMessage) {
    ...
    utilityPaymentsCountEl.textContent = `${payments.length} payment${payments.length === 1 ? "" : "s"}`;
    payments.slice(0, 8).forEach((payment) => {
        ...
        details.textContent = `${payment.billingMonth ?? "latest"} • ${formatDateTime(payment.paidAt)}`;
...
function renderRentPayments(payments, fallbackMessage) {
    ...
    rentPaymentsCountEl.textContent = `${payments.length} payment${payments.length === 1 ? "" : "s"}`;
    payments.slice(0, 8).forEach((payment) => {
        const card = document.createElement("article");
        card.className = "stack-item receipt-history-item";
...
function toggleInlineReceiptPanel(toggleBtn) {
...
function showSignedInState() {
    ...
    residentSessionSummaryEl.textContent = `House ${session.houseNumber} (${session.phoneMask}) • ${formatResidentVerificationLabel(session.verificationStatus)} • Expires ${formatDateTime(session.expiresAt)}`;
    renderOverviewSession();
    renderResidentIdentityNotice();
...
async function loadTenantData() {
    ...
    try {
        ...
        renderUtilityBills(state.utilityBills, state.utilityMeters, messages.utilities, state.utilityLatestReadings);
        syncUtilityPaymentFormFromBalances();
        ...
        const message = error instanceof Error ? error.message : "Unable to load resident data.";
        showFeedback(message);
    }
}
...
async function requestResidentPasswordRecovery() {
    ...
    if (!payload.buildingId || !payload.houseNumber || !payload.phoneNumber) {
        showResidentAuthFeedback("Provide building, house number, and phone number for password recovery.", "error", { reveal: true });
        return;
    }
    residentForgotBtnEl.disabled = true;
    try {
        ...
        showResidentAuthFeedback(response.message ??
            "Recovery request received. Management will share a temporary password after verification.", "success", { reveal: true });
    }
    catch (error) {
        const message = error instanceof Error
            ? error.message
...
async function loginResident(event) {
    ...
    try {
        ...
        if (isPasswordChangeRequired()) {
            showFeedback("Temporary password detected. Set a new password before using the portal.", "success");
        }
        else {
            showFeedback(isResidentPendingReview()
        ...
        const message = error instanceof Error ? error.message : "Unable to sign in resident.";
        showResidentAuthFeedback(message, "error", { reveal: true });
    }
    finally {
        residentLoginBtnEl.disabled = false;
...
async function signupResident() {
    ...
    if (await syncAuthConflictState()) {
        showResidentAuthFeedback("Another account is signed in. Sign out first, then create resident account.", "error", { reveal: true });
        return;
    }
    const payload = buildResidentSignupPayload();
    if (!payload.buildingId || !payload.houseNumber || !payload.phoneNumber) {
        showResidentAuthFeedback("Choose the building, enter your house number, and add your phone number to request access.", "error", { reveal: true });
        return;
    }
    if (!payload.password || payload.password.length < 8) {
        showResidentAuthFeedback("Set a password with at least 8 characters to request access.", "error", { reveal: true });
        return;
    }
    residentSignupBtnEl.disabled = true;
    residentLoginBtnEl.disabled = true;
    try {
        ...
        showFeedback(response.message ??
            "Access request submitted. You are signed in now and pending landlord review.", "success");
        if (!isPasswordChangeRequired()) {
            await loadTenantData();
        }
    }
...
async function submitResidentPasswordChange(event) {
    ...
    try {
        const response = await requestJson("/api/auth/resident/change-password", {
            method: "POST",
            headers: {
                "content-type": "application/json"
            },
...
async function submitTicket(event) {
    ...
    try {
        ...
        showFeedback(`Request ${report?.id?.slice(0, 8) ?? ""} submitted successfully.`, "success");
        resetReportForm();
        await loadTenantData();
    }
    catch (error) {
...
async function submitUtilityMpesaPayment({ utilityType, billingMonth, amountKsh }) {
    ...
    const response = await requestJson(`/api/user/utilities/${encodeURIComponent(utilityType)}/payments/mpesa/initialize`, {
        method: "POST",
        headers: {
            "content-type": "application/json"
        },
    ...
    state.activeMpesaFlow = {
        ...
        copy: `A Safaricom STK prompt has been sent to ${response.data?.phoneMask ?? "your phone"}. Enter your M-PESA PIN on your phone to complete this ${utilityLabel(utilityType).toLowerCase()} payment.`,
        note: response.data?.customerMessage ??
            "If the prompt delays, keep your phone unlocked and tap Check status below."
    };
    syncUtilityPaymentProviderUi();
    state.mpesaStatusModalDismissed = false;
    openMpesaStatusModal(null, { force: true });
    showFeedback("M-PESA prompt sent for utility payment. Complete it on your phone.", "success");
...
async function pollUtilityMpesaPayment(checkoutRequestId, utilityType, { manual = false } = {}) {
    openMpesaStatusModal({
        ...
        badge: "Checking",
        copy: `Checking whether your ${utilityLabel(utilityType).toLowerCase()} payment has been approved in M-PESA.`,
        note: manual
    ...
    try {
        const response = await requestJson(`/api/user/utilities/${encodeURIComponent(utilityType)}/payments/mpesa/verify`, {
            method: "POST",
            headers: {
                "content-type": "application/json"
            },
        ...
        if (status === "paid") {
            ...
            showFeedback(synced
                ...
                : "M-PESA utility payment confirmed. Receipt will appear shortly.", "success");
            if (payment) {
                openPaymentReceiptModal(buildUtilityReceiptRecord(payment));
            }
            return;
        ...
        if (state.utilityPaymentPollAttempts >= RENT_PAYMENT_POLL_MAX_ATTEMPTS) {
            ...
            showFeedback("Still waiting for M-PESA utility confirmation. Reopen the status box or tap Check status again.");
            return;
        }
        openMpesaStatusModal({
            stage: "pending",
        ...
        stopUtilityPaymentPolling();
        const message = error instanceof Error ? error.message : "Unable to verify M-PESA utility payment.";
        state.utilityPaymentPollAttempts = 0;
        utilityPaymentBtnEl.disabled = false;
        syncUtilityPaymentProviderUi();
        openMpesaStatusModal({
...
async function submitUtilityPayment(event) {
    ...
    try {
        ...
        await requestJson(`/api/user/utilities/${encodeURIComponent(utilityType)}/payments`, {
            method: "POST",
            headers: {
                "content-type": "application/json"
            },
        ...
        const message = error instanceof Error ? error.message : "Unable to submit utility payment.";
        showFeedback(message);
    }
    finally {
...
async function pollRentMpesaPayment(checkoutRequestId, { manual = false } = {}) {
    ...
    try {
        const response = await requestJson("/api/user/rent/payments/mpesa/verify", {
            method: "POST",
            headers: {
                "content-type": "application/json"
            },
        ...
        if (status === "paid") {
            ...
            showFeedback(synced
                ...
                : "M-PESA payment confirmed. Receipt will appear shortly.", "success");
            if (payment) {
                openPaymentReceiptModal(buildRentReceiptRecord(payment));
            }
            return;
        ...
        if (state.rentPaymentPollAttempts >= RENT_PAYMENT_POLL_MAX_ATTEMPTS) {
            ...
            const note = response.data?.resultDesc ??
        ...
        stopRentPaymentPolling();
        const message = error instanceof Error ? error.message : "Unable to verify M-PESA payment.";
        state.rentPaymentPollAttempts = 0;
        rentPaymentBtnEl.disabled = false;
        syncRentPaymentButtonUi();
        openMpesaStatusModal({
...
async function submitRentPayment(event) {
    ...
    clearFeedback();
    if (!RESIDENT_ONLINE_RENT_PAYMENT_ENABLED) {
        showFeedback("Online rent payment is not enabled yet. Use the payment instructions and wait for management to confirm your rent receipt.", "info");
        focusResidentPaymentSection(paymentInstructionsCardEl, null);
        return;
    }
    ...
    rentPaymentBtnEl.disabled = true;
    try {
        const response = await requestJson("/api/user/rent/payments/mpesa/initialize", {
            method: "POST",
            headers: {
                "content-type": "application/json"
            },
        ...
        state.activeMpesaFlow = {
            ...
            badge: "Waiting for PIN",
            copy: `A Safaricom STK prompt has been sent to ${response.data?.phoneMask ?? "your phone"}. Enter your M-PESA PIN on your phone to complete this rent payment.`,
            note: response.data?.customerMessage ??
                "If the prompt delays, keep your phone unlocked and tap Check status below."
        };
        syncRentPaymentButtonUi();
        state.mpesaStatusModalDismissed = false;
        openMpesaStatusModal(null, { force: true });
        showFeedback("M-PESA prompt sent. Complete it on your phone to post rent payment.", "success");
        scheduleRentPaymentPolling(checkoutRequestId);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Unable to initialize M-PESA payment.";
        showFeedback(message);
        rentPaymentBtnEl.disabled = false;
    }
    finally {
...
async function signOutResident() {
    ...
    try {
        await requestJson("/api/auth/resident/logout", {
            method: "POST"
        }, { auth: true });
    }
    catch (_error) {
...
function startResidentPortal() {
    ...
    if (userMenuToggleEl && userMenuPanelEl) {
        ...
        document.addEventListener("click", (event) => {
            ...
            if (userMenuPanelEl.contains(event.target) ||
    ...
    reportAttachmentsEl?.addEventListener("change", () => {
        try {
            ...
            const message = error instanceof Error ? error.message : "Unable to preview selected photos.";
            showFeedback(message);
            if (reportAttachmentsEl instanceof HTMLInputElement) {
                reportAttachmentsEl.value = "";
            }
    ...
    document
        ...
        button.addEventListener("click", () => {
            const mode = button.dataset.quick || "full";
            const utilityType = String(utilityPaymentTypeEl.value || "water");
            applyQuickPayAmount(utilityPaymentAmountEl, getUtilityOutstandingBalance(utilityType), mode);
        });
    });
    document.querySelectorAll("[data-fixed-pay]").forEach((row) => {
        ...
        row.querySelectorAll("button[data-fixed]").forEach((button) => {
            ...
            button.addEventListener("click", () => {
                ...
                if (target === "rent") {
                    setPaymentAmountValue(rentPaymentAmountEl, getRentOutstandingBalance(), fixedAmount);
                    return;
                }
                const utilityType = String(utilityPaymentTypeEl.value || "water");
                setPaymentAmountValue(utilityPaymentAmountEl, getUtilityOutstandingBalance(utilityType), fixedAmount);
            });
        });
    });
