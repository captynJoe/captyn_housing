...
// --- Wi-Fi Department -------------------------------------------------
// Building-scoped, read-only access-history and live-session views that
// proxy through to the real CAPTYN Wi-Fi service.
...
if (adminWifiEntitlementsBodyEl instanceof HTMLElement) {
  adminWifiEntitlementsBodyEl.addEventListener("click", (event) => {
    ...
    const shouldProceed = window.confirm(
      "Revoke this Wi-Fi voucher? The customer will no longer be able to (re)authenticate. This does not force-disconnect an already-connected device."
    );
    if (!shouldProceed) {
      return;
    }
    ...
    void (async () => {
      try {
        await requestJson(
          `/api/admin/wifi/entitlements/${encodeURIComponent(entitlementId)}/revoke`,
          {
            method: "POST",
