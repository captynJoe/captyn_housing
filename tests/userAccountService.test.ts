...
test("resident phone login resolves the only active tenancy without building selection", async () => {
...
test("resident phone login requires room selection when phone has multiple active tenancies", async () => {
  const createdSessions: unknown[] = [];
  const user = {
    id: "tenant-1",
    fullName: "Tenant One",
      ...
      findMany: async () => [
        {
          id: "tenancy-rosa-a1",
          userId: user.id,
          buildingId: "BLDG-ROSA-STUDIO",
          unit: { houseNumber: "A1" },
          user,
          active: true
        },
        {
          id: "tenancy-village-14",
          userId: user.id,
          buildingId: "BLDG-VILLAGE-INN",
          unit: { houseNumber: "14" },
          user,
          active: true
        }
  ...
  await assert.rejects(
    () =>
      service.createResidentPhoneSession({
        phoneNumber: "0711635755",
        password: "tenant-secret"
      }),
    /RESIDENT_TENANCY_SELECTION_REQUIRED/
  );
  assert.equal(createdSessions.length, 0);
});

test("resident phone login uses requested building and house when multiple rooms exist", async () => {
  const tenancyQueries: unknown[] = [];
  const createdSessions: unknown[] = [];
  const user = {
    id: "tenant-1",
  ...
  const service = new UserAccountService({
    ...
    tenancy: {
      ...
      findMany: async () => {
        throw new Error("Scoped login should not run unscoped tenancy lookup");
      }
    },
    tenantApplication: {
      findFirst: async () => null
  ...
  assert.equal(result.buildingId, "BLDG-VILLAGE-INN");
  assert.equal(result.tenancyId, "tenancy-village-14");
  assert.equal(result.houseNumber, "14");
  assert.equal(createdSessions.length, 1);
  assert.deepEqual(tenancyQueries[0], {
    where: {
      active: true,
